...
merges in `computers/<id>/computer.json` and `services/<id>/{service,overlay}.json`, seeds each machine from its
`computers/<id>/root/` and derives each service's node,
