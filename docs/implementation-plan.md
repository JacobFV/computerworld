...
| H. Wasm/JS / `wasm`, the site's live machines | Facade signature and generated schema types | Ergonomic JS package, Node entry, Canvas machines in a page | Browser Wasm execution, zero episode requests, input/frame parity | Immediately with test facade; compile pure crates early | Real runtime as soon as slice passes |
