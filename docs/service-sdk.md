...
For synthetic websites, answer with `text/html` — see [HTML services](#html-services)
below, which is what every service in this repository does and what a new one should do.
Do not construct views by reading a service store directly from the browser.
...
## The `Page` format (legacy)

Before 0.2.0 a service returned a `Page` with
`application/vnd.computerworld.page+json`, a tree of seventeen data element kinds. It
is still served, and still rendered — `cw_web::page::to_document` converts it to HTML
on the way to the browser — so a world or a site seeded in this format keeps working.
New services should not use it; [html-migration.md](html-migration.md) is the recipe
for moving one over.

The original eight element kinds are headings, text, links, buttons, inputs, forms,
groups and image descriptions. A form's `PageAction` identifies a method, URL and
fields; the browser submits through networking.
