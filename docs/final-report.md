...
the episode** after static assets were loaded. That file is regenerated by
`scripts/test-browser.mjs`, so the figures here and the artifact cannot drift apart. Real browser pointer/keyboard input,
