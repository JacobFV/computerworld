...
```sh
npm install computerworld@0.1.1
```
...
`computerworld-0.1.1.tgz` asset of the GitHub release, after checking it against that
release's `SHA256SUMS`.

## Install a pinned bundle

Download from [v0.1.1](https://github.com/JacobFV/computerworld/releases/tag/v0.1.1)
and verify the accompanying `SHA256SUMS`. Choose:

- `computerworld-0.1.1-wasm-web.tar.gz`: browser ES module, Wasm and TypeScript declarations.
- `computerworld-0.1.1-wasm-node.tar.gz`: Node CommonJS module, Wasm, declarations and runnable Node demo.
- `computerworld-0.1.1-browser-demo.zip`: complete static interactive console.
...
```sh
tar -xzf computerworld-0.1.1-wasm-node.tar.gz
cd computerworld-0.1.1-wasm-node
...
await init();
console.log(engineVersion()); // 0.1.1
...
```sh
git clone --branch v0.1.1 https://github.com/JacobFV/computerworld.git
cd computerworld
