...
//
//   node scripts/site/build-docs.mjs            # writes site/docs/ and docs/README.md
//   node scripts/site/build-docs.mjs --check    # converts everything, writes nothing, and
...
const unpublished = [
  ...
  { file: 'archive/IMPLEMENTATION-CONTRACTS.md', kind: 'record', blurb: 'How the agents that built phase 3 divided file ownership between them.' },
];
...
function checkIndexIsComplete() {
  ...
  const missing = readdirSync(join(root, 'docs'))
    ...
    .sort();
  if (missing.length) {
    console.error(`docs/ has guides in neither \`sections\` nor \`unpublished\` in ${'scripts/site/build-docs.mjs'}:`);
...
function indexMarkdown() {
  ...
  lines.push('See [historical records](archive/README.md) and [binding contracts](contracts/README.md).', '');
...
if (check) {
  ...
  if (onDisk !== index) {
    console.error('docs/README.md is stale: run `node scripts/site/build-docs.mjs`');
    process.exit(1);
  }
} else {
