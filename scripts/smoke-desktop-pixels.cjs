...
click('shell:launch:terminal');step('keyboard.v1','type',{text:'echo desktop-pixel-parity'});step('keyboard.v1','key',{key:'Enter'});click('shell:maximize');
const frame=env.render(960,640);const pixelHash=crypto.createHash('sha256').update(frame.rgba).digest('hex');frame.free();
fs.writeFileSync(output,JSON.stringify({definition:def,session:env.id,snapshot:world.exportSnapshot(),stateHash:world.stateHash(),pixelHash}));
console.log(JSON.stringify({desktopPixelHash:pixelHash,stateHash:world.stateHash()}));
