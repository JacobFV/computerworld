...
async function target(action){const s=await scene();const n=s.nodes.find(n=>n.interaction===action);assert.ok(n,`missing interaction ${action}`);const t=n.transform??{a:1024,b:0,c:0,d:1024,tx:0,ty:0},x=n.bounds.x+n.bounds.width/2,y=n.bounds.y+n.bounds.height/2;return{x:(t.a*x+t.c*y)/1024+t.tx,y:(t.b*x+t.d*y)/1024+t.ty};}
...
try{
 ...
 for(const theme of ['macos','windows','ubuntu','ios','android']){
  ...
  if(!['ios','android'].includes(theme)){
   ...
   for(const delta of [{x:50,y:20},{x:-35,y:-15},{x:20,y:30}]){await drag(dragTarget,delta);const next=(await state()).desktop.windows[id].frame;assert.equal(next.x-previous.x,delta.x);assert.equal(next.y-previous.y,delta.y);previous=next;}checks.push('real pointer window dragging at four positions');
   ...
   const firstBrowser=await launch('browser');await act('browser.v1','navigate',{url:'http://intranet.internal/'});const secondBrowser=await launch('browser');await act('browser.v1','navigate',{url:'http://guide.example/'});assert.notEqual(firstBrowser,secondBrowser);await click(`window:${firstBrowser}:focus`);assert.match(JSON.stringify((await state()).browser),/intranet\.internal/);await click(`window:${secondBrowser}:focus`);assert.match(JSON.stringify((await state()).browser),/guide\.example/);checks.push('independent browser window histories');
   await shot(`${theme}-multiwindow`);await assertSnapshot();checks.push('snapshot and fork preserve full geometry and app state');
  }else{
   await click('shell:launch:browser');await act('browser.v1','navigate',{url:'http://intranet.internal/'});await shot(`${theme}-browser`);await click('shell:home');assert.equal((await state()).desktop.focused,null);checks.push('touch launcher and Home navigation');
  ...
  report.devices.push({id:dev.id,theme,checks});
 }
