...
#[ignore]
fn linear_stills() {
    let ctx = ServiceContext { actor: "alice".into(), source: "alice-mac".into(), tick: 1, seed: 1, instance: "linear".into() };
    let raw = std::fs::read_to_string("../../worlds/company-2026/sites/linear.json").unwrap();
    let site: Value = serde_json::from_str(&raw).unwrap();
    let mut state = IssuesService.initialize(site["initial_state"].clone(), &ctx).unwrap();
    for (path, file, height) in [
        ...
        let response = IssuesService.handle(&mut state, &ctx, &HttpRequest::get(format!("http://linear.app{path}"))).unwrap();
    ...
    let response = IssuesService.handle(&mut plain, &ctx, &HttpRequest::get("http://issues.internal/projects/OPS")).unwrap();
    support::still(&String::from_utf8(response.body).unwrap(), "issues-internal.png", 1280, 700);
}
