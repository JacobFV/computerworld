...
use cw_service_common::html::{
    self, button, div, el, form, hidden, href, label, link, span, text_input, Document, Html,
};
...
fn avatar(id: &str, who: &str, size: u32) -> Html {
    let initial: String = who
        .chars()
        .next()
        .map(|c| c.to_uppercase().collect())
        .unwrap_or_default();
    let node = idn(span("avatar"), id)
        .class(&format!("s{size}"))
        .attr("title", if who.is_empty() { "Unassigned" } else { who });
    if who.is_empty() {
        node.class("nobody")
    } else {
        node.style(&format!("background: {}", wire::avatar_tint(who)))
            .text(initial)
    }
}
...
fn status_icon(id: &str, status: &str) -> Html {
    idn(span("st"), id)
        .class(&format!("st-{status}"))
        .attr("title", column_label(status))
...
fn priority_icon(id: &str, issue: &Issue) -> Html {
    ...
    let node = idn(span("prio"), id)
        .class(&format!("prio-{level}"))
        .attr("title", name);
    match level {
        4 => node.text("!"),
        0 => node
            ...
            .child(el("i"))
            .class("none"),
        n => node.each(1..=3u8, |bar| {
            el("i").class(if bar <= n { "on" } else { "" })
        }),
    }
}
...
fn label_pill(id: &str, name: &str) -> Html {
    idn(span("pill"), id)
        .child(
            el("i")
                .class("dot")
                .style(&format!("background: {}", wire::avatar_tint(name))),
        )
        .child(Html::from(name))
}
fn label_pills(prefix: &str, issue: &Issue) -> Vec<Html> {
    issue
        ...
        .iter()
        .enumerate()
        .map(|(i, l)| label_pill(&format!("{prefix}-label-{i}"), l))
        .collect()
}
...
fn rail(look: &Look, projects: &[(String, String)], spot: Spot, view: View) -> Html {
    ...
    let mut nav = el("nav")
        .class("teams")
        .child(sp("teams-label", "rail-label", "Your teams"));
    for (key, name) in projects {
        ...
        nav = nav.child(
            el("a")
                ...
                .child(
                    span("team-icon")
                        .style(&format!("background: {}", wire::avatar_tint(key)))
                        .text(key.chars().next().map(String::from).unwrap_or_default()),
                )
        ...
        if here {
            ...
            nav = nav
                .child(sub("issues", "Issues", View::List))
                .child(sub("board", "Board", View::Board))
                .child(sub("cycle", "Cycles", View::Cycle));
        }
    }
    let mut links = div("rail-links")
        .child(sp("", "rail-label", "Workspace"))
        .child(
            link("nav-projects", "/", "Projects").class(if spot == Spot::Home {
                "rail-item current"
            } else {
                "rail-item"
            }),
    ...
    if !look.actor.is_empty() {
        links = links.child(
            link(
                "nav-mine",
                href("/search", &[("assignee", look.actor.as_str())]),
                "My issues",
            )
            .class(if spot == (Spot::Search { mine: true }) {
                "rail-item current"
            } else {
                "rail-item"
            }),
        ...
        .child(
            div("rail-top").child(
                el("a")
                    ...
                    .class("workspace")
                    .attr("href", "/")
                    .child(span("workspace-logo").text(initial))
                    .child(Html::from(look.brand())),
            ),
        )
        .child(
            form("search", "/search", "get")
                .class("search")
                .child(
                    text_input("search-q", "q", &look.query)
...
fn document(look: &Look, title: &str, rail: Html, main: Html) -> Result<HttpResponse> {
    let mut doc = Document::new(title)
        .lang("en")
        .stylesheet(CSS)
        .body_class("skin-linear")
        .body([div("shell").id("shell").child(rail).child(main)]);
...
fn progress(done: usize, total: usize) -> Html {
    ...
    span("progress")
        .child(span("bar").child(el("i").style(&format!("width: {percent}%"))))
        .child(span("percent").text(format!("{percent}%")))
}
...
pub fn home(look: &Look, projects: &[(String, Project)]) -> Result<HttpResponse> {
    ...
    for (key, project) in projects {
        ...
        table = table.child(
            el("a")
                ...
                .child(
                    span("c-health")
                        .child(span(if blocked > 0 {
                            "health at-risk"
                        } else {
                            "health"
                        }))
                        .child(Html::from(if blocked > 0 { "At risk" } else { "On track" })),
                )
                .child(
                    span("c-lead")
                        .child(avatar("", lead, 18))
                        .child(Html::from(if lead.is_empty() { "No lead" } else { lead })),
                )
                .child(span("c-open").child(sp(
                    &format!("team-open-{key}"),
                    "count",
                    format!("{} unfinished", total - done),
                )))
    ...
    let main = el("main")
        ...
        .child(
            el("header")
                .class("topbar")
                .child(el("h1").id("home-title").text("Projects"))
                .child(span("tabs").child(span("tab current").text("All projects"))),
        )
        .child(
            el("p")
                .id("home-sub")
                .class("sub")
                .text("Your teams: boards, cycles and everything still open."),
        )
        .child(table);
    document(
        look,
        &format!("{} · Linear", look.brand()),
        rail(look, &names, Spot::Home, View::List),
        main,
    )
}
    ...
    if searching && found.is_empty() {
        list = list.child(
            el("p")
                .id("search-empty")
                .class("empty")
                .text("No issue matches. Try a word from a title, a label or an assignee."),
        );
    }
    let main = el("main")
        .id("main")
        ...
        .child(
            el("header")
                .class("topbar")
                .id("search-head")
                .child(el("h1").id("search-title").text(&heading)),
        )
        .child(el("p").id("search-sub").class("sub").text(sub))
        .child(list);
    document(
        look,
        &format!("{heading} · {}", look.brand()),
        rail(look, projects, Spot::Search { mine }, View::List),
        main,
    )
}
...
fn list_row(key: &str, issue: &Issue, view: View) -> Html {
    ...
    div("row")
        ...
        .child(
            el("a")
                .id(format!("card-open-{id}"))
                .class("title")
                .attr("href", format!("/projects/{key}/issues/{id}"))
                .child(sp(&format!("card-title-{id}"), "", &issue.title)),
        )
        .child(span("grow"))
        .child(
            span("pills")
                .id(format!("card-head-{id}"))
                .children(label_pills(&format!("card-{id}"), issue)),
        )
        .child(moves(key, issue, view))
        .child(
            span("who")
                .id(format!("card-who-{id}"))
                .child(avatar(&format!("card-avatar-{id}"), &issue.assignee, 18))
                .child(sp(
                    &format!("card-assignee-{id}"),
                    "assignee",
                    &issue.assignee,
                )),
        )
...
pub fn board(
    ...
    for (status, name) in COLUMNS {
        ...
        let mut group = el("section")
            .id(format!("col-{status}"))
            .class(if view == View::Board {
                "column"
            } else {
                "group"
            })
        ...
        for issue in members {
            group = group.child(if view == View::Board {
                board_card(key, issue, view)
            } else {
                list_row(key, issue, view)
            });
        }
    ...
    let mut filters = div("filters")
        .id("filters")
        .child(sp("", "filter-label", "Assignee"))
        .child(
            link("filter-all", to(None, view), "All").class(if assignee.is_none() {
                "chip current"
            } else {
                "chip"
            }),
    ...
    let tab = |id: &str, text: &str, v: View| {
        link(id, to(assignee, v), text).class(if v == view { "tab current" } else { "tab" })
    };
    let total = project.issues.len();
    let done = project
        .issues
    ...
    let started = project
        .issues
        .values()
        .filter(|i| i.status == "in_progress" || i.status == "blocked")
        .count();
    let mut main = el("main").id("main").class("main").child(
        el("header")
            ...
            .child(
                span("tabs")
                    .child(tab("view-list", "All issues", View::List))
                    .child(tab("view-board", "Board", View::Board))
                    .child(tab("view-cycle", "Current cycle", View::Cycle)),
            )
            .child(span("grow")),
    );
    if view == View::Cycle {
        main = main.child(
            div("cycle")
                ...
                .child(
                    div("cycle-title")
                        .child(el("h2").id("cycle-title").text("Cycle 12"))
                        .child(span("pill current").text("Current"))
                        .child(span("muted").text("Two weeks · ends Friday")),
                )
                .child(
                    div("cycle-stats")
                        .child(span("stat").child(span("muted").text("Scope")).child(sp(
                            "cycle-scope",
                            "strong",
                            total.to_string(),
                        )))
                        .child(span("stat").child(span("muted").text("Started")).child(sp(
                            "cycle-started",
                            "strong",
                            started.to_string(),
                        )))
                        .child(
                            span("stat")
                                .child(span("muted").text("Completed"))
                                .child(sp("cycle-done", "strong", done.to_string())),
                        )
        ...
        el("section")
            ...
            .child(
                form("new-issue", format!("/projects/{key}/issues"), "post")
                    .class("new-issue")
                    .child(
                        text_input("new-issue-title", "title", "")
                            .attr("placeholder", "Issue title")
                    ...
                    .child(
                        div("actions")
                            .child(button("new-issue-submit", "Create issue").class("primary")),
                    ),
            ),
    );
    document(
        ...
        rail(look, projects, Spot::Team(key), view),
        main,
    )
}
...
pub fn issue(
    ...
    if !item.source_ref.is_empty() {
        feed = feed.child(div("event").child(span("glyph").text("⎇")).child(sp(
            "issue-refs",
            "mono",
    ...
    for (i, c) in item.comments.iter().enumerate() {
        feed = feed.child(
            div("comment")
                ...
                .child(
                    div("prose")
                        .id(format!("comment-body-{i}"))
                        .children(linked(&format!("comment-body-{i}"), &c.body)),
                ),
        );
    }
    ...
    let mut props =
        el("aside")
            .id("properties")
            .class("props")
            .child(sp("", "rail-label", "Properties"));
    let prop = |name: &str, value: Html| {
        div("prop")
            .child(span("prop-name").text(name))
            .child(span("prop-value").child(value))
    };
    props = props
        .child(prop(
            "Status",
            Html::Fragment(vec![
                ...
                sp("issue-state", "", column_label(&item.status)),
            ]),
        ))
        .child(prop(
            "Priority",
            Html::Fragment(vec![
                priority_icon("", item),
                sp("issue-priority", "", priority(item).0),
            ]),
        ))
        .child(prop(
            "Assignee",
        ...
        .child(prop(
            "Labels",
            span("pills")
                .children(label_pills("issue", item))
                .when(item.labels.is_empty(), |n| {
                    n.child(span("muted").text("No labels"))
                }),
        ))
        .child(prop(
            "Project",
            link("issue-project", format!("/projects/{key}"), team),
        ));
    let mut moves =
        div("status-moves")
            .id("status-moves")
            .child(sp("", "rail-label", "Change status"));
            ...
            form(&format!("status-{status}-form"), base.clone(), "post")
                ...
                .child(
                    el("button")
                        ...
                        .child(Html::from(format!("Move to {name}"))),
                ),
        );
    }
    props = props.child(moves).child(
        form("update", base.clone(), "post")
            ...
            .child(
                text_input("update-assignee", "assignee", &item.assignee)
            ...
            div("issue-columns")
                .child(
                    el("article")
                        ...
                        .child(div("prose body").id("issue-body").child(
                            idn(div(""), "body").children(if item.body.trim().is_empty() {
                                vec![span("muted").text("Add description…")]
                            } else {
                                linked("body", &item.body)
                            }),
                        ))
    ...
    document(
        ...
        rail(look, projects, Spot::Team(key), View::List),
        main,
    )
}
