...
use cw_service_common::html::{self, button, div, el, form, label, link, span, text_input, Document, Html};
...
fn document(title: &str, query: &str, trail: &[(&str, Option<String>)], main: Html) -> Result<HttpResponse> {
    let mut bar = el("header").class("masthead").child(link("home", "/", "Issues").class("brand"));
    ...
    bar = bar.child(
        form("search", "/search", "get")
            .class("search")
            .child(text_input("search-q", "q", query).attr("placeholder", "Search issues").attr("aria-label", "Search issues"))
            .child(button("search-go", "Search")),
    );
    html::page(&Document::new(title).lang("en").stylesheet(CSS).body_class("skin-plain").body([bar, main]))
}
...
fn fields(id: &str, url: &str, title: &str, inputs: &[(&str, &str, &str)]) -> Html {
    let mut node = form(id, url, "post").class("panel").child(el("h2").text(title));
    for (key, text, value) in inputs {
        let input = format!("{id}-{key}");
        node = node.child(div("field").child(label(&input, *text)).child(text_input(&input, key, value)));
    }
...
pub(crate) fn projects(visible: &[(String, String)]) -> Result<HttpResponse> {
    let list = el("ul").class("list").each(visible.iter(), |(id, name)| {
        el("li").child(link(&format!("project-{id}"), format!("/projects/{id}"), name.as_str())).child(span("muted").text(format!(" {id}")))
    });
    document("Projects", "", &[], el("main").child(el("h1").id("title").text("Projects")).child(list))
}
pub(crate) fn project(key: &str, project: &Project) -> Result<HttpResponse> {
    ...
    let mut table = el("table").class("issues").child(
        el("thead").child(el("tr").child(el("th").text("Id")).child(el("th").text("Summary")).child(el("th").text("Status")).child(el("th").text("Assignee"))),
    );
    ...
    for (id, item) in &project.issues {
        rows = rows.child(
            el("tr")
                ...
                .child(el("td").child(link(&format!("issue-{id}"), format!("/projects/{key}/issues/{id}"), item.title.as_str())))
                .child(el("td").child(span("status").class(&format!("s-{}", item.status)).text(item.status.as_str())))
    ...
    let main = el("main")
        ...
        .child(table)
        .child(fields("new-issue", &format!("/projects/{key}/issues"), "New issue", &[("title", "Title", ""), ("body", "Description", "")]));
...
pub(crate) fn issue(project: &str, issue: &Issue) -> Result<HttpResponse> {
    ...
    let mut main = el("main")
        ...
        .child(el("h1").id("title").text(format!("{project}-{} {}", issue.id, issue.title)))
        .child(el("p").id("status").class("meta").text(match issue.assignee.as_str() {
    ...
    for (i, c) in issue.comments.iter().enumerate() {
        main = main.child(el("p").id(format!("comment-{i}")).class("note").text(format!("{}: {}", c.author, c.body)));
    }
    for (i, r) in issue.reviews.iter().enumerate() {
        main = main.child(el("p").id(format!("review-{i}")).class("note review").text(format!("{}: {} {}", r.author, r.decision, r.body)));
    }
    main = main
        .child(fields("comment", &format!("{base}/comments"), "Add a comment", &[("body", "Comment", "")]))
        .child(fields("update", &base, "Update", &[("status", "Status", &issue.status), ("assignee", "Assignee", &issue.assignee)]));
    if issue.kind == "pull_request" {
        main = main.child(fields("review", &format!("{base}/reviews"), "Review", &[("decision", "Decision", "approve"), ("body", "Review", "")]));
    }
    document(&issue.title, "", &[(project, Some(format!("/projects/{project}")))], main)
}
/// `/search`: every issue of every readable project that matches, as one list.
pub(crate) fn results(found: &[(String, Issue)], query: &str, assignee: Option<&str>) -> Result<HttpResponse> {
    ...
    let summary = if !searching {
        ...
        format!("{} {} found.", found.len(), if found.len() == 1 { "issue" } else { "issues" })
    };
    let list = el("ul").class("list").each(found.iter(), |(key, item)| {
        el("li")
            .child(link(&format!("result-{key}-{}", item.id), format!("/projects/{key}/issues/{}", item.id), format!("{key}-{}: {}", item.id, item.title)))
            .child(span("muted").text(format!(" {}", item.status)))
    });
    let main = el("main").child(el("h1").id("title").text(&heading)).child(el("p").id("search-sub").class("meta").text(summary)).child(list);
