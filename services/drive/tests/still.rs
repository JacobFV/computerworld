    ...
    for (site, actor, pages) in [
        (
            ...
            &[
                ...
                ("http://drive.google.com/drive/folders/f-atlas", "google-drive-folder.png"),
                ("http://drive.google.com/file/press-kit", "google-drive-file.png"),
            ][..],
        ),
        (
            "dropbox",
            ...
            &[
                ...
                ("http://dropbox.com/drive/folders/atlas-assets", "dropbox-folder.png"),
                ("http://dropbox.com/file/press-kit", "dropbox-file.png"),
            ][..],
        ),
    ] {
        let service = world["services"].as_array().unwrap().iter().find(|s| s["id"] == site).unwrap();
        let ctx = ServiceContext { actor: actor.into(), source: "alice-mac".into(), tick: 1, seed: 1, instance: site.into() };
        let mut state = DriveService.initialize(service["initial_state"].clone(), &ctx).unwrap();
        for (url, file) in pages {
            let response = DriveService.handle(&mut state, &ctx, &HttpRequest::get(*url)).unwrap();
