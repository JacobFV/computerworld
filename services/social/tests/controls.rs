...
use cw_service_social::SocialService;
use serde_json::Value;
use std::collections::BTreeSet;

const ACTORS: [&str; 2] = ["alice", "bob"];
const SITES: [&str; 7] = ["x-social", "bsky", "mastodon", "facebook", "instagram", "linkedin", "pinterest"];
/// Where a sweep starts. Everything else is reached by following links.
const SEEDS: &[&str] = &["/", "/explore", "/search", "/messages", "/messaging"];
/// Links that may land on the page they sit on, because the real product's do too.
const ALLOW_SELF: &[&str] = &["brand", "rail-me", "side-me", "card-me", "short-me", "nav-me", "top-me"];
...
/// The pages that carry a link off the site: an account's own `site` field.
fn off_site_pages(seed: &Value) -> Vec<String> {
    seed["accounts"].as_object().into_iter().flatten().map(|(handle, _)| format!("/{handle}")).collect()
}
