...
fn render(html: &str, file: &str, viewport: Viewport) {
    ...
    let target = if file.starts_with('/') { PathBuf::from(file) } else { PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../research/site-stills").join(file) };
...
fn forum_site_stills() {
    let ctx = ServiceContext { actor: "bob".into(), source: "bob-linux".into(), tick: 60, seed: 1, instance: "forum".into() };
    ...
    for (site, host, thread) in [
        ...
        let mut state = ForumService.initialize(file["initial_state"].clone(), &ctx).unwrap();
        for (url, name) in [(format!("http://{host}/"), format!("{site}.png")), (format!("http://{host}{thread}"), format!("{site}-thread.png"))] {
            let response = ForumService.handle(&mut state, &ctx, &HttpRequest::get(&url)).unwrap();
...
#[ignore]
fn scratch() {
    let Ok(path) = std::env::var("FORUM_SCRATCH") else { return };
    let html = std::fs::read_to_string(&path).unwrap();
    let size = std::env::var("FORUM_SCRATCH_SIZE").unwrap_or_default();
    let (w, h) = size.split_once('x').and_then(|(w, h)| Some((w.parse().ok()?, h.parse().ok()?))).unwrap_or((400, 200));
    render(&html, &format!("{path}.png"), Viewport { width: w, height: h, scale: 1, zoom: 100 });
}
