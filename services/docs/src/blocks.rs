...
pub(crate) fn thumbnail(body: &str, lines: usize) -> Html {
    el("div").class("mini").each(
        parse(body)
            .into_iter()
            .filter(|b| b.kind != Kind::Gap)
            .take(lines),
        |b| {
            let text = b
                .words
                ...
                .join(" ");
            el("p")
                .class(if b.kind == Kind::Heading { "h" } else { "" })
                .text(text)
        },
    )
}
...
mod tests {
    ...
    fn bodies_read_as_headings_lists_todos_and_wrapped_lines_with_stable_word_positions() {
        ...
        let expected: Vec<usize> = body
            ...
            .map(|(i, _)| i)
            .collect();
        let found: Vec<usize> = blocks
            .iter()
            .flat_map(|b| &b.words)
            .filter(|w| w.1.starts_with("http://"))
            .map(|w| w.0)
            .collect();
        ...
        assert_eq!(
            blocks[6].words.last().unwrap().1,
            "http://youtube.com/watch?v=1",
            "the wrapped line joined its item"
        );
    }
}
