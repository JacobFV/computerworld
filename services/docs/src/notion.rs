    ...
    el("aside")
        ...
        .child(
            el("nav")
                ...
                .child(
                    link("side-home", "/", "Home").class(if current.is_none() && !starred {
                        "on"
                    } else {
                        ""
                    }),
                ...
                .child(
                    link("side-starred", "/starred", "Favorites").class(if starred {
                        "on"
                    } else {
                        ""
                    }),
        ...
        .child(el("nav").class("pages").attr("aria-label", "Pages").each(
            s.visible(actor, kind),
            |d| {
                el("a")
                    .id(d.id.as_str())
                    .class(if current == Some(d.id.as_str()) {
                        "page-link on"
                    } else {
                        "page-link"
                    })
    ...
    if let Some(d) = d {
        ...
        bar = bar
            ...
            .child(
                span("edited")
                    .id("crumb-meta")
                    .text(format!("Revision {} · owner {}", d.revision, d.owner)),
            )
            .child(
...
fn block_field(form_id: &str, name: &str, text: &str, value: &str) -> Html {
    ...
    div("prop").child(label(&id, text)).child(
...
fn block_area(form_id: &str, name: &str, text: &str, value: &str, rows: u32) -> Html {
    let id = format!("{form_id}-{name}");
    div("prop tall").child(label(&id, text)).child(
        el("textarea")
            .id(id.as_str())
            .attr("name", name)
        ...
        .each(blocks::parse(&d.body).iter(), |b| {
            ...
            match &b.kind {
                ...
                Kind::Todo(done) => div(&format!("block item todo{depth}"))
                    ...
                    .child(
                        span(if *done { "txt struck" } else { "txt" })
                            .children(blocks::inline(&b.words)),
                    ),
                Kind::Numbered(n) => div(&format!("block item{depth}"))
...
fn document(d: &Document, actor: &str) -> Html {
    ...
    let mut page = el("article")
        ...
        .child(
            div("props")
                .child(
                    div("prop-row")
                        .child(span("k").text("Owner"))
                        .child(span("v").text(capital(&d.owner))),
                )
                .child(
                    div("prop-row")
                        .child(span("k").text("Revision"))
                        .child(span("v").text(d.revision.to_string())),
                )
                .child(
                    div("prop-row")
                        .child(span("k").text("Comments"))
                        .child(span("v").text(d.comments.len().to_string())),
                ),
        )
        .child(body(d));
    if !d.cells.is_empty() {
        page = page.child(
            el("table")
                ...
                .child(
                    el("thead").child(
                        el("tr")
                            .child(el("th").text("Cell"))
                            .child(el("th").text("Value")),
                    ),
                )
                .child(el("tbody").each(&d.cells, |(cell, value)| {
                    el("tr")
                        .id(format!("cell-{cell}"))
                        .child(el("td").class("k").text(cell.as_str()))
                        .child(el("td").text(value.as_str()))
                })),
        );
    }
    page = page.each(d.slides.iter().enumerate(), |(i, slide)| {
        div("callout")
            ...
            .child(
                div("c")
                    .child(el("strong").text(slide.title.as_str()))
    ...
    if d.writable(actor) {
        page = page.child(
            el("section")
                .class("editor")
                .child(div("section-title").text("Edit this page"))
                .child(
                    form("edit", format!("/documents/{id}"), "post")
                        ...
                        .child(
                            div("actions")
                                .child(button("edit-submit", "Save changes").class("primary")),
                        ),
                ),
        );
    } else {
        ...
        el("section")
            ...
            .each(d.comments.iter().enumerate(), |(i, c)| {
                div("comment")
                    .id(format!("comment-{i}"))
                    .child(
                        span("tile small").attr("aria-hidden", "true").text(
                            c.author
                                .chars()
                                .next()
                    ...
                    .child(
                        div("c")
                            .child(span("by").text(capital(&c.author)))
                            .child(el("p").text(c.text.as_str())),
                    )
            })
            .child(
...
fn home(s: &DocsState, actor: &str, kind: Option<DocType>) -> Html {
    ...
    el("article")
        ...
        .child(
            el("h1")
                .class("greeting")
                .text(format!("Good morning, {}", capital(actor))),
        )
        .child(div("section-title").text("Recently visited"))
        .child(if docs.is_empty() {
            el("p").class("block muted").text("No pages yet.")
        } else {
            cards("recent", &docs)
        })
        .child(
            el("section")
                .class("editor")
                .child(div("section-title").text("New page"))
                .child(
                    form("create", "/documents", "post")
                        ...
                        .child(
                            div("actions")
                                .child(button("create-submit", "Create page").class("primary")),
                        ),
                ),
        )
}
pub(crate) fn view(s: &DocsState, actor: &str, screen: Screen) -> Result<HttpResponse> {
    let (title, side, top, main) = match screen {
        Screen::Home(kind) => (
            "Notion".to_owned(),
            sidebar(s, actor, kind, None, false),
            topbar("Documents", None, actor),
            home(s, actor, kind),
        ),
        Screen::Starred => {
            ...
            let main = el("article")
                ...
                .child(if docs.is_empty() {
                    el("p")
                        .class("block muted")
                        .text("Nothing in your Favorites yet.")
                } else {
                    cards("starred", &docs)
                });
            (
                "Favorites · Notion".to_owned(),
                sidebar(s, actor, None, None, true),
                topbar("Favorites", None, actor),
                main,
            )
        }
    ...
    let page = Page::new(title)
        ...
        .body([div("app")
            .child(side)
            .child(el("main").child(top).child(main))]);
    web::html::page(&page)
}
