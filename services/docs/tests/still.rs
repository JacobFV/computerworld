...
fn docs_stills() {
    ...
    for (name, pages) in [
        ...
        ("notion", vec![("/documents/onboarding-checklist", ""), ("/", "-home")]),
    ] {
        let ctx = ServiceContext { actor: "alice".into(), source: "pc".into(), tick: 1, seed: 1, instance: name.into() };
        let mut state = DocsService.initialize(site(name)["initial_state"].clone(), &ctx).unwrap();
        for (path, suffix) in pages {
            let response = DocsService.handle(&mut state, &ctx, &HttpRequest::get(format!("http://docs{path}"))).unwrap();
