...
const ACTORS: [&str; 2] = ["alice", "bob"];
const SITES: [&str; 8] = ["youtube", "netflix", "twitch", "vimeo", "tiktok", "spotify", "soundcloud", "youtube-music"];
/// Where a sweep starts, per mode. Everything else is reached by following links.
fn seeds(mode: &str) -> &'static [&'static str] {
    match mode {
        "audio" => &["/", "/search", "/collection", "/queue", "/lyrics"],
        "music" => &["/", "/explore", "/library", "/search"],
        _ => &["/", "/results", "/playlists"],
    }
}
/// Links that may land on the page they sit on, because the real product's do too.
const ALLOW_SELF: &[&str] = &["chrome-home", "topbar-logo", "topbar-home"];
...
fn no_page_of_any_seeded_site_draws_a_control_that_cannot_act() {
    for name in SITES {
        let (host, mode, seed) = load(name);
        for actor in ACTORS {
            let mut state = seed.clone();
            let mut call = caller(&mut state, actor, &host);
            let faults = Sweep::new(seeds(&mode), &mut call).allow_self(ALLOW_SELF).run();
...
/// The pages that carry a link off the site: a description or an about text with a URL in it.
fn off_site_pages(mode: &str, seed: &Value) -> Vec<String> {
    let mut out: Vec<String> = seeds(mode).iter().map(|s| (*s).to_owned()).collect();
    for id in seed["items"].as_object().into_iter().flatten().map(|(k, _)| k) {
        if mode == "video" {
            out.push(format!("/watch?v={id}"));
        }
        // `/track/<id>` is a page on every mode: the item with its own controls.
        if mode != "music" {
            out.push(format!("/track/{id}"));
        }
    }
    for id in seed["channels"].as_object().into_iter().flatten().map(|(k, _)| k) {
        out.push(match mode {
            "music" => format!("/channel/{id}"),
            _ => format!("/artist/{id}"),
        });
    }
    out
...
fn a_link_that_leaves_the_site_names_a_host_this_world_answers_on() {
    ...
    for name in SITES {
        ...
        for path in off_site_pages(&mode, &seed) {
