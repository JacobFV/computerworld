//! Renders each media site's pages through the engine to `research/site-stills/`.
...
fn ctx() -> ServiceContext {
    ServiceContext { actor: "alice".into(), source: "alice-mac".into(), tick: 1, seed: 1, instance: "media".into() }
}
fn render(state: &mut Value, host: &str, html: &str, file: &str, viewport: Viewport) {
    ...
    for node in doc.descendants(Document::ROOT) {
        ...
        if doc.is(node, "img") {
            ...
            let reply = MediaService.handle(state, &ctx(), &HttpRequest::get(format!("http://{host}{src}"))).unwrap();
            let asset: Value = serde_json::from_slice(&reply.body).unwrap();
            let rgba = asset["rgba"].as_array().unwrap().iter().map(|v| v.as_u64().unwrap() as u8).collect();
            images.insert(
                &src,
                RgbaImage { width: asset["width"].as_u64().unwrap() as u32, height: asset["height"].as_u64().unwrap() as u32, rgba },
            );
        }
    }
    ...
    let tree = cw_web::layout::layout_with(&doc, &styles, viewport, LayoutOptions { images: &images, scroll: &scroll }, &mut cache);
...
fn site(name: &str) -> (Value, String, Value) {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(format!("../../worlds/company-2026/sites/{name}.json"));
    let file: Value = serde_json::from_str(&std::fs::read_to_string(path).unwrap()).unwrap();
    let host = file["domains"][0].as_str().unwrap().to_owned();
    let state = MediaService.initialize(file["initial_state"].clone(), &ctx()).unwrap();
...
#[ignore]
fn repro() {
    let Ok(path) = std::env::var("REPRO") else { return };
    let html = std::fs::read_to_string(&path).unwrap();
    let (_, host, mut state) = site("youtube");
    let viewport = Viewport { width: 800, height: 400, scale: 1, zoom: 100 };
...
fn site_stills() {
    ...
    for name in ["youtube", "netflix", "twitch", "vimeo", "tiktok", "spotify", "soundcloud", "youtube-music"] {
        if !only.is_empty() && !only.split(',').any(|s| s == name) {
            continue;
        }
        let (_, host, mut state) = site(name);
        let mode = state["mode"].as_str().unwrap().to_owned();
        let first_item = state["items"].as_object().unwrap().keys().next().unwrap().clone();
        let first_channel = state["channels"].as_object().unwrap().keys().next().unwrap().clone();
        let mut pages = vec![("/".to_owned(), format!("{name}.png"))];
        if mode != "video" {
            // Something playing, so the pinned player bar is in the picture.
            let first_album = state["items"][&first_item]["album"].as_str().unwrap_or_default().to_owned();
            let _ = first_album;
            let request = HttpRequest::json("POST", format!("http://{host}/api/player"), &json!({"action": "play", "item": first_item})).unwrap();
            MediaService.handle(&mut state, &ctx(), &request).unwrap();
        }
        if !main_only {
            match mode.as_str() {
                "video" => {
                    pages.push((format!("/watch?v={first_item}"), format!("{name}-watch.png")));
                    pages.push((format!("/channel/{first_channel}"), format!("{name}-channel.png")));
                    pages.push(("/results?search_query=a".to_owned(), format!("{name}-results.png")));
                }
                "audio" => {
                    let playlist = state["playlists"].as_object().unwrap().keys().next_back().unwrap().clone();
                    pages.push((format!("/playlist/{playlist}"), format!("{name}-playlist.png")));
                    pages.push((format!("/artist/{first_channel}"), format!("{name}-artist.png")));
                }
                _ => {
                    pages.push((format!("/watch?v={first_item}"), format!("{name}-watch.png")));
                    pages.push((format!("/channel/{first_channel}"), format!("{name}-artist.png")));
                }
            }
        }
        for (path, file) in pages {
            ...
            request.headers.insert(cw_protocol::REFRESH_HEADER.into(), "1".into());
            let response = MediaService.handle(&mut state, &ctx(), &request).unwrap();
            assert_eq!(response.status, 200, "{host}{path}");
            render(&mut state, &host, &String::from_utf8(response.body).unwrap(), &file, viewport);
        }
    }
}
