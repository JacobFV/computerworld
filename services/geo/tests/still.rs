...
fn ctx(actor: &str) -> ServiceContext {
    ServiceContext { actor: actor.into(), source: "alice-mac".into(), tick: 12, seed: 1, instance: "geo".into() }
}
fn fetch(state: &mut Value, actor: &str, url: &str) -> Vec<u8> {
    let r = GeoService.handle(state, &ctx(actor), &HttpRequest::get(url)).unwrap();
...
fn render(state: &mut Value, actor: &str, origin: &str, path: &str, file: &str) {
    ...
    draw(&html, &target, |src| fetch(state, actor, &format!("{origin}{src}")));
}
...
fn draw(html: &str, target: &std::path::Path, mut image: impl FnMut(&str) -> Vec<u8>) {
    ...
    for node in doc.descendants(Document::ROOT) {
        ...
        if let Some(src) = doc.attr(node, "src").filter(|_| doc.is(node, "img")) {
            ...
            let rgba = asset["rgba"].as_array().unwrap().iter().map(|b| b.as_u64().unwrap() as u8).collect();
            images.insert(src, RgbaImage::new(asset["width"].as_u64().unwrap() as u32, asset["height"].as_u64().unwrap() as u32, rgba));
        }
    }
    ...
    let mut out = Vec::new();
    {
        ...
        encoder.write_header().unwrap().write_image_data(&frame.rgba).unwrap();
    }
...
fn geo_site_stills() {
    let sites = [
        ("google-maps", include_str!("../../../worlds/company-2026/sites/google-maps.json"), "http://maps.google.com", "carol"),
        ("osm", include_str!("../../../worlds/company-2026/sites/osm.json"), "http://openstreetmap.org", "bob"),
        ("weather", include_str!("../../../worlds/company-2026/sites/weather.json"), "http://weather.com", "alice"),
    ];
    let only = std::env::var("STILL").ok();
    for (site, raw, origin, actor) in sites {
        let seed: Value = serde_json::from_str(raw).unwrap();
        let mut state = GeoService.initialize(seed["initial_state"].clone(), &ctx(actor)).unwrap();
        let mut pages = vec![("/".to_owned(), format!("{site}.png"))];
        if seed["initial_state"]["mode"] == "maps" {
            pages.push(("/maps/place/harborline-coffee".into(), format!("{site}-place.png")));
            pages.push(("/maps/dir?from=northstar-hq&to=devcon-center&mode=driving".into(), format!("{site}-directions.png")));
        } else {
            pages.push(("/weather/tenday/l/seattle".into(), format!("{site}-tenday.png")));
        }
...
fn render_a_file() {
    ...
    draw(&std::fs::read_to_string(source).unwrap(), std::path::Path::new(&out), |_| unreachable!("no images"));
}
