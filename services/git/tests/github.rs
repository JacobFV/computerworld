...
fn ctx(actor: &str) -> ServiceContext {
    ServiceContext { actor: actor.into(), source: format!("{actor}-pc"), tick: 40, seed: 5, instance: "github".into() }
}
fn get(state: &mut Value, actor: &str, path: &str) -> (u16, String) {
    let response = GitService.handle(state, &ctx(actor), &HttpRequest::get(format!("http://github.com{path}"))).unwrap();
...
fn post(state: &mut Value, actor: &str, path: &str, body: Value) -> (u16, String) {
    let response = GitService
        .handle(state, &ctx(actor), &HttpRequest::json("POST", format!("http://github.com{path}"), &body).unwrap())
        .unwrap();
    (response.status, String::from_utf8(response.body).unwrap())
}
...
fn submit(state: &mut Value, actor: &str, path: &str, fields: &[(&str, &str)]) -> (u16, String) {
    let body = url::form_urlencoded::Serializer::new(String::new()).extend_pairs(fields.iter().copied()).finish();
...
fn plain_state_and_api_are_frozen_and_its_pages_are_html() {
    ...
    assert_eq!(page.text("ref-refs/heads/main"), "refs/heads/main 2cea0d22873f219e1468c9496a3504dc9604088456bed23168d92c7e0b1492db");
    assert!(page.raw_text("file-README.md").contains("# Atlas onboarding\nUse http://intranet.internal/"));
    assert!(page.raw_text("file-checklist.txt").starts_with("checklist.txt"));
    assert!(page.raw_text("file-checklist.txt").contains("mail read: pending\ndocument updated: pending\n"));
    ...
    assert!(value["objects"].as_object().unwrap().values().next().unwrap()["files"]["checklist.txt"].as_str().unwrap().contains("mail read: pending"));
    assert_eq!(get(&mut state, "bob", "/api/git/repos").1, r#"["onboarding"]"#);
    // The legacy route is the same plain page on a skinned instance: no theme, no chrome.
    let mut skinned = github();
    let legacy = Page::parse("/repos/atlas", get(&mut skinned, "alicechen", "/repos/atlas").1);
...
fn issue_opens_comments_and_closes() {
    ...
    assert_eq!(post(&mut state, "praman", "/northstar/atlas/issues/15/comments", json!({"body":"Worth a paragraph in the README."})).0, 200);
    assert_eq!(post(&mut state, "praman", "/northstar/atlas/issues/15/state", json!({"state":"closed"})).0, 403);
    let (status, body) = post(&mut state, "alicechen", "/northstar/atlas/issues/15/state", json!({"state":"closed"}));
    assert_eq!(status, 200);
    let page = Page::parse("closed issue", body);
    assert_eq!(page.text("state-text"), "Closed");
    assert_eq!(page.text("comment-0-text"), "Worth a paragraph in the README.");
    assert!(page.has("reopen") && !page.has("close"));
    assert_eq!(state["repositories"]["atlas"]["issues"]["15"]["state"], "closed");
    // An empty title is refused and changes nothing.
    let before = state.clone();
    assert_eq!(post(&mut state, "praman", "/northstar/atlas/issues", json!({"title":"  "})).0, 422);
...
fn the_pages_forms_drive_the_same_routes() {
    let mut state = github();
    let new = Page::parse("/issues/new", get(&mut state, "alicechen", "/northstar/atlas/issues/new").1);
    let (action, method, fields) = new.form("new-issue");
    assert_eq!((action.as_str(), method.as_str()), ("/northstar/atlas/issues", "post"));
    ...
    let (status, body) = submit(&mut state, "alicechen", &action, &[("title", "From the form"), ("body", "See http://example.com/x.")]);
    assert_eq!(status, 200);
    ...
    assert_eq!(issue.attr("thread-body-text-link-1", "href"), "http://example.com/x");
    let (action, method, fields) = issue.form("comment");
    assert_eq!((action.as_str(), method.as_str()), ("/northstar/atlas/issues/15/comments", "post"));
    ...
    assert_eq!(issue.attr("close", "formaction"), "/northstar/atlas/issues/15/state");
    assert_eq!((issue.attr("close", "name"), issue.attr("close", "value")), ("state".to_owned(), "closed".to_owned()));
    assert_eq!(submit(&mut state, "alicechen", &action, &[("body", "A comment from the form.")]).0, 200);
    let (_, body) = submit(&mut state, "alicechen", "/northstar/atlas/issues/15/state", &[("body", ""), ("state", "closed")]);
    ...
    assert_eq!((closed.attr("reopen", "name"), closed.attr("reopen", "value")), ("state".to_owned(), "open".to_owned()));
    // Star is a one-button form; search is a GET form on `q`.
    let (action, method, _) = closed.form("star-form");
    assert_eq!((action.as_str(), method.as_str()), ("/northstar/atlas/star", "post"));
    assert_eq!(closed.form_of("star"), "star-form");
    let (action, method, fields) = closed.form("search");
    assert_eq!((action.as_str(), method.as_str(), fields[0].0.as_str()), ("/search", "get", "q"));
}
...
fn pull_request_opens_reviews_and_merges_the_ref() {
    ...
    let main = state["repositories"]["atlas"]["refs"]["refs/heads/main"].as_str().unwrap().to_owned();
    state["repositories"]["atlas"]["refs"][head] = json!(main);
    let compare = Page::parse("/compare", get(&mut state, "bmartinez", "/northstar/atlas/compare").1);
    let (action, method, fields) = compare.form("new-pull");
    assert_eq!((action.as_str(), method.as_str()), ("/northstar/atlas/pulls", "post"));
    assert_eq!(
        fields,
        vec![("title".to_owned(), String::new()), ("head".to_owned(), "refs/heads/".to_owned()), ("base".to_owned(), "refs/heads/main".to_owned())]
    );
    ...
    let (status, body) =
        post(&mut state, "bmartinez", "/northstar/atlas/pulls", json!({"title":"Sort before you iterate","head":head,"base":"refs/heads/main"}));
    assert_eq!(status, 200);
    ...
    assert_eq!((action.as_str(), method.as_str()), ("/northstar/atlas/pull/15/merge", "post"));
    assert_eq!(pull.form_of("merge"), "merge-form");
    // The files view's review button approves with a fixed body.
    let files = Page::parse("files", get(&mut state, "alicechen", "/northstar/atlas/pull/15/files").1);
    let (action, _, fields) = files.form("approve-form");
    assert_eq!(action, "/northstar/atlas/pull/15/reviews");
    assert_eq!(fields, vec![("decision".to_owned(), "approve".to_owned()), ("body".to_owned(), "Looks good.".to_owned())]);
    assert_eq!(post(&mut state, "alicechen", "/northstar/atlas/pull/15/reviews", json!({"decision":"approve","body":"Ship it."})).0, 200);
    let (status, body) = submit(&mut state, "alicechen", "/northstar/atlas/pull/15/merge", &[]);
    assert_eq!(status, 200);
    ...
    assert_eq!(state["repositories"]["atlas"]["refs"]["refs/heads/main"], state["repositories"]["atlas"]["refs"][head]);
    assert_eq!(post(&mut state, "alicechen", "/northstar/atlas/pull/15/merge", json!({})).0, 409);
}
...
fn starring_toggles_and_survives_a_round_trip() {
    ...
    assert_eq!(Page::parse("stargazers", body).text("stars-count"), "2 people starred northstar/atlas");
}
...
fn owner_repo_blob_and_gist_pages_resolve() {
    let mut state = github();
    let home = Page::parse("/", get(&mut state, "alice", "/").1);
    assert_eq!(home.attr("repo-northstar-atlas", "href"), "/northstar/atlas");
    assert_eq!(home.text("repo-name-atlas"), "northstar/atlas");
    assert_eq!(home.attr("gists", "href"), "/gists");
    assert_eq!(Page::parse("/northstar", get(&mut state, "alice", "/northstar").1).attr("owned-name-atlas", "href"), "/northstar/atlas");
    let repo = Page::parse("/northstar/atlas", get(&mut state, "alice", "/northstar/atlas").1);
    ...
    assert!(Page::parse("blob", get(&mut state, "alice", "/northstar/atlas/blob/README.md").1)
        .raw_text("blob-text")
...
fn seeded(site: &str, skin: &str) -> Value {
    ...
    GitService.initialize(site["initial_state"].clone(), &ctx("alicechen")).unwrap()
}
...
fn repository_home_has_tabs_branch_selector_files_and_about() {
    ...
    assert_eq!((page.text("crumb-0"), page.attr("crumb-1", "href")), ("northstar".to_owned(), "/northstar/atlas".to_owned()));
    assert_eq!(page.text("tab-issues-link"), "Issues 6");
    assert_eq!(page.attr("tab-issues-link", "href"), "/northstar/atlas/issues");
    ...
    assert_eq!(page.attr("history", "href"), "/northstar/atlas/commits/main");
    assert_eq!(page.text("latest-message"), "Expand the README with a quick start and pin serde");
    ...
    let icons: Vec<String> = page.ids_with_prefix("entry-icon-").iter().map(|id| page.attr(id, "data-icon")).collect();
    assert_eq!(icons, vec!["folder", "folder", "folder", "file", "file", "file", "file"]);
    assert_eq!(page.text("file-0"), ".github");
    assert_eq!(page.attr("file-0", "href"), "/northstar/atlas/tree/main/.github");
    assert_eq!(page.attr("file-3", "href"), "/northstar/atlas/blob/main/CONTRIBUTING.md");
    assert_eq!(page.text("entry-message-0"), "Run the test matrix on Windows too");
    assert_eq!(page.text("entry-message-3"), "Merge pull request #11 from northstar/contributing");
    assert_eq!(page.text("entry-message-6"), "Expand the README with a quick start and pin serde");
    // README rendered as headings, a mono code block and lists with live links.
    let headings = page.ids_with_prefix("readme-h-");
    assert!(headings.iter().any(|id| page.text(id) == "Quick start" && page.tag(id) == "h2"));
    let code = page.ids_with_prefix("readme-code-");
    assert!(page.tag(&code[0]) == "pre" && page.raw_text(&code[0]).contains("cargo add atlas"));
    assert!(page.ids_with_prefix("readme-li-").iter().any(|id| id.contains("-link-") && page.attr(id, "href") == "http://status.northstar.example/"));
    ...
    assert_eq!(page.attr("stargazers", "href"), "/northstar/atlas/stargazers");
...
fn commits_commit_tree_blob_and_branches_pages_render_history() {
    ...
    let titles: Vec<String> =
        commits.ids_with_prefix("commits-").into_iter().filter(|id| id.ends_with("-title") && id != "commits-title").map(|id| commits.text(&id)).collect();
    assert_eq!(titles.len(), 7);
    assert_eq!(titles[0], "Expand the README with a quick start and pin serde");
    assert_eq!(titles[6], "Initial commit");
    assert!(commits.text("commits-day-label-1").starts_with("Commits on Aug") || commits.text("commits-day-label-1").starts_with("Commits on Sep"));
    ...
    assert!(commit.text("diff-summary").starts_with("Showing 2 changed files with"));
    ...
    let names: Vec<String> = tree.ids_with_prefix("file-").iter().map(|id| tree.text(id)).collect();
    ...
    assert!(rows.iter().any(|id| branches.text(id).contains("sort-refs") && branches.text(id).contains("0 behind · 2 ahead")));
}
...
fn issues_and_pull_requests_look_like_githubs() {
    ...
    assert!(icons.iter().all(|id| issues.attr(id, "data-icon") == "issue-open" && issues.has_class(id, "st-open")));
    assert!(issues.attr("thread-14-label-0", "style").contains("#d73a4a"));
    assert_eq!(issues.text("thread-14-label-0"), "bug");
    assert!(issues.attr("thread-13-label-1", "style").contains("#7057ff"));
    assert!(issues.text("meta-14").starts_with("#14 opened") && issues.text("meta-14").ends_with("by bmartinez"));
    assert_eq!(issues.text("comments-14"), "3");
    assert_eq!(issues.text("list-open"), "6 Open");
    assert_eq!(issues.attr("thread-14", "href"), "/northstar/atlas/issues/14");
    ...
    assert_eq!(["comment-0", "comment-1", "comment-2", "comment-3"].iter().filter(|id| issue.has(id)).count(), 3);
    assert_eq!(issue.attr("comment-1-text-link-8", "href"), "http://stackoverflow.com/questions/t-4411");
    assert_eq!(issue.text("side-assignee-name"), "praman");
    assert!(issue.text("side-dev-link-1").starts_with("#15 "));
    assert_eq!(issue.attr("side-dev-link-1", "href"), "/northstar/atlas/pull/15");
    assert_eq!(issue.tag("comment-body"), "textarea");
    ...
    assert_eq!(pull.text("thread-verb"), "praman wants to merge 2 commits into");
    assert_eq!((pull.text("thread-base"), pull.text("thread-head-ref")), ("main".to_owned(), "sort-refs".to_owned()));
    ...
    assert_eq!(pull.attr("ptab-files-link", "href"), "/northstar/atlas/pull/15/files");
    let files = page(&mut state, "/northstar/atlas/pull/15/files");
    assert!(files.text("diff-summary").starts_with("Showing 2 changed files with"));
    ...
    assert!(commits.all_text().contains("Pin the neighbour order in the path test"));
    let merged = page(&mut state, "/northstar/atlas/pull/11");
    assert_eq!(merged.text("state-text"), "Merged");
    assert!(merged.text("merged-status-title").starts_with("Pull request successfully merged"));
    ...
    assert!(owner.text("contrib-title").ends_with("contributions in the last year"));
    assert_eq!(owner.text("pinned-title"), "Popular repositories");
    assert!(owner.ids_with_prefix("cell-").iter().any(|id| owner.has_class(id, "l1")));
    assert_eq!(page(&mut state, "/northstar/atlas/stargazers").text("stars-count"), "7 people starred northstar/atlas");
}
...
fn the_gitlab_look_keeps_the_ids_and_changes_the_words() {
    ...
    assert_eq!(home.attr("repo-opensim-replay-tools", "href"), "/opensim/replay-tools");
    ...
    assert_eq!(project.attr("tab-issues-link", "href"), "/opensim/replay-tools/issues");
    ...
    assert!(page(&mut plain, "/opensim/replay-tools").html.contains("body class=\"skin-github\""));
    assert_eq!(Page::parse("/", get(&mut plain, "alice", "/").1).attr("repo-replay-tools", "href"), "/repos/replay-tools");
}
