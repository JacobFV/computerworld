...
fn press_site_stills() {
    ...
    if let Ok(path) = std::env::var("PRESS_STILL_HTML") {
        ...
        render(&html, "scratch.png", Viewport { width: 400, height: 200, scale: 1, zoom: 100 });
        return;
    }
    let only = std::env::var("PRESS_STILL").ok();
    let ctx = ServiceContext { actor: "alice".into(), source: "alice-mac".into(), tick: 1, seed: 1, instance: "press".into() };
    // `PRESS_STILL_HEIGHT` renders a taller frame, to look at the foot of a long page.
    let height = std::env::var("PRESS_STILL_HEIGHT").ok().and_then(|h| h.parse().ok()).unwrap_or(1100);
    let viewport = Viewport { width: 1280, height, scale: 1, zoom: 100 };
    for (site, source) in SITES {
        if only.as_deref().is_some_and(|o| !o.split(',').any(|s| s == *site)) {
            continue;
        }
        let file: Value = serde_json::from_str(source).unwrap();
        let origin = format!("http://{}", file["domains"][0].as_str().unwrap());
        let mut state = PressService.initialize(file["initial_state"].clone(), &ctx).unwrap();
        let front = PressService.handle(&mut state, &ctx, &HttpRequest::get(format!("{origin}/"))).unwrap();
        ...
        let href = front[at..].split("href=\"").nth(1).unwrap().split('"').next().unwrap();
        let story = PressService.handle(&mut state, &ctx, &HttpRequest::get(format!("{origin}{href}"))).unwrap();
        assert_eq!(story.status, 200);
        render(&String::from_utf8(story.body).unwrap(), &format!("{site}-article.png"), viewport);
    }
}
