...
fn webmail_stills() {
    ...
    for (name, actor, pages) in [
        ("google-mail", "alice", vec![("/", ""), ("/?folder=inbox&thread=mail-1", "-thread"), ("/?folder=inbox&compose=1", "-compose")]),
        ("outlook-mail", "bob", vec![("/?folder=inbox&thread=mail-1", ""), ("/?folder=inbox&compose=1", "-compose")]),
        ("mail-com", "carol", vec![("/?folder=inbox&thread=mail-2", ""), ("/", "-list")]),
    ] {
        let ctx = ServiceContext { actor: actor.into(), source: "pc".into(), tick: 1, seed: 1, instance: name.into() };
        let mut state = MailService.initialize(site(name)["initial_state"].clone(), &ctx).unwrap();
        for (path, suffix) in pages {
            let response = MailService.handle(&mut state, &ctx, &HttpRequest::get(format!("http://mail{path}"))).unwrap();
...
#[ignore]
fn scratch() {
    let (Ok(html), Ok(png)) = (std::env::var("SCRATCH_HTML"), std::env::var("SCRATCH_PNG")) else { return };
    let width = std::env::var("SCRATCH_W").ok().and_then(|w| w.parse().ok()).unwrap_or(640);
    let viewport = Viewport { width, height: width * 5 / 8, scale: 1, zoom: 100 };
