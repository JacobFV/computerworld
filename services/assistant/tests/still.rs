...
const SITES: [(&str, &str, &str); 2] = [
    ("openai", "http://chatgpt.com", include_str!("../../../worlds/company-2026/sites/openai.json")),
    ("anthropic", "http://claude.ai", include_str!("../../../worlds/company-2026/sites/anthropic.json")),
];
...
fn assistant_home_and_conversation_stills() {
    ...
    for (site, origin, source) in SITES {
        ...
        let mut ctx = ServiceContext { actor: "alice".into(), source: "alice-mac".into(), tick: 1, seed: 1, instance: site.into() };
        let mut state = AssistantService.initialize(seed["initial_state"].clone(), &ctx).unwrap();
        ...
        let chat = typed.mine(&owner).max_by_key(|c| c.messages.len()).map(|c| c.id.clone());
        let mut pages = vec![(format!("{origin}/"), format!("{site}.png"))];
        if let Some(id) = chat {
            pages.push((format!("{origin}/c/{id}"), format!("{site}-chat.png")));
        }
        for (url, file) in pages {
            let response = AssistantService.handle(&mut state, &ctx, &HttpRequest::get(url)).unwrap();
