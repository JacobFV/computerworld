...
Milestone 0 of [`docs/web-engine-plan.md`](../docs/web-engine-plan.md) asks one question in
...
[`research/google-ceiling/`](google-ceiling/); nothing in a crate or service was changed.

## What was attempted

1. **A reference.** [`mock.html`](google-ceiling/mock.html) is a self-contained HTML+CSS
   ...
   `scripts/dom-to-site/capture.mjs`) to [`chromium.png`](google-ceiling/chromium.png), with
   the box tree and ARIA snapshot in `capture.json`.

2. **The same page in the format.** [`page.json`](google-ceiling/page.json) is a
   ...
   [`computerworld.png`](google-ceiling/computerworld.png).

3. **A second variant** [`page-form.json`](google-ceiling/page-form.json) replaces the
   decorative pill with a real `form` + `input`, so the search box is a genuine textbox
   the actor can type into. Rendered to
   [`computerworld-form.png`](google-ceiling/computerworld-form.png). The two variants are
...
4. **Scoring** with `scripts/dom-to-site/compare.mjs` (structure recall of accessible
   ...
   [`side-by-side.png`](google-ceiling/side-by-side.png) (Chromium left, computerworld
   right).

## The render pair

![Chromium (left) and the in-world browser (right)](google-ceiling/side-by-side.png)

The real-textbox variant, for the record:

![The form variant](google-ceiling/computerworld-form.png)
...
The format got much closer than `research/dom-to-site.md` predicted, and the evidence is
...
- `research/google-ceiling/mock.html`, `shot.mjs`, `chromium.png`, `capture.json`, `capture.png`
- `research/google-ceiling/page.json`, `page-form.json`, `render-page.mjs`,
...
- `research/google-ceiling/side-by-side.mjs`, `side-by-side.png`, `compare.json`, `compare-form.json`
