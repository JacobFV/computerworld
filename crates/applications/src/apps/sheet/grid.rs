...
pub fn paint(p: &mut Painter, book: &Book, area: Rect, geom: Geom, pal: &Palette) -> Rect {
    ...
    for &(row, y) in &rows {
        ...
        for (i, (c, d)) in stored.iter().enumerate() {
            ...
            // A conditional format's bold or italic adds to the cell's own. Bold and italic
            // cells are set in the real bold and italic faces, and measured in them, so
            // overflow and #### follow what is drawn.
            let style = effect.map(|e| e.style).unwrap_or_default();
            let bold = cw_scene::Style::new(
                d.style.bold || style.bold,
                d.style.italic || style.italic,
                cw_scene::Lang::Auto,
            );
