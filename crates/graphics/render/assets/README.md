...
frame hashes are unchanged by it, and `crates/graphics/scene/src/metrics_data.rs` needed no
...
which both measurement and the renderer use. `crates/graphics/scene/src/metrics_italic.rs` is
...
`build-fonts.py` also writes `crates/graphics/scene/src/metrics_data.rs`, the advance table that
...
reproducible on fontTools 4.55.3. `crates/graphics/render/src/faces.rs` pins each file's
...
and `Typeface::web_face_index` and `crates/graphics/render/src/faces.rs` agree on which file
...
`crates/graphics/scene/src/kerning_data.rs`. `extract_kerning` walks the feature's PairPos
...
failing — `crates/graphics/render/tests/icon_assets.rs` renders every advertised id and asserts
...
ambiguous. `crates/graphics/render/src/glyph_fit.rs` grid-fits the fixed-pitch `Text` face to
...
`crates/graphics/render/tests/terminal_legibility.rs` asserts each of these at 10–20 px.
