...
/// `total` is the block count line, which coreutils prints for a directory's contents and
/// never for file operands named on the command line.
fn ls_render(c: &Computer, entries: &[(String, String)], f: &LsFlags, total: bool) -> String {
...
pub(crate) fn ls(c: &Computer, args: &[String]) -> Result<String, Fail> {
    ...
    for operand in &paths {
        ...
        if dir && !flag(&opts, 'd') {
            ...
            out.push_str(&ls_render(c, &[(operand.clone(), resolved)], &f, false));
        }
    }
