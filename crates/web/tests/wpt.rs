...
fn manifest() -> Manifest {
    ...
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("read {}: {e} (run tools/fetch-wpt.py)", path.display()));
...
fn manifest_expectations_and_files_agree() {
    ...
    assert_eq!(
        ...
        (WPT_WIDTH, WPT_HEIGHT),
        "manifest viewport is not the WPT default"
    );
    assert!(
        wpt_dir().join("LICENSE.md").exists(),
        "tests/wpt/LICENSE.md (the WPT licence) is missing"
    );
    assert!(
        wpt_dir().join("README.md").exists(),
        "tests/wpt/README.md is missing"
    );
    ...
    for d in &m.directories {
        ...
        assert!(
            expectations.contains_key(&d.dir),
            "{}: no entry in expectations.json",
            d.dir
        );
        assert!(
            expectations[&d.dir] <= d.pairs.len(),
            "{}: expectation above the pair count",
            d.dir
        );
        for p in &d.pairs {
            assert!(
                p.test.starts_with(&format!("{}/", d.dir)),
                "{}: {} is not in its directory",
                d.dir,
                p.test
            );
            assert!(
                matches!(p.kind.as_str(), "match" | "mismatch"),
                "{}: kind {}",
                p.test,
                p.kind
            );
    ...
    for dir in expectations.keys() {
        assert!(
            m.directories.iter().any(|d| &d.dir == dir),
            "expectations.json names `{dir}` but the manifest has no such directory"
        );
    }
    assert!(
        total >= 1000,
        "expected at least a thousand pairs, found {total}"
    );
}
...
fn render_caught(rel: &str) -> Result<cw_scene::Scene, String> {
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| render(rel))).unwrap_or_else(
        |payload| {
            let msg = payload
                .downcast_ref::<String>()
                .cloned()
    ...
    let (passed, detail) = match (a, b) {
        (Ok(sa), Ok(sb)) => {
            ...
            let differing = if content_digest(&sa) == content_digest(&sb) {
                ...
                fa.rgba
                    .chunks(4)
                    .zip(fb.rgba.chunks(4))
                    .filter(|(x, y)| x != y)
                    .count()
                    + fa.rgba.len().abs_diff(fb.rgba.len()) / 4
            };
            let same = differing == 0;
            (
                if p.kind == "match" { same } else { !same },
...
fn run_all(m: &Manifest) -> Vec<DirOutcome> {
    ...
    let flat: Vec<(usize, &Pair)> = m
        .directories
        .iter()
        .enumerate()
        .flat_map(|(di, d)| d.pairs.iter().map(move |p| (di, p)))
        .collect();
    let next = std::sync::atomic::AtomicUsize::new(0);
    let results: std::sync::Mutex<Vec<Option<PairOutcome>>> =
        std::sync::Mutex::new(flat.iter().map(|_| None).collect());
...
fn report(commit: &str, dirs: &[DirOutcome], expectations: &BTreeMap<String, usize>) -> String {
    ...
    md.push_str(
        "| Directory | Pass | Pairs | Rate | Expected | Status |\n|---|---|---|---|---|---|\n",
    );
    for d in dirs {
...
#[cfg_attr(
    not(feature = "wpt"),
    ignore = "the full WPT run takes about half an hour (`--features pipeline,wpt`)"
)]
fn wpt_reftests_meet_expectations() {
    #[cfg(feature = "pipeline")]
    {
        ...
        assert!(
            failures.is_empty(),
            "directories below expectations.json:\n{}",
            failures.join("\n")
        );
    }
}
...
fn ahem_is_mapped_to_the_bundled_monospace_face() {
    assert_eq!(
        map_ahem("font: 20px/1 Ahem;"),
        "font: 20px/1 \"JetBrains Mono\";"
    );
    assert_eq!(
        map_ahem("font-family: 'Ahem', sans-serif"),
        "font-family: \"JetBrains Mono\", sans-serif"
    );
    assert_eq!(
        map_ahem("font-family: \"ahem\""),
...
fn imports_resolve_inside_the_tree() {
    ...
    let css = inline_imports(
        "@import url(\"/css/support/nonexistent.css\"); p { color: red }",
        &base,
        0,
    );
    assert_eq!(css.trim(), "p { color: red }");
    assert_eq!(
        resolve("css/css-flexbox/x.html", "../reference/ref.xht").unwrap(),
        wpt_dir().join("css/reference/ref.xht")
    );
    assert_eq!(
        resolve("css/css-flexbox/x.html", "/css/support/a.css?x#y").unwrap(),
        wpt_dir().join("css/support/a.css")
    );
