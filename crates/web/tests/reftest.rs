...
fn every_test_has_a_reference() {
    ...
    assert!(
        names.len() >= 20,
        "expected at least twenty reftest pairs, found {}",
        names.len()
    );
    for name in &names {
        ...
        assert!(
            a.contains("<body") && b.contains("<body"),
            "{name}: both files need a body"
        );
...
fn run_pairs() -> Vec<Outcome> {
    ...
    for name in pairs() {
        ...
        let reference =
            std::fs::read_to_string(ref_dir().join(format!("{name}-ref.html"))).unwrap();
        ...
        out.push(Outcome {
            ...
            digest_test: da,
            digest_ref: db,
            nodes_test: a.scene.nodes.len(),
            nodes_ref: b.scene.nodes.len(),
            differing_pixels,
        });
    }
    out
...
fn reftests_paint_identical_pixels() {
    let failures: Vec<String> = run_pairs()
        ...
        .map(|o| {
            format!(
                "{}: {} pixels differ (see target-parity/ref-{}.test.png and .ref.png)",
                o.name, o.differing_pixels, o.name
            )
        })
        .collect();
    assert!(
...
fn reftests_paint_identical_scenes() {
    let failures: Vec<String> = run_pairs()
        .into_iter()
        .filter(|o| {
            if DIFFERENT_DECOMPOSITION.contains(&o.name.as_str()) {
                o.differing_pixels > tolerance_of(&o.name)
            } else {
                o.digest_test != o.digest_ref
            }
        })
        .map(|o| {
            format!(
