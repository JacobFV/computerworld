...
use crate::layout::fragment::{
    ControlKind, Fragment, FragmentKind, FragmentTree, Replaced, ScrollInfo, StyleSource,
};
...
fn boxf(n: u32, r: Rect) -> Fragment {
    Fragment::new(
        FragmentKind::Box {
            source: StyleSource::Element(NodeId(n)),
            padding: Edges::ZERO,
            border: Edges::ZERO,
            replaced: None,
            scroll: None,
...
fn text_run(elem: u32, text_node: Option<u32>, text: &str, r: Rect, baseline: i32) -> Fragment {
    Fragment::new(
        FragmentKind::Text {
            source: StyleSource::Element(NodeId(elem)),
            text: text.into(),
            node: text_node.map(NodeId),
            range: (0, text.len()),
            baseline: au(baseline),
            ellipsis: false,
        },
        r,
    )
...
fn backgrounds(scene: &Scene) -> Vec<u32> {
    scene
        ...
        .iter()
        .map(|n| decode(n.id))
        .filter(|(_, _, p)| *p == parts::BACKGROUND)
        .map(|(n, _, _)| n)
        .collect()
}
...
fn paint_no_doc(styles: &StyleSet, t: &FragmentTree) -> Scene {
    paint_fragments(
        styles,
        t,
        viewport(upx(t.viewport_width), upx(t.viewport_height)),
...
fn appendix_e_order_within_one_stacking_context() {
    ...
    assert_eq!(
        backgrounds(&scene),
        vec![1, 5, 2, 3, 7, 4, 6],
        "root, negative, block, float, inline, positioned, positive"
    );
    // Text paints after the inline box background and before the positioned box.
    let order = painted(&scene);
    let text_at = order
        .iter()
        .position(|(n, p)| *n == 7 && *p == parts::TEXT)
        .unwrap();
    let inline_bg = order
        .iter()
        .position(|(n, p)| *n == 7 && *p == parts::BACKGROUND)
        .unwrap();
    let pos_bg = order
        .iter()
        .position(|(n, p)| *n == 4 && *p == parts::BACKGROUND)
        .unwrap();
...
fn floats_paint_before_inline_content_even_when_later_in_tree() {
    ...
    let l = line(
        rect(0, 0, 100, 20),
        vec![text_run(1, None, "text", rect(0, 0, 40, 20), 16)],
    );
    ...
    let float_bg = order
        .iter()
        .position(|(n, p)| *n == 2 && *p == parts::BACKGROUND)
        .unwrap();
    let inner = order
        .iter()
        .position(|(n, p)| *n == 3 && *p == parts::BACKGROUND)
        .unwrap();
    let text_at = order
        .iter()
        .position(|(n, p)| *n == 1 && *p == parts::TEXT)
        .unwrap();
...
fn body_background_propagates_to_the_canvas() {
    ...
    set(&mut styles, body.0, |s| {
        ...
        s.border = Sides::uniform(BorderSide {
            width: au(2),
    ...
    assert!(
        !backgrounds(&scene).contains(&body.0),
        "body paints no background of its own"
    );
    assert!(
        scene
            .nodes
            .iter()
            .any(|n| decode(n.id) == (body.0, 0, parts::BORDER_TOP)),
        "borders still paint"
    );
...
fn border_nodes(scene: &Scene) -> Vec<&cw_scene::Node> {
    scene
        .nodes
        .iter()
        .filter(|n| decode(n.id).0 == 2 && decode(n.id).2 >= parts::BORDER_TOP)
        .collect()
}
...
fn border_styles_produce_the_expected_node_counts() {
    ...
    assert!(border_nodes(&dotted)
        .iter()
        .all(|n| matches!(n.primitive, Primitive::RoundedBox { .. })));
    let groove = border_scene(|s| {
        s.border = Sides::uniform(side(BorderStyle::Groove, 4, Color(128, 128, 128, 255)))
    });
    assert_eq!(border_nodes(&groove).len(), 8);
    let inset = border_scene(|s| {
        s.border = Sides::uniform(side(BorderStyle::Inset, 4, Color(128, 128, 128, 255)))
    });
...
fn rounded_borders_use_one_rounded_box_or_per_side_paths() {
    ...
    let bg = uniform
        .nodes
        .iter()
        .find(|n| decode(n.id) == (2, 0, parts::BACKGROUND))
        .unwrap();
    assert!(matches!(
        bg.primitive,
        Primitive::RoundedBox {
    ...
    let mixed = border_scene(|s| {
        ...
        s.border_radius = Corners {
            ...
            top_right: r,
            bottom_right: r,
            bottom_left: (LengthPercentage::ZERO, LengthPercentage::ZERO),
        };
        s.background_color = BLUE;
    });
...
fn outline_and_shadows_are_drawn_outside_the_box() {
    ...
    let outline: Vec<_> = scene
        .nodes
        .iter()
        .filter(|n| decode(n.id).2 == parts::OUTLINE)
        .collect();
    assert_eq!(
        outline.len(),
        1,
        "the first strip carries the outline part; the rest are dynamic parts"
    );
    assert_eq!(
        outline[0].bounds,
        cw_scene::Rect::new(5, 5, 110, 2),
        "top strip of the offset outline"
    );
    let strips = scene
        .nodes
        .iter()
        .filter(|n| decode(n.id).0 == 2 && matches!(n.primitive, Primitive::Box { fill: RED, .. }))
        .count();
    assert_eq!(strips, 4);
    let shadow = scene
        .nodes
        .iter()
        .find(|n| matches!(n.primitive, Primitive::Shadow { .. }))
        .unwrap();
    ...
    let shadow_at = scene
        .nodes
        .iter()
        .position(|n| matches!(n.primitive, Primitive::Shadow { .. }))
        .unwrap();
...
fn text_is_placed_by_its_baseline_and_font_metrics() {
    ...
    let t = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).2 == parts::TEXT)
        .unwrap();
    let font = styles.get(NodeId(2)).unwrap().font.clone();
    let w =
        cw_scene::metrics::text_width(cw_scene::Typeface::Inter, font.scene_style(), "Hello", 16);
...
fn decorations_sit_at_the_baseline_and_x_height() {
    ...
    assert_eq!(
        find(parts::UNDERLINE).bounds,
        cw_scene::Rect::new(0, 23, w, 1)
    );
    assert_eq!(
        find(parts::LINE_THROUGH).bounds,
        cw_scene::Rect::new(0, 22 - 7, w, 1)
    );
    assert_eq!(
        find(parts::OVERLINE).bounds,
        cw_scene::Rect::new(0, 2, w, 1)
    );
    assert!(matches!(
        find(parts::UNDERLINE).primitive,
        Primitive::Box { fill: RED, .. }
    ));
...
fn selection_highlights_the_selected_bytes() {
    ...
    let sel = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).2 == parts::SELECTION)
    ...
    let order: Vec<u32> = scene.nodes.iter().map(|n| decode(n.id).2).collect();
    assert!(
        order.iter().position(|p| *p == parts::SELECTION)
            < order.iter().position(|p| *p == parts::TEXT)
    );
}
...
fn semantics_roles_labels_and_interactions_follow_the_dom() {
    ...
    assert_eq!(
        semantics::interaction_id(&doc, cb),
...
fn regions_carry_semantics_focus_and_control_state() {
    let (doc, n) = form_doc();
    let [html, body, a, _h2, input, button, _img, cb, _nav, _div] = n[..] else {
        unreachable!()
    };
    let mut styles = StyleSet::new();
    for id in &n {
    ...
    let mut field = Fragment::new(
        FragmentKind::Box {
            source: StyleSource::Element(input),
            padding: Edges::uniform(au(2)),
            border: Edges::uniform(au(1)),
            replaced: Some(Replaced::Control(ControlKind::TextInput)),
            scroll: None,
            baseline: Some(au(16)),
        },
        rect(0, 30, 150, 24),
    ...
    let btn = Fragment::new(
        FragmentKind::Box {
            ...
            padding: Edges::ZERO,
            border: Edges::ZERO,
            replaced: Some(Replaced::Control(ControlKind::Button)),
            scroll: None,
            baseline: None,
        },
        rect(0, 60, 80, 24),
    );
    let check = Fragment::new(
        FragmentKind::Box {
            source: StyleSource::Element(cb),
            padding: Edges::ZERO,
            border: Edges::ZERO,
            replaced: Some(Replaced::Control(ControlKind::Checkbox)),
            scroll: None,
            baseline: None,
        },
        rect(0, 90, 13, 13),
    ...
    assert_eq!(
        sem.value.as_deref(),
        Some("dogs"),
        "live value overrides the attribute"
    );
    ...
    assert!(scene
        .nodes
        .iter()
        .any(|n| decode(n.id).0 == cb.0 && matches!(n.primitive, Primitive::Path { .. })));
    assert!(scene.nodes.iter().any(|n| n.painted_text() == Some("dogs")));
    let caret = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).2 == parts::CARET)
        .expect("caret");
    ...
    assert!(ax
        .iter()
        .any(|a| a.id == "q" && a.focused && a.role == "textbox"));
}
...
fn hit_testing_honours_stacking_clips_and_pointer_events() {
    ...
    assert_eq!(
        hit::hit_test(&t, &styles, 75, 25),
        Some(NodeId(1)),
        "clipped child is not hit beyond its parent"
    );
    assert_eq!(
        hit::hit_test(&t, &styles, 120, 120),
        Some(NodeId(1)),
        "pointer-events: none falls through"
    );
    assert_eq!(
        hit::hit_test(&t, &styles, 10, 10),
        Some(NodeId(5)),
        "topmost stacking context wins"
    );
...
fn ids_are_stable_across_paints_and_unrelated_edits() {
    ...
    assert!(
        ids_a.is_subset(&ids_c),
        "inserting a sibling first keeps every existing id"
    );
    assert_eq!(ids_c.len(), ids_a.len() + 1);
    // Ids decode to their node and part.
    assert_eq!(
        decode(scene_id(0, NodeId(3), 0, parts::BACKGROUND)),
        (3, 0, parts::BACKGROUND)
    );
    assert_eq!(
        scene_id(1000, NodeId(3), 0, 1),
        1000 + scene_id(0, NodeId(3), 0, 1)
    );
    ...
    root.children = vec![
        line(
            rect(0, 0, 200, 20),
            vec![inline_box(2, rect(0, 0, 50, 20), true, false)],
        ),
        line(
            rect(0, 20, 200, 20),
    ...
    let ords: Vec<u32> = s
        .nodes
        .iter()
        .filter(|n| decode(n.id).0 == 2 && decode(n.id).2 == parts::BACKGROUND)
        .map(|n| decode(n.id).1)
        .collect();
...
fn opacity_groups_clips_and_visibility() {
    ...
    assert!(
        bg.contains(&2) && bg.contains(&4) && !bg.contains(&3),
        "hidden box paints nothing but its visible child does"
    );
    let group_bg = scene
        .nodes
        .iter()
    ...
    let child = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (4, 0, parts::BACKGROUND))
        .unwrap();
    assert_eq!(
        child.opacity, 128,
        "the group opacity reaches every node of the group"
    );
    assert_eq!(
        child.clip,
        Some(cw_scene::Rect::new(10, 10, 100, 100)),
        "overflow clip is the padding box"
    );
    assert_eq!(
        group_bg.clip,
        Some(cw_scene::Rect::new(0, 0, 200, 200)),
        "a box is not clipped by its own overflow"
    );
}
...
fn transforms_compose_about_the_origin() {
    ...
    set(&mut styles, 2, |s| {
        s.background_color = RED;
        s.transform = vec![TransformOp::Translate(
            LengthPercentage::Length(au(10)),
            LengthPercentage::Length(au(-5)),
        )];
    });
    set(&mut styles, 3, |s| {
        s.background_color = RED;
    ...
    assert_eq!(
        child.transform.point(25, 25),
        (35, 20),
        "the centre only translates"
    );
    assert_eq!(
        child.transform.point(20, 20),
        (25, 10),
        "a corner moves out from the centre, then translates"
    );
    // The rotated box's own background rotates about its origin (its top-left here).
    let turned = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (3, 0, parts::BACKGROUND))
        .unwrap();
    assert_eq!(turned.transform.point(140, 100), (100, 140));
    // Rotation about (100, 100): the point (140, 100) maps to (100, 140).
    let rot = display_list::transform_matrix(
        &[TransformOp::Rotate(9000)],
        crate::geom::Size {
            width: au(40),
            height: au(40),
        },
    ...
    let sc = display_list::transform_matrix(
        &[TransformOp::Scale(2000, 2000)],
        crate::geom::Size {
            width: au(10),
            height: au(10),
        },
...
fn scroll_areas_are_registered_for_the_root_and_scroll_containers() {
    ...
    let mut list = Fragment::new(
        FragmentKind::Box {
            source: StyleSource::Element(div),
            padding: Edges::ZERO,
            border: Edges::uniform(au(1)),
            replaced: None,
            scroll: Some(ScrollInfo {
                content_width: au(90),
                content_height: au(400),
                scroll_x: Au::ZERO,
                scroll_y: au(30),
                origin_x: Au::ZERO,
                origin_y: Au::ZERO,
    ...
    let child = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (item.0, 0, parts::BACKGROUND))
        .unwrap();
    assert_eq!(
        child.bounds,
        cw_scene::Rect::new(10, 10 - 50 - 30, 90, 400),
        "shifted by both scrolls"
    );
    assert_eq!(
        child.clip,
        Some(cw_scene::Rect::new(11, 0, 98, 98 - 39)),
        "clipped to the padding box and the viewport"
    );
    let list_bg = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (div.0, 0, parts::BACKGROUND))
        .unwrap();
...
fn gradients_rasterise_deterministically_and_cache() {
    let g = BackgroundImage::LinearGradient {
        angle_centi_deg: 18_000,
        stops: vec![
            GradientStop {
                color: RED,
                position: None,
    ...
    assert_eq!(
        ...
        (1, 100),
        "an axis-aligned gradient is a strip"
    );
    assert!(
        img.rgba[0] > 250 && img.rgba[2] < 5,
        "top is the start colour: {:?}",
        &img.rgba[0..4]
    );
    assert!(
        img.rgba[396] < 5 && img.rgba[398] > 250,
        "bottom is the end colour: {:?}",
        &img.rgba[396..400]
    );
    let mid = &img.rgba[200..204];
    assert!(mid[0] > 100 && mid[2] > 100);
    let diag = BackgroundImage::LinearGradient {
        angle_centi_deg: 4500,
        stops: vec![
            GradientStop {
                color: RED,
                position: Some(LengthPercentage::Percent(2000)),
            },
            GradientStop {
                color: BLUE,
                position: Some(LengthPercentage::Percent(8000)),
            },
        ],
    };
    ...
    assert_eq!(
        &d.rgba[(19 * 20) * 4..(19 * 20) * 4 + 4],
        &[255, 0, 0, 255],
        "bottom-left corner is the start colour"
    );
    assert_eq!(
        &d.rgba[19 * 4..19 * 4 + 4],
        &[0, 0, 255, 255],
        "top-right corner is the end colour"
    );
    let radial = BackgroundImage::RadialGradient {
        circle: true,
        stops: vec![
            GradientStop {
                color: RED,
    ...
    assert_eq!(
        &r.rgba[(10 * 21 + 10) * 4..(10 * 21 + 10) * 4 + 4],
        &[255, 0, 0, 255]
    );
    ...
    let scene = p.finish();
    let imgs: Vec<_> = scene
        .nodes
        .iter()
        .filter(|n| matches!(n.primitive, Primitive::Image { .. }))
        .collect();
...
fn background_images_tile_scale_and_fix() {
    ...
    images
        .0
        .insert("tile.png".into(), RgbaImage::solid(10, 10, RED));
    let ctx = PaintContext::new(&images);
    let mut styles = StyleSet::new();
    set(&mut styles, 1, |_| {});
    let base = BackgroundLayer {
        image: BackgroundImage::Url("tile.png".into()),
        repeat: BackgroundRepeat::Repeat,
        size: BackgroundSize::Auto,
        position: (LengthPercentage::ZERO, LengthPercentage::ZERO),
        origin: BackgroundBox::PaddingBox,
        clip: BackgroundBox::PaddingBox,
        attachment_fixed: false,
    };
    ...
    set(&mut styles, 5, |s| {
        s.background = vec![BackgroundLayer {
            ...
            position: (
                ...
                LengthPercentage::Percent(10_000),
            ),
            ..base.clone()
        }]
    });
    ...
    let of = |n: u32| {
        scene
            .nodes
            .iter()
            .filter(move |x| decode(x.id).0 == n && matches!(x.primitive, Primitive::Image { .. }))
            .collect::<Vec<_>>()
    };
    assert_eq!(of(2).len(), 9, "3 x 3 tiles cover a 25 px box");
    assert!(of(2)
        .iter()
        .all(|n| n.clip == Some(cw_scene::Rect::new(0, 0, 25, 25))));
    let cover = of(3);
    assert_eq!(cover.len(), 1);
    assert_eq!(
        cover[0].bounds,
        cw_scene::Rect::new(50, 0, 40, 40),
        "cover scales to the larger ratio"
    );
    assert!(
        matches!(
            cover[0].primitive,
        ...
        "resampled here, not by the renderer"
    );
    let fixed = of(4);
    ...
    assert!(
        fixed
            .iter()
            .all(|n| n.bounds.x % 10 == 0 && n.bounds.y % 10 == 0),
        "fixed layers tile from the viewport origin"
    );
    assert!(fixed
        .iter()
        .all(|n| n.clip == Some(cw_scene::Rect::new(0, 100, 30, 30))));
    let row = of(5);
    assert_eq!(
        row.len(),
        4,
        "tiles at 108, 118, 128, 138 cross the 100..145 box"
    );
    assert!(
        row.iter().all(|n| n.bounds.y == 120),
        "bottom-aligned by the 100% position"
    );
...
fn replaced_images_and_placeholders() {
    ...
    images
        .0
        .insert("a.png".into(), RgbaImage::solid(20, 10, RED));
    let ctx = PaintContext::new(&images);
    let mut styles = StyleSet::new();
    set(&mut styles, 1, |_| {});
    ...
    root.children = vec![
        ...
        Fragment::new(
            FragmentKind::Box {
                source: StyleSource::Element(NodeId(4)),
                padding: Edges::ZERO,
                border: Edges::ZERO,
                replaced: Some(Replaced::Placeholder("iframe".into())),
                scroll: None,
                baseline: None,
            },
            rect(100, 100, 100, 60),
    ...
    let contain = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).0 == 2 && matches!(n.primitive, Primitive::Image { .. }))
        .unwrap();
    assert_eq!(contain.bounds, cw_scene::Rect::new(0, 25, 100, 50));
    let missing = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (3, 0, parts::CONTENT))
        .unwrap();
    assert!(matches!(
        missing.primitive,
        Primitive::Box {
    ...
    assert!(scene
        .nodes
        .iter()
        .any(|n| decode(n.id).0 == 3 && n.painted_text() == Some("photo")));
    assert!(scene
        .nodes
        .iter()
        .any(|n| decode(n.id).0 == 4 && n.painted_text() == Some("iframe")));
    let cover = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).0 == 5 && matches!(n.primitive, Primitive::Image { .. }))
        .unwrap();
...
fn controls_draw_from_dom_state() {
    ...
    let ctl = |n: NodeId, k: ControlKind, r: Rect| {
        Fragment::new(
            FragmentKind::Box {
                source: StyleSource::Element(n),
                padding: Edges::ZERO,
                border: Edges::ZERO,
                replaced: Some(Replaced::Control(k)),
                scroll: None,
                baseline: None,
            },
            r,
    ...
    let chevron = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (select.0, 0, parts::CONTENT_GLYPH))
        .unwrap();
    assert!(matches!(chevron.primitive, Primitive::Path { .. }));
    let thumb = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (range.0, 0, parts::CONTENT_GLYPH))
        .unwrap();
    assert_eq!(
        thumb.bounds,
        cw_scene::Rect::new(25, 34, 12, 12),
        "25% along a 100 px span"
    );
    let dot = scene
        .nodes
        .iter()
        .find(|n| decode(n.id) == (radio.0, 0, parts::CONTENT_GLYPH))
        .unwrap();
    assert!(
        matches!(
            dot.primitive,
            Primitive::RoundedBox {
                fill: Color(109, 109, 109, 255),
                ..
            }
        ),
        "disabled radio dot is grey"
    );
    let placeholder = scene
        .nodes
        .iter()
        .find(|n| n.painted_text() == Some("Type here"))
        .unwrap();
    assert!(matches!(
    ...
    assert!(
        scene.nodes.iter().all(|n| decode(n.id).2 != parts::CARET),
        "no caret without focus"
    );
    scene.validate().unwrap();
}
...
fn inline_fragments_open_their_cut_ends() {
    ...
    a.kind = FragmentKind::InlineBox {
        source: StyleSource::Element(NodeId(2)),
        padding: Edges::ZERO,
        border: Edges {
            top: au(1),
            right: Au::ZERO,
    ...
    let parts_of = |ord: u32| {
        scene
            .nodes
            .iter()
            .filter(|n| decode(n.id).0 == 2 && decode(n.id).1 == ord)
            .map(|n| decode(n.id).2)
            .collect::<Vec<_>>()
    };
    assert_eq!(
...
fn golden_composite_digest() {
    ...
    set(&mut styles, nav.0, |s| {
        s.background = vec![BackgroundLayer {
            image: BackgroundImage::LinearGradient {
                angle_centi_deg: 9000,
                stops: vec![
                    GradientStop {
                        color: RED,
                        position: None,
    ...
    let ctl = |n: NodeId, k: ControlKind, r: Rect, b: Edges| {
        Fragment::new(
            FragmentKind::Box {
                source: StyleSource::Element(n),
                padding: Edges::uniform(au(2)),
    ...
    let picture = Fragment::new(
        FragmentKind::Box {
            source: StyleSource::Element(img),
            padding: Edges::ZERO,
            border: Edges::ZERO,
            replaced: Some(Replaced::Image {
                src: "cat.png".into(),
    ...
    let empty = {
        let mut s = paint(
            &doc,
            &styles,
            &tree(boxf(html.0, rect(0, 0, 320, 240))),
            viewport(320, 240),
            &ctx,
        );
        s.stamp();
    ...
    assert_eq!(
        scene.digest,
        GOLDEN_DIGEST,
        "scene digest changed: {}",
        serde_json::to_string(
            &scene
                .nodes
                .iter()
                .map(|n| (n.id, n.bounds, n.z))
                .collect::<Vec<_>>()
        )
        .unwrap()
    );
...
fn the_root_scroll_offset_is_applied_once() {
    ...
    let bg = scene
        .nodes
        .iter()
        .find(|n| decode(n.id).0 == 1 && decode(n.id).2 == parts::BACKGROUND)
        .expect("background");
...
fn path_points_are_relative_to_the_node_bounds() {
    ...
    let paths: Vec<_> = scene
        .nodes
        .iter()
        .filter(|n| matches!(n.primitive, Primitive::Path { .. }))
        .collect();
    assert_eq!(paths.len(), 5, "four border sides and one background");
    for n in paths {
        let Primitive::Path { points, .. } = &n.primitive else {
            unreachable!()
        };
        ...
        assert!(
            ...
            "points lie inside the {w}x{h} bounds at {:?}: {points:?}",
            n.bounds
        );
    }
}
