...
pub(crate) fn paint(p: &mut Painter, f: &Fragment, state: &State) {
    ...
    match replaced {
        ...
        Replaced::Control(kind) => paint_control(
            p,
            key,
            state,
            &style,
        ...
        Replaced::Placeholder(tag) => {
            ...
            p.emit(
                ...
                r,
                Primitive::Box {
                    fill: PLACEHOLDER_BOX,
                    border: Some(PLACEHOLDER_EDGE),
                    border_width: 1,
                },
            );
            let label = tag.clone();
        ...
        Replaced::Marker(marker) => {
            ...
            let baseline_px = if baseline.is_some() {
                baseline_px
            } else {
                r.y + style.font.size_px() as i32
            };
...
fn paint_image(
    ...
    let Some(img) = p.ctx.images.image(src) else {
        ...
        if !alt.is_empty() && r.width > 6 && r.height as i32 > style.font.size_px() as i32 + 2 {
            ...
            let shown = cw_scene::metrics::ellipsize(
                style.font.typeface,
                style.font.scene_style(),
                alt,
                style.font.size_px(),
                r.width - 4,
            );
    ...
    let bounds = SRect::new(
        dx,
        dy,
        dw.min(u32::MAX as u64) as u32,
        dh.min(u32::MAX as u64) as u32,
    );
    let s = if clip {
        state.clipped(r)
    } else {
...
fn paint_control(
    ...
    let placeholder = doc
        .and_then(|d| d.attr(node, "placeholder"))
        .map(semantics::collapse)
        .unwrap_or_default();
    ...
    match kind {
        ControlKind::TextInput | ControlKind::Password | ControlKind::File | ControlKind::Color => {
            let shown = if kind == ControlKind::Password {
                "\u{2022}".repeat(value.chars().count())
            } else {
                value.clone()
            };
            let (text_shown, color) = if shown.is_empty() {
                (placeholder.clone(), hint)
            } else {
                (shown.clone(), ink)
            };
            ...
            let baseline = baseline
                .map(|b| rect_y + px(b))
                .filter(|b| *b >= r.y && *b <= r.bottom())
                .unwrap_or_else(|| centred_baseline(r, size));
        ...
        ControlKind::TextArea => {
            let (text_shown, color) = if value.is_empty() {
                (placeholder.clone(), hint)
            } else {
                (value.clone(), ink)
            };
            let lh = text::line_height_px(size) as i32;
            let lines = cw_scene::metrics::wrap(
                font.typeface,
                font.scene_style(),
                &text_shown,
                size,
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                last = (
                    r.x + text::width_px(&font, shown) as i32,
                    y,
                    line_no,
                    shown.chars().count() as u32,
                );
                y += lh;
            }
            if focused && !disabled {
                let (cx, cy, line, col) = if value.is_empty() {
                    (r.x, r.y + size as i32, 0, 0)
                } else {
                    last
                };
        ...
        ControlKind::Checkbox | ControlKind::Radio => {
            let checked = doc.is_some_and(|d| {
                d.has_attr(node, "checked") || d.attr(node, "aria-checked") == Some("true")
            });
        ...
        ControlKind::Select => {
            ...
            if !value.is_empty() && text_w > 0 {
                ...
                text::draw_text(
                    p,
                    &state.clipped(SRect::new(r.x, r.y, text_w, r.height)),
                    id,
                    r.x,
                    centred_baseline(r, size),
                    &shown,
        ...
        ControlKind::Range => {
            ...
            let frac = if max > min {
                (val - min).clamp(0, max - min)
            } else {
                0
            };
            let tx = r.x
                + if max > min {
                    (span * frac / (max - min)) as i32
                } else {
                    0
                };
            ...
            p.emit(
                state,
                id,
                SRect::new(tx, ty, thumb, thumb),
                Primitive::RoundedBox {
                    fill: if disabled { DISABLED_TEXT } else { ACCENT },
                    border: None,
                    border_width: 0,
                    radius: thumb / 2,
                },
