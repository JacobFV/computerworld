#!/usr/bin/env python3
"""Fetches web-platform-tests at a pinned commit and regenerates `crates/web/engine/tests/wpt/`.
...
What is written under `crates/web/engine/tests/wpt/`: every selected test and reference plus
...
Run from anywhere: `python3 crates/web/engine/tools/fetch-wpt.py [--tarball path] [--check]`.
...
def readme(taken, skipped, file_count):
    ...
    lines = [
        ...
        f"`{COMMIT}`, produced by `crates/web/engine/tools/fetch-wpt.py` (pinned tarball SHA-256 `{SHA256}`). "
        "The tests are copyright the W3C and the WPT contributors and are used under the 3-Clause BSD "
        "License in `LICENSE.md`.",
        "",
        "The runner is `crates/web/engine/tests/wpt.rs`: for every pair in `manifest.json` it parses the test "
        ...
        "they differ. The report lands in `crates/web/engine/target-parity/wpt-report.md`, grouped by directory; "
