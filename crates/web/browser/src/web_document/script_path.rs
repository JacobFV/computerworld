    ...
    doc.descendants(Document::ROOT).any(|n| match doc.kind(n) {
        NodeKind::Element { tag, attrs, .. } => {
            tag == "script"
                || attrs
                    .iter()
                    .any(|a| a.name.len() > 2 && a.name.starts_with("on"))
        }
        _ => false,
    })
}
...
impl WebDocument {
        ...
        self.base = self
            .with_document(|d| {
                ...
                let href = d
                    .descendants(head)
                    .find(|n| d.is(*n, "base"))
                    .and_then(|n| d.attr(n, "href").map(str::to_owned))?;
        ...
        s.read(|realm| {
            ...
            let push = |reference: String, base: Option<&Url>, out: &mut Vec<(String, String)>| {
                if self.refs.contains_key(&reference)
                    || out.iter().any(|(r, _)| *r == reference)
                    || known + out.len() >= MAX_IMAGES
                {
                    return;
                }
                ...
                if let Some(mut u) = resolved.filter(|u| {
    ...
    pub(crate) fn script_set_image_sizes(&mut self, env: &mut HostEnv, references: &[String]) {
        let sizes: Vec<(String, u32, u32)> = references
            .iter()
            .filter_map(|r| {
                self.images
                    .get(self.refs.get(r)?)
                    .map(|a| (r.clone(), a.width, a.height))
            })
            .collect();
        if sizes.is_empty() {
            return;
    ...
    pub(crate) fn script_fields(&self) -> BTreeMap<String, String> {
        ...
        s.read(|realm| {
            ...
            d.descendants(Document::ROOT)
                .filter(|n| inner.is_text_control(*n))
                .map(|n| (semantics::interaction_id(d, n), inner.control_value(n)))
                .collect()
        })
    }
    ...
    pub(crate) fn script_focus_and_scroll(&self) -> (Option<String>, i32) {
        ...
        s.read(|realm| {
            ...
            let focused = inner
                .focused
                .map(|n| semantics::interaction_id(&inner.doc, n));
    ...
    pub(super) fn script_scene(&self, inputs: Inputs<'_>) -> Scene {
        let Some(s) = &self.script else {
            return Scene::new(inputs.width, inputs.height);
        };
        ...
        let mut scene = s.read(|realm| {
            let canvases: BTreeMap<String, RgbaImage> = realm
                .canvases()
                .into_iter()
                .map(|(n, image)| (format!("canvas:{}", n.0), image))
                .collect();
            let inner = realm.layout();
            let Some(tree) = inner.tree.as_ref() else {
                return Scene::new(width, height);
            };
            let images = ScriptImages {
                refs: &self.refs,
                decoded,
                canvases: &canvases,
            };
            ...
            pctx.scroll = Point {
                x: sx.clamp(Au::ZERO, max_x),
                y: sy.clamp(Au::ZERO, max_y),
            };
            pctx.scroll_offsets = inner
                .scroll
                .iter()
                .filter(|(n, _)| **n != Document::ROOT)
                .map(|(n, (x, y))| (*n, Point { x: *x, y: *y }))
                .collect();
        ...
        guard.0.script_render = Some(ScriptRender {
            epoch,
            width,
            height,
            zoom,
    ...
    pub(crate) fn css_point(x: i32, y: i32, zoom: u16) -> (i32, i32) {
        ...
        (
            (i64::from(x) * 100 / z) as i32,
            (i64::from(y) * 100 / z) as i32,
        )
    }
    ...
    pub(super) fn script_cursor_at(&self, x: i32, y: i32, inputs: Inputs<'_>) -> &'static str {
        let Some(node) = self.script_hit(x, y, inputs) else {
            return "default";
        };
        let Some(s) = &self.script else {
            return "default";
        ...
        s.read(|realm| {
            ...
            if std::iter::once(node)
                .chain(d.ancestors(node))
                .any(|n| (d.is(n, "a") || d.is(n, "area")) && d.has_attr(n, "href"))
            {
                return "pointer";
            }
            ...
            if matches!(
                d.tag(node),
                Some("button" | "select" | "summary" | "label" | "input")
            ) {
                return "default";
            }
            let (sx, sy) = inner.window_scroll();
            let on_text = inner.tree.as_ref().is_some_and(|t| {
                t.hit(Au::from_px_i32(cx) + sx, Au::from_px_i32(cy) + sy)
                    .last()
