...
pub fn resolve_family_in(families: &[String], env: FontEnvironment) -> Typeface {
    match env {
        ...
        FontEnvironment::LinuxBaseline => families.iter().find_map(|f| linux_baseline_face(f)).unwrap_or(Typeface::Arimo),
    }
}
...
fn linux_baseline_face(name: &str) -> Option<Typeface> {
    ...
    Some(match name.as_str() {
        "sans-serif" | "system-ui" | "ui-sans-serif" | "arial" | "helvetica" | "liberation sans" | "arimo" | "nimbus sans" => Typeface::Arimo,
        "serif" | "times" | "times new roman" | "liberation serif" | "tinos" | "nimbus roman" => Typeface::Tinos,
        ...
        "monospace" | "courier" | "courier new" | "liberation mono" | "cousine" => Typeface::Cousine,
...
mod tests {
    ...
    fn a_linux_baseline_device_has_only_liberation_and_dejavu() {
        ...
        assert_eq!(resolve_family_in(&list(&["Inter", "-apple-system", "Segoe UI", "Roboto", "sans-serif"]), env), Typeface::Arimo);
        assert_eq!(resolve_family_in(&list(&["Poppins", "Montserrat", "Inter", "sans-serif"]), env), Typeface::Arimo);
        assert_eq!(resolve_family_in(&list(&["Georgia", "Times New Roman", "serif"]), env), Typeface::Tinos);
        assert_eq!(resolve_family_in(&list(&["JetBrains Mono", "Courier New", "monospace"]), env), Typeface::Cousine);
        assert_eq!(resolve_family_in(&list(&["ui-monospace", "JetBrains Mono", "Courier  NEW", "monospace"]), env), Typeface::Cousine);
        assert_eq!(resolve_family_in(&list(&["JetBrains Mono", "Fira Code", "Menlo", "Consolas", "monospace"]), env), Typeface::Mono);
        assert_eq!(resolve_family_in(&list(&["Roboto", "Open Sans", "Arial", "sans-serif"]), env), Typeface::Arimo);
        assert_eq!(resolve_family_in(&list(&["monospace"]), env), Typeface::Cousine);
        assert_eq!(resolve_family_in(&list(&["Nope"]), env), Typeface::Arimo);
        // The world's browser keeps its bundled faces.
        assert_eq!(resolve_family_in(&list(&["Inter", "sans-serif"]), FontEnvironment::Bundled), Typeface::Inter);
    }
