...
use crate::style::{
    Clear, ComputedStyle, Content, ContentItem, Display, Float, LengthPercentage, LengthPercentageAuto, ListStylePosition, ListStyleType, Position, Sizing, StyleSet,
    WhiteSpace,
};
...
impl Dim {
    pub fn parse(s: &str) -> Option<Dim> {
        ...
        let frac: i64 = frac_s.chars().take(2).collect::<String>().parse().unwrap_or(0) * if frac_s.len() >= 2 { 1 } else { 10 };
        let centi = whole * 100 + frac;
        if pct {
            Some(Dim::Percent(centi.min(i32::MAX as i64) as i32))
        } else {
            Some(Dim::Px(Au((centi * 64 / 100).clamp(0, Au::MAX.0 as i64) as i32)))
        }
    }
...
impl LayoutBox {
    pub fn is_block_container(&self) -> bool {
        matches!(self.kind, BoxKind::Block | BoxKind::InlineBlock | BoxKind::Cell(_) | BoxKind::Caption)
    }
    pub fn is_table_internal(&self) -> bool {
        matches!(self.kind, BoxKind::RowGroup | BoxKind::Row | BoxKind::Cell(_) | BoxKind::ColGroup(_) | BoxKind::Col(_) | BoxKind::Caption)
    }
    ...
    pub fn is_scroll_container(&self) -> bool {
        ...
        matches!(self.style.overflow_x, O::Hidden | O::Scroll | O::Auto | O::Clip) || matches!(self.style.overflow_y, O::Hidden | O::Scroll | O::Auto | O::Clip)
    }
}
...
pub fn build(doc: &Document, styles: &StyleSet, images: &dyn ImageSizes) -> BoxTree {
    let mut b = Builder { doc, styles, images, boxes: Vec::with_capacity(doc.len()), by_node: BTreeMap::new(), counters: Vec::new(), quote_depth: 0 };
    let root = doc.document_element().and_then(|html| {
        let items = b.build_element(html, 0);
        items.into_iter().find(|&i| b.boxes[i.index()].level == Level::Block)
    });
    if let Some(r) = root {
        ...
        if s.display.is_inline_level() || s.is_out_of_flow() {
            ...
            st.position = if st.position == Position::Relative { Position::Relative } else { Position::Static };
            st.float = Float::None;
            b.boxes[r.index()].style = Rc::new(st);
            b.boxes[r.index()].level = Level::Block;
        }
    }
    BoxTree { boxes: b.boxes, root, by_node: b.by_node }
}
...
impl<'a> Builder<'a> {
    fn push(&mut self, kind: BoxKind, style: Rc<ComputedStyle>, source: StyleSource, node: Option<NodeId>, level: Level) -> BoxId {
    ...
    fn level_of(display: Display) -> Level {
        if display.is_inline_level() { Level::Inline } else { Level::Block }
    }

    // Counters (CSS 2.1 §12.4).

    fn counter_reset(&mut self, name: &str, value: i32, depth: u32) {
        self.counters.push(Counter { name: name.to_owned(), value, depth });
    }
    fn counter_increment(&mut self, name: &str, by: i32, depth: u32) {
        match self.counters.iter_mut().rev().find(|c| c.name == name) {
            ...
            None => self.counters.push(Counter { name: name.to_owned(), value: by, depth }),
        }
    }
    fn counter_set(&mut self, name: &str, value: i32, depth: u32) {
        match self.counters.iter_mut().rev().find(|c| c.name == name) {
            Some(c) => c.value = value,
            None => self.counters.push(Counter { name: name.to_owned(), value, depth }),
        }
    }
    fn counter_value(&self, name: &str) -> i32 {
        self.counters.iter().rev().find(|c| c.name == name).map(|c| c.value).unwrap_or(0)
    }
    ...
    fn build_element(&mut self, node: NodeId, depth: u32) -> Vec<BoxId> {
        ...
        if matches!(tag, "head" | "script" | "style" | "template" | "meta" | "link" | "title" | "base" | "param" | "datalist" | "noscript" | "area") {
            return Vec::new();
        }
        if tag == "input" && self.doc.attr(node, "type").is_some_and(|t| t.eq_ignore_ascii_case("hidden")) {
            return Vec::new();
        }
        let Some(style) = self.styles.get_rc(node).cloned() else { return Vec::new() };
        ...
        if tag == "br" {
            let clear = if style.clear != Clear::None {
                ...
                match self.doc.attr(node, "clear").map(|s| s.to_ascii_lowercase()).as_deref() {
        ...
        match display {
            ...
            Display::TableRowGroup | Display::TableHeaderGroup | Display::TableFooterGroup => {
                let id = self.push(BoxKind::RowGroup, style.clone(), source, Some(node), Level::Block);
                self.by_node.insert(node, id);
            ...
            Display::TableRow => {
                let id = self.push(BoxKind::Row, style.clone(), source, Some(node), Level::Block);
                self.by_node.insert(node, id);
                let kids = self.build_children(node, &style, depth);
            ...
            Display::TableCell => {
                let cell = CellBox { colspan: attr_u32(self.doc, node, "colspan", 1).clamp(1, 1000), rowspan: attr_u32(self.doc, node, "rowspan", 1).clamp(1, 65534) };
                let id = self.push(BoxKind::Cell(cell), style.clone(), source, Some(node), Level::Block);
                self.by_node.insert(node, id);
            ...
            Display::TableCaption => {
                let id = self.push(BoxKind::Caption, style.clone(), source, Some(node), Level::Block);
                self.by_node.insert(node, id);
                self.build_container_contents(id, node, &style, depth);
                self.counters_leave(depth);
                vec![id]
            ...
            Display::TableColumnGroup => {
                ...
                let id = self.push(BoxKind::ColGroup(ColBox { span, width }), style.clone(), source, Some(node), Level::Block);
                self.by_node.insert(node, id);
                let mut cols = Vec::new();
                for c in self.doc.children(node).collect::<Vec<_>>() {
                    if self.doc.is(c, "col") {
                        let items = self.build_element(c, depth + 1);
                        cols.extend(items.into_iter().filter(|i| matches!(self.boxes[i.index()].kind, BoxKind::Col(_))));
                    }
                }
            ...
            Display::TableColumn => {
                let span = attr_u32(self.doc, node, "span", 1).clamp(1, 1000);
                let width = self.doc.attr(node, "width").and_then(Dim::parse);
                let id = self.push(BoxKind::Col(ColBox { span, width }), style, source, Some(node), Level::Block);
                self.by_node.insert(node, id);
                vec![id]
            }
            Display::Inline => {
                let id = self.push(BoxKind::Inline, style.clone(), source, Some(node), Level::Inline);
                self.by_node.insert(node, id);
                ...
                let has_block = kids.iter().any(|k| self.boxes[k.index()].level == Level::Block && !self.boxes[k.index()].is_out_of_flow());
                if !has_block {
                    self.boxes[id.index()].children = kids;
                    return vec![id];
                }
                ...
                for k in kids {
                    ...
                    if kb.level == Level::Block && !kb.is_out_of_flow() {
                        ...
                        out.push(k);
                        first = false;
                        piece = self.push(BoxKind::Inline, style.clone(), source, Some(node), Level::Inline);
                    } else {
                        piece_kids.push(k);
                    }
                }
            ...
            _ => {
                ...
                let kind = if display == Display::InlineBlock || (display.is_inline_level()) { BoxKind::InlineBlock } else { BoxKind::Block };
    ...
    fn build_container_contents(&mut self, id: BoxId, node: NodeId, style: &Rc<ComputedStyle>, depth: u32) {
        let mut kids = Vec::new();
    ...
    fn build_children(&mut self, node: NodeId, style: &Rc<ComputedStyle>, depth: u32) -> Vec<BoxId> {
        let mut out = Vec::new();
        ...
        for c in kids {
            match self.doc.kind(c) {
                ...
                NodeKind::Text(t) => {
                    ...
                    let id = self.push(BoxKind::Text(TextBox { text: t.clone(), node: Some(c) }), style.clone(), StyleSource::Element(node), None, Level::Inline);
                    out.push(id);
                }
                _ => {}
            }
    ...
    fn make_container(&mut self, id: BoxId, kids: Vec<BoxId>) {
        ...
        let flush = |this: &mut Self, run: &mut Vec<BoxId>, out: &mut Vec<BoxId>| {
            ...
            if has_content {
                let anon = this.push(BoxKind::Block, anon_style(&parent_style, Display::Block), StyleSource::Anonymous(this.boxes[id.index()].source.node()), None, Level::Block);
    ...
    fn wrap_stray_table_parts(&mut self, parent: BoxId, kids: Vec<BoxId>) -> Vec<BoxId> {
        if !kids.iter().any(|k| self.boxes[k.index()].is_table_internal()) {
            return kids;
        }
        ...
        let flush = |this: &mut Self, run: &mut Vec<BoxId>, out: &mut Vec<BoxId>| {
            ...
            let items = this.make_table(node, StyleSource::Anonymous(node), None, tstyle, std::mem::take(run));
            out.extend(items);
        };
        for k in kids {
            let b = &self.boxes[k.index()];
            if b.is_table_internal() {
                run.push(k);
            } else if !run.is_empty() && matches!(&b.kind, BoxKind::Text(t) if text::is_collapsible_whitespace(&t.text, b.style.white_space)) {
    ...
    fn build_table(&mut self, node: NodeId, style: &Rc<ComputedStyle>, depth: u32) -> Vec<BoxId> {
        ...
        self.make_table(node, StyleSource::Element(node), Some(node), style.clone(), kids)
    }

    /// Builds the wrapper and grid boxes for a table with these children, applying the
    /// missing-child rules (§17.2.1 rules 1 and 2).
    fn make_table(&mut self, node: NodeId, source: StyleSource, elem: Option<NodeId>, style: Rc<ComputedStyle>, kids: Vec<BoxId>) -> Vec<BoxId> {
        ...
        wrapper_style.display = if level == Level::Inline { Display::InlineBlock } else { Display::Block };
        ...
        grid_style.margin = crate::style::Sides::uniform(LengthPercentageAuto::Set(LengthPercentage::ZERO));
        ...
        let wrapper = self.push(BoxKind::TableWrapper, Rc::new(wrapper_style), anon, elem, level);
        let grid = self.push(BoxKind::Table, Rc::new(grid_style), source, elem, Level::Block);
        if let Some(n) = elem {
        ...
        let flush_stray = |this: &mut Self, stray: &mut Vec<BoxId>, groups: &mut Vec<BoxId>| {
            ...
            let g = this.push(BoxKind::RowGroup, anon_style(&style_rc, Display::TableRowGroup), anon, None, Level::Block);
        ...
        for k in kids {
            ...
            match &b.kind {
                ...
                BoxKind::Text(t) if text::is_collapsible_whitespace(&t.text, b.style.white_space) => {}
                _ => stray.push(k),
            }
        }
        flush_stray(self, &mut stray, &mut groups);
    ...
    fn fixup_row_group_children(&mut self, style: &Rc<ComputedStyle>, kids: Vec<BoxId>) -> Vec<BoxId> {
        let mut out = Vec::new();
        let mut stray: Vec<BoxId> = Vec::new();
        ...
        let flush = |this: &mut Self, stray: &mut Vec<BoxId>, out: &mut Vec<BoxId>| {
            ...
            let r = this.push(BoxKind::Row, anon_style(style, Display::TableRow), anon, None, Level::Block);
    ...
    fn fixup_row_children(&mut self, style: &Rc<ComputedStyle>, kids: Vec<BoxId>) -> Vec<BoxId> {
        ...
        let flush = |this: &mut Self, stray: &mut Vec<BoxId>, out: &mut Vec<BoxId>| {
            ...
            let c = this.push(BoxKind::Cell(CellBox { colspan: 1, rowspan: 1 }), anon_style(style, Display::TableCell), anon, None, Level::Block);
    ...
    fn build_pseudo(&mut self, node: NodeId, parent: &Rc<ComputedStyle>, before: bool, depth: u32) -> Option<BoxId> {
        let pstyle = if before { self.styles.before(node) } else { self.styles.after(node) }?;
        ...
        let source = if before { StyleSource::Before(node) } else { StyleSource::After(node) };
        ...
        for it in items {
            match it {
                ...
                ContentItem::Counter(name, st) => buf.push_str(&text::counter_text(self.counter_value(&name), st)),
                ...
                ContentItem::Url(u) => {
                    if !buf.is_empty() {
                        let t = self.push(BoxKind::Text(TextBox { text: std::mem::take(&mut buf), node: None }), pstyle.clone(), source, None, Level::Inline);
                        kids.push(t);
                    }
                    let intrinsic = self.images.size(&u).map(|(w, h)| Size { width: Au::from_px_i32(w as i32), height: Au::from_px_i32(h as i32) });
                    let rb = ReplacedBox { replaced: Replaced::Image { src: u, alt: String::new() }, intrinsic, attr_width: None, attr_height: None };
                    let mut s = ComputedStyle::inherit_from(&pstyle);
                    s.display = Display::Inline;
                    let r = self.push(BoxKind::Replaced(rb), Rc::new(s), source, None, Level::Inline);
                    kids.push(r);
                }
            }
        }
        if !buf.is_empty() {
            let t = self.push(BoxKind::Text(TextBox { text: buf, node: None }), pstyle.clone(), source, None, Level::Inline);
            kids.push(t);
        }
        if is_inline {
            self.boxes[id.index()].children = kids;
    ...
    fn list_item_marker(&mut self, id: BoxId, node: NodeId, style: &Rc<ComputedStyle>, depth: u32) {
        ...
        let tag = self.doc.tag(node).unwrap_or("");
        if tag == "li" {
            if let Some(v) = self.doc.attr(node, "value").and_then(|v| v.trim().parse::<i32>().ok()) {
        ...
        let m = if outside {
            self.push(BoxKind::Marker(txt), mstyle, StyleSource::Marker(node), None, Level::Block)
        } else {
            let m = self.push(BoxKind::Inline, mstyle.clone(), StyleSource::Marker(node), None, Level::Inline);
            let t = self.push(BoxKind::Text(TextBox { text: txt, node: None }), mstyle, StyleSource::Marker(node), None, Level::Inline);
    ...
    /// rendering section).
    fn replaced_box(&mut self, node: NodeId, tag: &str, style: &ComputedStyle) -> Option<ReplacedBox> {
        let doc = self.doc;
        let px = |w: u32, h: u32| Some(Size { width: Au::from_px_i32(w as i32), height: Au::from_px_i32(h as i32) });
        ...
        match tag {
            "img" => {
                ...
                let intrinsic = self.images.size(&src).map(|(w, h)| Size { width: Au::from_px_i32(w as i32), height: Au::from_px_i32(h as i32) });
                Some(ReplacedBox { replaced: Replaced::Image { src, alt }, intrinsic, attr_width: attr_w, attr_height: attr_h })
            }
            "input" => {
                let ty = doc.attr(node, "type").map(|t| t.to_ascii_lowercase()).unwrap_or_default();
                let (kind, intrinsic) = match ty.as_str() {
                    ...
                    "submit" | "button" | "reset" | "image" => {
                        let label = doc.attr(node, "value").map(str::to_owned).unwrap_or_else(|| match ty.as_str() {
                            "submit" => "Submit".into(),
                            "reset" => "Reset".into(),
                            _ => String::new(),
                        });
                        let w = text::measure(font, &label, style.letter_spacing, style.word_spacing);
                        (if ty == "submit" { ControlKind::Submit } else { ControlKind::Button }, Some(Size { width: w, height: lh }))
                    }
            ...
            "select" => Some(ReplacedBox { replaced: Replaced::Control(ControlKind::Select), intrinsic: Some(Size { width: ch * 20, height: lh }), attr_width: None, attr_height: None }),
            "textarea" => {
            ...
            "audio" => {
                if doc.has_attr(node, "controls") {
                    Some(ReplacedBox { replaced: Replaced::Placeholder(tag.to_owned()), intrinsic: px(300, 54), attr_width: attr_w, attr_height: attr_h })
                } else {
                    Some(ReplacedBox { replaced: Replaced::Placeholder(tag.to_owned()), intrinsic: px(0, 0), attr_width: None, attr_height: None })
                }
            }
            "progress" | "meter" => Some(ReplacedBox { replaced: Replaced::Placeholder(tag.to_owned()), intrinsic: Some(Size { width: Au::from_px_i32(160), height: lh }), attr_width: None, attr_height: None }),
            _ => None,
        }
    }
}
...
fn attr_u32(doc: &Document, node: NodeId, name: &str, default: u32) -> u32 {
    doc.attr(node, name).and_then(|v| v.trim().parse::<u32>().ok()).unwrap_or(default)
}
