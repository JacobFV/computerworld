...
impl<'a> Stream<'a> {
    ...
    fn is_open_curly(&self) -> bool {
        matches!(self.peek(), Some(ComponentValue::Token(Token::OpenCurly)) | Some(ComponentValue::Block { open: Token::OpenCurly, .. }))
    }
    ...
    fn consume_component_value(&mut self) -> ComponentValue {
        ...
        self.pos += 1;
        match v {
            ComponentValue::Token(Token::OpenCurly) => self.consume_block(Token::OpenCurly, &Token::CloseCurly),
            ComponentValue::Token(Token::OpenSquare) => self.consume_block(Token::OpenSquare, &Token::CloseSquare),
            ComponentValue::Token(Token::OpenParen) => self.consume_block(Token::OpenParen, &Token::CloseParen),
    ...
    fn consume_qualified_rule(&mut self, nested: bool, stop_semicolon: bool) -> Option<QualifiedRule> {
        ...
        loop {
            ...
            if self.is_open_curly() {
                ...
                let non_ws: Vec<&ComponentValue> = prelude.iter().filter(|v| !v.is_whitespace()).take(2).collect();
                if let [ComponentValue::Token(Token::Ident(name)), ComponentValue::Token(Token::Colon)] = non_ws.as_slice() {
                    if name.starts_with("--") {
                        if nested {
    ...
    fn consume_declaration(&mut self, nested: bool, stop_semicolon: bool) -> Option<Declaration> {
        ...
        {
            let non_ws: Vec<usize> = value.iter().enumerate().rev().filter(|(_, v)| !v.is_whitespace()).map(|(i, _)| i).take(2).collect();
        ...
        if !custom {
            let has_curly = value.iter().any(|v| matches!(v, ComponentValue::Block { open: Token::OpenCurly, .. }));
            if has_curly && value.iter().filter(|v| !v.is_whitespace()).count() > 1 {
                return None;
            }
        }
        let name = if custom { name } else { name.to_ascii_lowercase() };
        ...
        loop {
            match self.peek() {
                ...
                Some(ComponentValue::Token(Token::Ident(_))) => {
                    ...
                    let mut sub = Stream { values: &self.values[start..temp_end], pos: 0 };
    ...
    fn consume_rule_list(&mut self, top_level: bool) -> Vec<Item> {
        ...
        loop {
            match self.peek() {
                None => return out,
                Some(v) if v.is_whitespace() => self.pos += 1,
                Some(ComponentValue::Token(Token::Cdo)) | Some(ComponentValue::Token(Token::Cdc)) => {
                    if top_level {
                        self.pos += 1;
                    } else {
...
fn to_values(src: &str) -> Vec<ComponentValue> {
    tokenize(src).into_iter().map(ComponentValue::Token).collect()
}

/// "Parse a list of component values".
pub fn parse_component_value_list(src: &str) -> Vec<ComponentValue> {
    let values = to_values(src);
    let mut s = Stream { values: &values, pos: 0 };
    let mut out = Vec::new();
    while !s.at_end() {
...
pub fn parse_declaration_values(values: &[ComponentValue]) -> Result<Declaration, ParseError> {
    ...
    s.consume_declaration(false, false).ok_or(ParseError::Invalid)
}
...
pub fn parse_rule_list(src: &str) -> Vec<Item> {
    let values = to_values(src);
    Stream { values: &values, pos: 0 }.consume_rule_list(false)
}
...
pub fn parse_stylesheet_rules(src: &str) -> Vec<Item> {
    let values = to_values(src);
    Stream { values: &values, pos: 0 }.consume_rule_list(true)
}
...
impl Stylesheet {
    ...
    pub fn effective_style_rules<'a>(&'a self, media: &Media, is_supported: &dyn Fn(&str, &[ComponentValue]) -> bool) -> Vec<StyleRuleRef<'a>> {
        let mut out = Vec::new();
        self.walk_rules(&self.rules, media, is_supported, None, &mut Vec::new(), &mut out);
        out
    }

    fn walk_rules<'a>(&'a self, rules: &'a [Rule], media: &Media, is_supported: &dyn Fn(&str, &[ComponentValue]) -> bool, layer: Option<usize>, layer_path: &mut Vec<String>, out: &mut Vec<StyleRuleRef<'a>>) {
        for r in rules {
            match r {
                Rule::Style { selectors, declarations, source_order } => out.push(StyleRuleRef { selectors, declarations, source_order: *source_order, layer }),
    ...
    pub fn all_style_rules(&self) -> Vec<StyleRuleRef<'_>> {
        fn walk<'a>(rules: &'a [Rule], out: &mut Vec<StyleRuleRef<'a>>) {
            for r in rules {
                match r {
                    Rule::Style { selectors, declarations, source_order } => out.push(StyleRuleRef { selectors, declarations, source_order: *source_order, layer: None }),
                    Rule::Media { rules, .. } | Rule::Supports { rules, .. } | Rule::Layer { rules, .. } => walk(rules, out),
                    _ => {}
                }
            }
        }
...
pub fn parse_stylesheet(src: &str, origin: Origin, strictness: Strictness) -> Result<Stylesheet, Unsupported> {
    let items = parse_stylesheet_rules(src);
    let mut c = Converter { strictness, unsupported: Vec::new(), order: 0, layer_order: Vec::new(), layer_path: Vec::new(), anon_layers: 0, seen_non_import: false };
    let rules = c.convert_items(items, None, true)?;
    Ok(Stylesheet { rules, origin, layer_order: c.layer_order, unsupported: c.unsupported })
}
...
impl Converter {
    fn report(&mut self, kind: UnsupportedKind, name: impl Into<String>, detail: impl Into<String>) -> Result<(), Unsupported> {
        let u = Unsupported { kind, name: name.into(), detail: detail.into() };
    ...
    fn register_layer(&mut self, name: &str) {
        let full = if self.layer_path.is_empty() { name.to_owned() } else { format!("{}.{}", self.layer_path.join("."), name) };
    ...
    fn convert_items(&mut self, items: Vec<Item>, parent: Option<&SelectorList>, top_level: bool) -> Result<Vec<Rule>, Unsupported> {
        let mut out = Vec::new();
                ...
                Item::QualifiedRule(q) => {
                    ...
                    if let Some(p) = parent {
                        if !pending_decls.is_empty() {
                            out.push(Rule::Style { selectors: p.clone(), declarations: std::mem::take(&mut pending_decls), source_order: self.next_order() });
                        }
                    }
        ...
        if let Some(p) = parent {
            if !pending_decls.is_empty() {
                out.push(Rule::Style { selectors: p.clone(), declarations: pending_decls, source_order: self.next_order() });
            }
        }
        Ok(out)
    }
    ...
    fn convert_style_rule(&mut self, q: QualifiedRule, parent: Option<&SelectorList>, out: &mut Vec<Rule>) -> Result<(), Unsupported> {
        ...
        let selectors = match parent {
            ...
            Some(p) => match parse_relative_list(&q.prelude, false) {
                ...
                Err(e) => {
                    self.report(UnsupportedKind::Selector, prelude_text.trim(), e.to_string())?;
                    return Ok(());
                }
            },
        };
        for s in &selectors.0 {
            if let Some(pe) = s.pseudo_element {
                if !pe.supported() {
                    self.report(UnsupportedKind::Selector, format!("::{}", pe.name()), "pseudo-element parsed but not rendered")?;
                }
            }
        }
        ...
        out.push(Rule::Style { selectors: selectors.clone(), declarations, source_order: order });
        if !rest.is_empty() {
    ...
    fn convert_at_rule(&mut self, a: AtRule, parent: Option<&SelectorList>, top_level: bool) -> Result<Option<Rule>, Unsupported> {
        let name = a.name.to_ascii_lowercase();
        let name = name.strip_prefix("-webkit-").map(str::to_owned).unwrap_or(name);
        match name.as_str() {
            "charset" => Ok(None),
            "import" => {
                ...
                let url = match items.next() {
                    Some(ComponentValue::Token(Token::String(s))) | Some(ComponentValue::Token(Token::Url(s))) => s.clone(),
                    Some(ComponentValue::Function { name, args }) if name.eq_ignore_ascii_case("url") => match args.iter().find(|v| !v.is_whitespace()) {
                ...
                let rest: Vec<ComponentValue> = a.prelude.iter().skip_while(|v| v.is_whitespace()).skip(1).cloned().collect();
                let mut rest = rest.as_slice();
                loop {
                    let trimmed_start = rest.iter().position(|v| !v.is_whitespace()).unwrap_or(rest.len());
                    rest = &rest[trimmed_start..];
                    match rest.first() {
                        Some(ComponentValue::Token(Token::Ident(i))) if i.eq_ignore_ascii_case("layer") => rest = &rest[1..],
                        Some(ComponentValue::Function { name, .. }) if name.eq_ignore_ascii_case("layer") || name.eq_ignore_ascii_case("supports") => rest = &rest[1..],
                        _ => break,
                    }
                }
                Ok(Some(Rule::Import { url, media: MediaQueryList::from_values(rest) }))
            }
            "namespace" => {
                if !top_level {
                    return Ok(None);
                }
                let items: Vec<&ComponentValue> = a.prelude.iter().filter(|v| !v.is_whitespace()).collect();
                let url_of = |v: &ComponentValue| match v {
                    ComponentValue::Token(Token::String(s)) | ComponentValue::Token(Token::Url(s)) => Some(s.clone()),
                    ComponentValue::Function { name, args } if name.eq_ignore_ascii_case("url") => match args.iter().find(|v| !v.is_whitespace()) {
                ...
                match items.as_slice() {
                    ...
                    [ComponentValue::Token(Token::Ident(p)), u] => Ok(url_of(u).map(|url| Rule::Namespace { prefix: Some(p.clone()), url })),
                    _ => Ok(None),
                }
            }
            "media" => {
                let Some(block) = a.block else { return Ok(None) };
                self.seen_non_import = true;
                let query = MediaQueryList::from_values(&a.prelude);
                let rules = self.convert_items(parse_block_contents_values(&block), parent, false)?;
            ...
            "supports" => {
                ...
                let Some(condition) = parse_supports_condition(&a.prelude) else {
                    self.report(UnsupportedKind::AtRule, "@supports", format!("invalid condition `{}`", component_values_to_string(&a.prelude).trim()))?;
                    return Ok(None);
                };
                let rules = self.convert_items(parse_block_contents_values(&block), parent, false)?;
            ...
            "layer" => {
                let names: Vec<String> = a
                    ...
                    .map(|group| {
                        let idents: Vec<&ComponentValue> = group.iter().filter(|v| !v.is_whitespace()).collect();
                        let mut s = String::new();
                        for (i, v) in idents.iter().enumerate() {
                            match v {
                                ComponentValue::Token(Token::Ident(x)) if i % 2 == 0 => s.push_str(x),
                                ComponentValue::Token(Token::Delim('.')) if i % 2 == 1 => s.push('.'),
                                _ => return None,
                            }
                        }
                ...
                match a.block {
                    None => {
                        ...
                        Ok(Some(Rule::Layer { names, rules: Vec::new() }))
                    }
                    Some(block) => {
                        self.seen_non_import = true;
                        ...
                        let rules = self.convert_items(parse_block_contents_values(&block), parent, false);
                        self.layer_path.pop();
                        Ok(Some(Rule::Layer { names: vec![name], rules: rules? }))
                    }
                }
            }
            "font-face" => {
            ...
            "keyframes" => {
                ...
                let name = match items.as_slice() {
                    [ComponentValue::Token(Token::Ident(n))] | [ComponentValue::Token(Token::String(n))] => n.clone(),
                    _ => return Ok(None),
                };
                ...
                for item in parse_rule_list_values(&block) {
                    if let Item::QualifiedRule(q) = item {
                        ...
                        let mut ok = true;
                        for group in q.prelude.split(|v| matches!(v, ComponentValue::Token(Token::Comma))) {
                            let parts: Vec<&ComponentValue> = group.iter().filter(|v| !v.is_whitespace()).collect();
                            match parts.as_slice() {
                                [ComponentValue::Token(Token::Ident(i))] if i.eq_ignore_ascii_case("from") => selectors.push(KeyframeSelector::From),
                                [ComponentValue::Token(Token::Ident(i))] if i.eq_ignore_ascii_case("to") => selectors.push(KeyframeSelector::To),
                                [ComponentValue::Token(Token::Percentage { value, .. })] => selectors.push(KeyframeSelector::Percentage(*value)),
                                _ => ok = false,
                            }
                        }
            ...
            "page" => {
                ...
                let selectors = a
                    ...
                    .filter(|s| !s.is_empty())
                    .collect();
                Ok(Some(Rule::Page { selectors, declarations: declarations_only(&block) }))
            }
            _ => {
                self.seen_non_import = true;
                self.report(UnsupportedKind::AtRule, format!("@{}", a.name), "unknown at-rule")?;
                Ok(Some(Rule::Unknown { name: a.name, prelude: a.prelude, block: a.block }))
            }
        }
    }
}
...
mod tests {
    ...
    fn decl(name: &str, value: &str, important: bool) -> Declaration {
        Declaration { name: name.into(), value: parse_component_value_list(value), important }
    }
    ...
    fn component_values_and_blocks() {
        ...
        assert_eq!(v.len(), 3);
        match &v[2] {
            ComponentValue::Block { open: Token::OpenSquare, contents } => {
                assert_eq!(contents.len(), 5);
                assert!(matches!(&contents[2], ComponentValue::Block { open: Token::OpenParen, contents } if contents.len() == 4 && contents[1] == ComponentValue::Token(Token::CloseSquare)));
            }
            o => panic!("{o:?}"),
        }
        let v = parse_component_value_list("a [b] (c) {d} e");
        assert_eq!(v.len(), 9);
        assert_eq!(parse_component_value(" rgba(1, 2) "), Ok(ComponentValue::Function { name: "rgba".into(), args: parse_component_value_list("1, 2") }));
        assert_eq!(parse_component_value(""), Err(ParseError::Empty));
        assert_eq!(parse_component_value(".foo"), Err(ParseError::ExtraInput));
        assert_eq!(parse_component_value_list("(a"), vec![ComponentValue::Block { open: Token::OpenParen, contents: vec![ComponentValue::Token(Token::Ident("a".into()))] }]);
        assert_eq!(component_values_to_string(&parse_component_value_list("a [b (c] d) e")), "a [b (c] d) e]");
        assert_eq!(component_values_to_string(&parse_component_value_list("a [b] (c) {d} e")), "a [b] (c) {d} e");
    }

    #[test]
    fn declarations() {
        assert_eq!(parse_declaration("color: red"), Ok(decl("color", "red", false)));
        assert_eq!(parse_declaration("COLOR : red !important"), Ok(decl("color", "red", true)));
        assert_eq!(parse_declaration("color: red ! /**/ IMPORTANT "), Ok(decl("color", "red", true)));
        assert_eq!(parse_declaration("color: red !important!"), Ok(decl("color", "red !important!", false)));
        assert_eq!(parse_declaration("color: red important"), Ok(decl("color", "red important", false)));
        assert_eq!(parse_declaration("--Custom-Prop: { a }"), Ok(decl("--Custom-Prop", "{ a }", false)));
        ...
        assert_eq!(parse_declaration("foo:;bar:;"), Ok(decl("foo", ";bar:;", false)));
    }

    #[test]
    fn declaration_list_recovery() {
        assert_eq!(parse_declaration_block("a:b; c:d 42!important;\n"), vec![decl("a", "b", false), decl("c", "d 42", true)]);
        assert_eq!(parse_declaration_block("z;a:b"), vec![decl("a", "b", false)]);
        assert_eq!(parse_declaration_block("z:x!;a:b"), vec![decl("z", "x!", false), decl("a", "b", false)]);
        assert_eq!(parse_declaration_block("a:b; c+:d"), vec![decl("a", "b", false)]);
        assert_eq!(parse_declaration_block("color: red; background: url(x;y); margin: 0"), vec![decl("color", "red", false), decl("background", "url(x;y)", false), decl("margin", "0", false)]);
        assert_eq!(parse_declaration_block("color: rgb(1,2; margin: 0"), vec![decl("color", "rgb(1,2; margin: 0", false)]);
        assert_eq!(parse_declaration_block("@media screen { div{;}} a:b;; @media print{div{"), vec![decl("a", "b", false)]);
        assert_eq!(parse_declaration_block("@ media screen { div{;}} a:b;; @media print{div{"), vec![]);
    ...
    fn rules_and_recovery() {
        ...
        assert_eq!(parse_rule("div { color: #aaa; } p{}"), Err(ParseError::ExtraInput));
        assert!(matches!(parse_rule("@foo bar;"), Ok(Item::AtRule(a)) if a.name == "foo" && a.block.is_none()));
        assert!(matches!(parse_rule("@foo { bar"), Ok(Item::AtRule(a)) if a.block.is_some()));
        assert!(matches!(parse_rule(" /**/ { color: #aaa  "), Ok(Item::QualifiedRule(q)) if q.prelude.is_empty()));
    ...
    fn style_rule(r: &Rule) -> (String, Vec<Declaration>, u32) {
        match r {
            Rule::Style { selectors, declarations, source_order } => (selectors.to_string(), declarations.clone(), *source_order),
    ...
    fn typed_stylesheet() {
        ...
        assert!(matches!(&s.rules[0], Rule::Import { url, media } if url == "a.css" && media.to_string() == "screen"));
        assert!(matches!(&s.rules[1], Rule::Import { url, media } if url == "b.css" && media.is_empty()));
        assert!(matches!(&s.rules[2], Rule::Namespace { prefix: Some(p), url } if p == "svg" && url == "http://www.w3.org/2000/svg"));
        let (sel, decls, order) = style_rule(&s.rules[3]);
        assert_eq!(sel, "a, b > c");
        assert_eq!(decls, vec![decl("color", "red", false), decl("margin", "0", true)]);
        assert_eq!(order, 0);
        match &s.rules[4] {
            Rule::Media { query, rules } => {
        ...
        match &s.rules[6] {
            Rule::Keyframes { name, frames } => {
                ...
                assert_eq!(frames[1].0, vec![KeyframeSelector::Percentage(Number::from_i64(50)), KeyframeSelector::To]);
                assert_eq!(frames[1].1, vec![decl("a", "2", false)]);
            }
            o => panic!("{o:?}"),
        }
        assert!(matches!(&s.rules[7], Rule::Supports { rules, .. } if rules.len() == 1));
        assert!(matches!(&s.rules[8], Rule::Layer { names, rules } if names == &["base", "theme"] && rules.is_empty()));
        assert!(matches!(&s.rules[9], Rule::Layer { names, rules } if names == &["base"] && rules.len() == 2));
        assert!(matches!(&s.rules[10], Rule::Page { selectors, declarations } if selectors == &[":first"] && declarations.len() == 1));
        ...
        let names: Vec<(String, Option<usize>)> = eff.iter().map(|r| (r.selectors.to_string(), r.layer)).collect();
        assert_eq!(names, vec![("a, b > c".into(), None), (".x".into(), None), (".g".into(), None), (".b".into(), Some(0)), (".i".into(), Some(2)), (".after".into(), None)]);
    ...
    fn nesting_is_flattened() {
        ...
        let flat: Vec<(String, Vec<Declaration>, u32)> = s.rules.iter().take(4).map(style_rule).collect();
        assert_eq!(flat[0], (".a".into(), vec![decl("color", "red", false)], 0));
        assert_eq!(flat[1], (":is(.a):hover".into(), vec![decl("color", "blue", false)], 1));
        assert_eq!(flat[2], (":is(.a) .b".into(), vec![decl("x", "1", false)], 2));
        assert_eq!(flat[3], (":is(.a) > .c, :is(.a) + .d".into(), vec![decl("y", "2", false)], 3));
        match &s.rules[4] {
            Rule::Media { query, rules } => {
                assert_eq!(query.to_string(), "(hover)");
                assert_eq!(style_rule(&rules[0]), (".a".into(), vec![decl("z", "3", false)], 4));
                assert_eq!(style_rule(&rules[1]), (":is(.a) .e".into(), vec![decl("w", "4", false)], 5));
            }
            o => panic!("{o:?}"),
        }
        ...
        assert_eq!(style_rule(&s.rules[2]), (".p".into(), vec![decl("c", "3", false)], 2));
    ...
    fn media_nesting_and_layers_in_rules() {
        let s = sheet("@media screen { @media (hover) { a { x: 1 } } @supports (a: b) { b { y: 2 } } }");
        match &s.rules[0] {
            Rule::Media { rules, .. } => {
    ...
    fn declaration_block_for_style_attribute() {
        let d = parse_declaration_block("color: red; ; background : url( 'x.png' ) no-repeat !IMPORTANT; --My-Var: 1 ; bogus");
        assert_eq!(d.len(), 3);
