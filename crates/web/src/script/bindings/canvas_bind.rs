...
fn canvas_op(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    match op.as_str() {
        ...
        "fillText" | "strokeText" => {
            ...
            let max = if a.arg(5).is_undefined() {
                None
            } else {
                Some(f(vm, 3)?)
            };
        ...
        "drawImage" => {
            ...
            if let Some(img) = img {
                with_canvas(vm, n, |c| {
                    c.draw_image(
                        &img, vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6],
                        vals[7],
                    )
                });
            }
...
fn gradient_of(vm: &mut Vm, o: &Obj) -> JsResult<Option<(Paint, String)>> {
    ...
    let coords: Vec<f64> = array_values(vm, &coords)?
        .iter()
        .map(|v| match v {
            Value::Num(n) => *n,
            _ => 0.0,
...
fn canvas_put_image_data(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    let n = arg_node(vm, a, 0)?;
    let Value::Obj(data) = a.arg(1) else {
