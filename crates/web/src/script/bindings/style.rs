...
use super::{arg_node, arg_num, arg_str, dom_exception, handle_obj, inner, node_array, opt_node, str_array, string_val};
...
fn is_member(name: &str) -> bool {
    matches!(name, "length" | "cssText" | "parentRule" | "getPropertyValue" | "setProperty" | "removeProperty" | "getPropertyPriority" | "item" | "constructor" | "then" | "toString" | "valueOf" | "toJSON") || name.starts_with("__") || name.starts_with('%')
}
...
fn computed_get(vm: &mut Vm, o: &Obj, k: &Key) -> JsResult<Option<Value>> {
    ...
    if let Some(idx) = k.array_index() {
        return Ok(LonghandId::all().nth(idx as usize).map(|l| Value::str(l.name())));
    }
    ...
    let name = css_name(s);
    Ok(Some(string_val(computed_value(vm, NodeId(id), &pseudo, &name))))
}
fn computed_set(_vm: &mut Vm, _o: &Obj, k: &Key, _v: &Value) -> JsResult<Option<bool>> {
    Ok(if k.as_str().map(is_member).unwrap_or(true) { None } else { Some(false) })
}
fn computed_delete(_vm: &mut Vm, _o: &Obj, _k: &Key) -> JsResult<Option<bool>> {
    Ok(None)
}
fn computed_keys(_vm: &mut Vm, _o: &Obj) -> JsResult<Vec<Key>> {
    Ok((0..LonghandId::all().count()).map(|i| Key::str(&i.to_string())).collect())
}
pub static COMPUTED_HOOKS: HostHooks = HostHooks { class: "CSSStyleDeclaration", get: computed_get, set: computed_set, delete: computed_delete, keys: computed_keys };
...
fn shorthand_value(s: &crate::style::ComputedStyle, name: &str) -> Option<String> {
    let four = |a: &str, b: &str, c: &str, d: &str| -> Option<String> {
        let v: Vec<String> = [a, b, c, d].iter().map(|p| s.serialize(p).unwrap_or_default()).collect();
    ...
    match name {
        ...
        "padding" => four("padding-top", "padding-right", "padding-bottom", "padding-left"),
        "border-width" => four("border-top-width", "border-right-width", "border-bottom-width", "border-left-width"),
        "border-color" => four("border-top-color", "border-right-color", "border-bottom-color", "border-left-color"),
        "border-style" => four("border-top-style", "border-right-style", "border-bottom-style", "border-left-style"),
        "border-radius" => four("border-top-left-radius", "border-top-right-radius", "border-bottom-right-radius", "border-bottom-left-radius"),
        "inset" => four("top", "right", "bottom", "left"),
        "border" | "border-top" | "border-right" | "border-bottom" | "border-left" => {
            let side = if name == "border" { "top" } else { &name[7..] };
            Some(format!("{} {} {}", s.serialize(&format!("border-{side}-width"))?, s.serialize(&format!("border-{side}-style"))?, s.serialize(&format!("border-{side}-color"))?))
        }
        "font" => Some(format!("{} {}px {}", s.serialize("font-weight")?, s.font.size.to_f64_px(), s.serialize("font-family")?)),
        ...
        "flex" => Some(format!("{} {} {}", s.serialize("flex-grow")?, s.serialize("flex-shrink")?, s.serialize("flex-basis")?)),
        "flex-flow" => Some(format!("{} {}", s.serialize("flex-direction")?, s.serialize("flex-wrap")?)),
        "text-decoration" => s.serialize("text-decoration-line"),
        "list-style" => Some(format!("{} {}", s.serialize("list-style-type")?, s.serialize("list-style-position")?)),
        "transition" | "animation" | "grid-template" | "grid" | "place-items" | "place-content" | "columns" | "outline" => Some(String::new()),
        _ => None,
    }
}
...
fn computed_style(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    let proto = inner(vm).borrow().protos.get("CSSStyleDeclaration").cloned();
    Ok(Value::Obj(vm.host_obj(proto, &COMPUTED_HOOKS, vec![Value::Num(n.0 as f64), string_val(pseudo)])))
}
...
fn settable_property(name: &str) -> bool {
    name.starts_with("--")
        ...
        || matches!(name, "fill" | "stroke" | "stroke-width" | "fill-opacity" | "stroke-opacity" | "stroke-dasharray" | "stroke-dashoffset" | "stroke-linecap" | "stroke-linejoin" | "fill-rule" | "clip-rule" | "clip-path" | "mask" | "filter" | "backdrop-filter" | "will-change" | "touch-action" | "scroll-behavior" | "overscroll-behavior" | "text-rendering" | "shape-rendering" | "vector-effect" | "paint-order" | "stop-color" | "stop-opacity" | "marker" | "marker-start" | "marker-mid" | "marker-end" | "color-interpolation" | "dominant-baseline" | "text-anchor" | "alignment-baseline" | "transform-box" | "transform-origin" | "perspective" | "backface-visibility" | "mix-blend-mode" | "isolation" | "contain" | "content-visibility" | "aspect-ratio" | "appearance" | "resize" | "caret-color" | "accent-color" | "scrollbar-width" | "scrollbar-color" | "text-size-adjust" | "font-display" | "font-feature-settings" | "font-variant-numeric" | "font-variation-settings" | "text-underline-offset" | "text-decoration-thickness" | "text-decoration-skip-ink" | "hyphens" | "tab-size" | "column-count" | "column-gap" | "column-width" | "columns" | "break-inside" | "page-break-inside" | "page-break-after" | "page-break-before" | "user-select" | "pointer-events" | "image-rendering" | "object-position" | "translate" | "rotate" | "scale" | "offset" | "inset-inline" | "inset-block" | "margin-inline" | "margin-block" | "padding-inline" | "padding-block" | "border-inline" | "border-block" | "inline-size" | "block-size" | "min-inline-size" | "min-block-size" | "max-inline-size" | "max-block-size" | "place-self" | "justify-self" | "justify-items" | "grid-area" | "grid-row" | "grid-column" | "grid-template-areas" | "grid-auto-rows" | "grid-auto-columns" | "grid-auto-flow" | "box-decoration-break" | "text-wrap" | "white-space-collapse" | "text-spacing-trim" | "field-sizing" | "anchor-name" | "view-transition-name" | "container" | "container-type" | "container-name" | "zoom" | "outline" | "outline-color" | "outline-style" | "outline-width" | "outline-offset" | "quotes" | "counter-reset" | "counter-increment" | "unicode-bidi" | "writing-mode" | "text-orientation" | "ruby-position" | "speak" | "widows" | "orphans" | "all")
}
...
fn set_declaration(decls: &mut Vec<Declaration>, name: &str, value: &str, important: bool) -> bool {
    ...
    let parsed = parse_declaration_block(&format!("{name}: {value}{}", if important { " !important" } else { "" }));
    let Some(mut d) = parsed.into_iter().next() else { return false };
    if !settable_property(&d.name) {
        return false;
    }
    // Values the engine understands are validated; the rest are kept as written.
    if crate::style::cascade::is_known_property(&d.name) && !d.name.starts_with("--") && !crate::style::cascade::is_supported_declaration(&d.name, &d.value) {
        let text = component_values_to_string(&d.value).to_ascii_lowercase();
        if !matches!(text.as_str(), "inherit" | "initial" | "unset" | "revert") && !text.contains("var(") && !text.starts_with("calc(") && !text.starts_with("min(") && !text.starts_with("max(") && !text.starts_with("clamp(") {
            return false;
        }
    }
...
fn inline_get(vm: &mut Vm, o: &Obj, k: &Key) -> JsResult<Option<Value>> {
    ...
    if let Some(idx) = k.array_index() {
        return Ok(Some(decls.get(idx as usize).map(|d| Value::str(&d.name)).unwrap_or(Value::Undefined)));
    }
    let name = css_name(s);
...
fn inline_set(vm: &mut Vm, o: &Obj, k: &Key, v: &Value) -> JsResult<Option<bool>> {
    ...
    let value = if v.is_nullish() { String::new() } else { vm.to_string(v)?.to_string() };
...
pub static INLINE_HOOKS: HostHooks = HostHooks { class: "CSSStyleDeclaration", get: inline_get, set: inline_set, delete: inline_delete, keys: inline_keys };

pub fn set_inline_property(vm: &mut Vm, n: NodeId, name: &str, value: &str, important: bool) -> JsResult<()> {
    let text = {
        let rc = inner(vm);
        let mut i = rc.borrow_mut();
...
fn inline_style(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    let n = arg_node(vm, a, 0)?;
    let proto = inner(vm).borrow().protos.get("CSSStyleDeclaration").cloned();
    Ok(Value::Obj(vm.host_obj(proto, &INLINE_HOOKS, vec![Value::Num(n.0 as f64)])))
}
...
fn decl_op(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    let Value::Obj(o) = a.arg(0) else { return Ok(Value::Undefined) };
    let op = arg_str(vm, a, 1)?;
    let name = css_name(&arg_str(vm, a, 2)?);
    let hooks = o.host_hooks();
    let is_inline = hooks.map(|h| std::ptr::eq(h, &INLINE_HOOKS)).unwrap_or(false);
    let is_computed = hooks.map(|h| std::ptr::eq(h, &COMPUTED_HOOKS)).unwrap_or(false);
    let is_rule = hooks.map(|h| std::ptr::eq(h, &RULE_STYLE_HOOKS)).unwrap_or(false);
    if is_computed {
        ...
        return Ok(match op.as_str() {
            ...
            "cssText" => Value::str(""),
            "set" | "remove" | "setCssText" => return Err(dom_exception(vm, "NoModificationAllowedError", "These styles are computed, and therefore read-only.")),
            _ => Value::Undefined,
        });
    }
    ...
    let (mut decls, rule_ref) = if is_inline {
        ...
        let decls = rule_at(&i, sheet, &path).and_then(|r| match r {
            Rule::Style { declarations, .. } | Rule::Page { declarations, .. } | Rule::FontFace(declarations) => Some(declarations.clone()),
            _ => None,
        });
    ...
    match op.as_str() {
        ...
        "priority" => Ok(Value::str(if decls.iter().any(|d| d.name == name && d.important) { "important" } else { "" })),
        "length" => Ok(Value::Num(decls.len() as f64)),
        "item" => {
            let idx = arg_num(vm, a, 2)? as usize;
            Ok(Value::str(decls.get(idx).map(|d| d.name.as_str()).unwrap_or("")))
        }
        ...
        "set" | "remove" | "setCssText" => {
            ...
            if is_inline {
                ...
                let rc = inner(vm);
                let mut i = rc.borrow_mut();
                if let Some(Rule::Style { declarations, .. } | Rule::Page { declarations, .. } | Rule::FontFace(declarations)) = rule_at_mut(&mut i, sheet, &path) {
                    *declarations = decls;
                }
                i.sheet_changed();
            }
...
fn rule_ref_of(o: &Obj) -> (u32, Vec<usize>) {
    ...
    let path = match o.host_slot(1) {
        Some(Value::Str(p)) => p.split('/').filter(|s| !s.is_empty()).filter_map(|s| s.parse().ok()).collect(),
        _ => Vec::new(),
    };
    (sheet, path)
}
...
fn rules_at<'a>(rules: &'a [Rule], path: &[usize]) -> Option<&'a [Rule]> {
    ...
    for &p in path {
        cur = match cur.get(p)? {
            Rule::Media { rules, .. } | Rule::Supports { rules, .. } | Rule::Layer { rules, .. } => rules,
            _ => return None,
        };
    }
    Some(cur)
...
fn renumber(rules: &mut [Rule], next: &mut u32) {
    for r in rules {
        match r {
            ...
            Rule::Media { rules, .. } | Rule::Supports { rules, .. } | Rule::Layer { rules, .. } => renumber(rules, next),
            _ => {}
        }
    }
}
...
fn supports_text(c: &SupportsCondition) -> String {
    match c {
        ...
        SupportsCondition::And(v) => v.iter().map(supports_text).collect::<Vec<_>>().join(" and "),
        SupportsCondition::Or(v) => v.iter().map(supports_text).collect::<Vec<_>>().join(" or "),
        SupportsCondition::Declaration { property, value } => format!("({property}: {})", component_values_to_string(value)),
...
pub fn rule_css_text(r: &Rule) -> String {
    match r {
        Rule::Style { selectors, declarations, .. } => format!("{selectors} {{ {}{}}}", serialize_declarations(declarations), if declarations.is_empty() { "" } else { " " }),
        Rule::Media { query, rules } => format!("@media {query} {{\n{}}}", rules.iter().map(|r| format!("  {}\n", rule_css_text(r))).collect::<String>()),
        Rule::Import { url, media } => {
            let m = media.to_string();
            format!("@import url(\"{url}\"){}{};", if m.is_empty() { "" } else { " " }, m)
        }
        Rule::FontFace(d) => format!("@font-face {{ {} }}", serialize_declarations(d)),
        Rule::Keyframes { name, frames } => format!("@keyframes {name} {{ {}}}", frames.iter().map(|(sel, d)| format!("{} {{ {} }} ", sel.iter().map(keyframe_selector).collect::<Vec<_>>().join(", "), serialize_declarations(d))).collect::<String>()),
        Rule::Supports { condition, rules } => format!("@supports {} {{\n{}}}", supports_text(condition), rules.iter().map(|r| format!("  {}\n", rule_css_text(r))).collect::<String>()),
        Rule::Layer { names, rules } => {
            if rules.is_empty() {
                format!("@layer {};", names.join(", "))
            } else {
                format!("@layer {} {{\n{}}}", names.join(", "), rules.iter().map(|r| format!("  {}\n", rule_css_text(r))).collect::<String>())
            }
        }
        Rule::Page { selectors, declarations } => format!("@page {}{{ {} }}", if selectors.is_empty() { String::new() } else { format!("{} ", selectors.join(", ")) }, serialize_declarations(declarations)),
        Rule::Namespace { prefix, url } => format!("@namespace {}url(\"{url}\");", prefix.as_ref().map(|p| format!("{p} ")).unwrap_or_default()),
        Rule::Unknown { name, prelude, block } => format!("@{name} {}{}", component_values_to_string(prelude).trim(), match block {
...
fn rule_style_get(vm: &mut Vm, o: &Obj, k: &Key) -> JsResult<Option<Value>> {
    ...
    let decls = match rule_at(&i, sheet, &path) {
        Some(Rule::Style { declarations, .. }) | Some(Rule::Page { declarations, .. }) | Some(Rule::FontFace(declarations)) => declarations,
        _ => return Ok(None),
    };
    if let Some(idx) = k.array_index() {
        return Ok(Some(decls.get(idx as usize).map(|d| Value::str(&d.name)).unwrap_or(Value::Undefined)));
...
fn rule_style_set(vm: &mut Vm, o: &Obj, k: &Key, v: &Value) -> JsResult<Option<bool>> {
    ...
    let changed = match rule_at_mut(&mut i, sheet, &path) {
        Some(Rule::Style { declarations, .. }) | Some(Rule::Page { declarations, .. }) | Some(Rule::FontFace(declarations)) => set_declaration(declarations, &name, &value, false),
        _ => false,
    };
    if changed {
        i.sheet_changed();
...
pub static RULE_STYLE_HOOKS: HostHooks = HostHooks { class: "CSSStyleDeclaration", get: rule_style_get, set: rule_style_set, delete: inline_delete, keys: rule_style_keys };
    ...
    match op.as_str() {
        ...
        "replace" => {
            ...
            if let Some(e) = i.sheet_mut(id) {
                e.sheet = parse_stylesheet(&text, Origin::Author, Strictness::Lenient).unwrap_or_default();
                e.source = text;
            }
            i.sheet_changed();
            Ok(Value::Undefined)
        ...
        "info" => {
            ...
            let (href, disabled, media, owner, ctor) = {
                let i = rc.borrow();
                match i.sheet(id) {
                    Some(e) => (e.href.clone(), e.disabled, e.media.clone(), match e.owner {
            ...
            let o = cw_jsvm::builtins::new_obj_from(vm, vec![("href", href.map(string_val).unwrap_or(Value::Null)), ("disabled", Value::Bool(disabled)), ("media", string_val(media)), ("ownerNode", owner), ("constructed", Value::Bool(ctor))]);
            Ok(Value::Obj(o))
        }
        "setDisabled" => {
        ...
        "count" => {
            ...
            let rc = inner(vm);
            let i = rc.borrow();
            let n = i.sheet(id).and_then(|e| rules_at(&e.sheet.rules, &path)).map(|r| r.len()).unwrap_or(0);
        ...
        "rule" => {
                ...
                match rule_at(&i, id, &path) {
                    Some(r) => {
                        let (sel, cond, name) = match r {
                            Rule::Style { selectors, .. } => (selectors.to_string(), String::new(), String::new()),
                            Rule::Media { query, .. } => (String::new(), query.to_string(), String::new()),
                            Rule::Supports { condition, .. } => (String::new(), supports_text(condition), String::new()),
                            Rule::Keyframes { name, .. } => (String::new(), String::new(), name.clone()),
                            Rule::Layer { names, .. } => (String::new(), String::new(), names.join(", ")),
                            Rule::Import { url, media } => (String::new(), media.to_string(), url.clone()),
            ...
            let Some((ty, text, sel, cond, name)) = info else { return Ok(Value::Null) };
            let p: Vec<String> = path.iter().map(|x| x.to_string()).collect();
            let o = cw_jsvm::builtins::new_obj_from(vm, vec![("type", Value::Num(ty as f64)), ("cssText", string_val(text)), ("selectorText", string_val(sel)), ("conditionText", string_val(cond)), ("name", string_val(name)), ("path", string_val(p.join("/")))]);
            Ok(Value::Obj(o))
        }
        "ruleStyle" => {
            let path = path_arg(vm, a, 2)?;
            let proto = inner(vm).borrow().protos.get("CSSStyleDeclaration").cloned();
            let p: Vec<String> = path.iter().map(|x| x.to_string()).collect();
            Ok(Value::Obj(vm.host_obj(proto, &RULE_STYLE_HOOKS, vec![Value::Num(id as f64), string_val(p.join("/"))])))
        }
        "insert" => {
            let path = path_arg(vm, a, 2)?;
            let text = arg_str(vm, a, 3)?;
            let idx = arg_num(vm, a, 4)? as usize;
            let parsed = parse_stylesheet(&text, Origin::Author, Strictness::Lenient).unwrap_or_default();
            let mut rules = parsed.rules;
            if rules.len() != 1 {
                return Err(dom_exception(vm, "SyntaxError", &format!("Failed to parse the rule '{text}'.")));
            }
            let rule = rules.remove(0);
            let rc = inner(vm);
            let mut i = rc.borrow_mut();
            let ok = {
                let Some(e) = i.sheet_mut(id) else { return Ok(Value::Undefined) };
            ...
            if !ok {
                drop(i);
                return Err(dom_exception(vm, "IndexSizeError", "The index provided is larger than the maximum index."));
            }
            i.sheet_changed();
        ...
        "keyframes" => {
            ...
            let frames: Vec<String> = match rule_at(&i, id, &path) {
                Some(Rule::Keyframes { frames, .. }) => frames.iter().map(|(sel, d)| format!("{} {{ {} }}", sel.iter().map(keyframe_selector).collect::<Vec<_>>().join(", "), serialize_declarations(d))).collect(),
                _ => Vec::new(),
            };
            drop(i);
...
fn path_arg(vm: &mut Vm, a: &Args, i: usize) -> JsResult<Vec<usize>> {
    ...
    Ok(s.split('/').filter(|x| !x.is_empty()).filter_map(|x| x.parse().ok()).collect())
}
...
fn set_adopted(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    let ids: Vec<u32> = items.iter().filter_map(|v| match v {
...
fn css_supports(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    if a.len() >= 2 {
        ...
        return Ok(Value::Bool(crate::style::cascade::is_supported_declaration(&first, &values)));
    }
    ...
    let ok = match crate::css::media::parse_supports_condition(&values) {
        ...
        None => {
            ...
            let inner_vals = match values.first() {
                Some(crate::css::ComponentValue::Block { contents, .. }) if values.len() == 1 => contents.clone(),
                _ => values.clone(),
            };
            let text = component_values_to_string(&inner_vals);
            match text.split_once(':') {
                Some((p, v)) => crate::style::cascade::is_supported_declaration(p.trim(), &crate::css::parser::parse_component_value_list(v.trim())),
                None => false,
            }
        }
    };
...
fn css_escape(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    for (idx, c) in s.chars().enumerate() {
        ...
        if code == 0 {
            out.push('\u{FFFD}');
        } else if (1..=0x1F).contains(&code) || code == 0x7F || (idx == 0 && c.is_ascii_digit()) || (idx == 1 && c.is_ascii_digit() && s.starts_with('-')) {
...
fn media_matches(vm: &mut Vm, a: &mut Args) -> JsResult<Value> {
    ...
    let m = i.media();
    Ok(vm.arr(vec![Value::Bool(list.evaluate(&m)), string_val(list.to_string())]))
}
...
fn style_elements(vm: &mut Vm, _a: &mut Args) -> JsResult<Value> {
    let nodes: Vec<NodeId> = {
        let rc = inner(vm);
        let i = rc.borrow();
        i.doc.descendants(crate::dom::Document::ROOT).filter(|n| i.doc.is(*n, "style") || i.doc.is(*n, "link")).collect()
    };
