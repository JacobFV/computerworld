...
fn host() -> MemoryHost {
    MemoryHost::new()
        .with_response("https://example.test/vendor/jquery-3.7.1.min.js", "text/javascript", &vendor("jquery-3.7.1.min.js"))
        .with_response("https://example.test/vendor/preact-10.19.3.umd.js", "text/javascript", &vendor("preact-10.19.3.umd.js"))
        .with_response("https://example.test/vendor/preact-hooks-10.19.3.umd.js", "text/javascript", &vendor("preact-hooks-10.19.3.umd.js"))
        .with_response("https://example.test/api/items", "application/json", "{\"items\":[1,2,3]}")
...
fn click(r: &mut Realm, node: NodeId) -> DefaultAction {
    r.dispatch(UiEvent::ClickNode { node, modifiers: Modifiers::default(), detail: 1 })
}

fn text_of(r: &mut Realm, id: &str) -> String {
    r.eval(&format!("document.getElementById('{id}').textContent")).unwrap()
}

#[test]
fn vanilla_tabs_accordion_menu() {
    let mut r = Realm::new(&fixture("tabs.html"), "https://example.test/tabs.html", Box::new(host()));
    r.run_document();
    ...
    assert!(!two.is_empty());
    let two_btn = r.document().descendants(id(&r, "tabs")).find(|n| r.document().attr(*n, "data-tab") == Some("two")).unwrap();
    ...
    let head2 = r.document().descendants(id(&r, "acc2")).find(|n| r.document().is(*n, "h3")).unwrap();
    click(&mut r, head2);
    ...
    let menu_rect = r.eval("JSON.stringify(menu.getBoundingClientRect())").unwrap();
    assert!(menu_rect.contains("\"width\":80"), "{menu_rect}");
    let (mx, my) = {
        let rect = r.eval("const q = menu.getBoundingClientRect(); [q.left + 5, q.top + 5].join()").unwrap();
        let mut it = rect.split(',').map(|v| v.parse::<f64>().unwrap() as i32);
        (it.next().unwrap(), it.next().unwrap())
    };
    r.dispatch(UiEvent::PointerMove { x: mx, y: my, modifiers: Modifiers::default() });
    assert_eq!(text_of(&mut r, "status"), "menu open block");
    assert_eq!(r.eval("getComputedStyle(document.getElementById('menu-list')).display").unwrap(), "block");
    let action = { let n = id(&r, "link-a"); click(&mut r, n) };
    assert_eq!(action, DefaultAction::Navigate("https://example.test/a".into()));
    r.dispatch(UiEvent::PointerMove { x: 600, y: 700, modifiers: Modifiers::default() });
    assert_eq!(text_of(&mut r, "status"), "menu closed");
    r.dispatch(UiEvent::Key { key: "Escape".into(), code: String::new(), modifiers: Modifiers::default(), repeat: false });
    assert_eq!(text_of(&mut r, "status"), "escaped");
}

#[test]
fn jquery_page_runs_unmodified() {
    let mut r = Realm::new(&fixture("jquery.html"), "https://example.test/jq.html", Box::new(host()));
    r.run_document();
    assert!(errors(&r).is_empty(), "{}", errors(&r));
    r.run_until_idle(100);
    assert!(errors(&r).is_empty(), "{}", errors(&r));
    assert_eq!(r.eval("typeof jQuery + ' ' + $.fn.jquery").unwrap(), "function 3.7.1");
    ...
    assert_eq!(r.eval("window.each + ' ' + window.filtered").unwrap(), "0o1t2t3f two|three");
    ...
    assert_eq!(r.eval("$('#ajax').text() + ' ' + window.gotText").unwrap(), "1,2,3 plain text");
    assert_eq!(r.eval("window.animDone + ' ' + window.boxAfterFade").unwrap(), "true none");
    // Click handlers bound through jQuery see the realm's trusted events.
    let li = r.document().descendants(id(&r, "list")).filter(|n| r.document().is(*n, "li")).nth(1).unwrap();
    click(&mut r, li);
    ...
    assert_eq!({ let n = id(&r, "go"); click(&mut r, n) }, DefaultAction::Prevented);
    assert_eq!(text_of(&mut r, "out"), "submitted q=changed&s=y");
}

#[test]
fn preact_counter_with_hooks_and_keyed_list() {
    let mut r = Realm::new(&fixture("preact.html"), "https://example.test/preact.html", Box::new(host()));
    r.run_document();
    assert!(errors(&r).is_empty(), "{}", errors(&r));
    r.run_until_idle(50);
    assert!(errors(&r).is_empty(), "{}", errors(&r));
    ...
    { let n = id(&r, "dec"); click(&mut r, n) };
    r.run_until_idle(50);
    assert_eq!(r.eval("document.querySelector('h1').textContent + '|' + !!document.getElementById('big')").unwrap(), "Count: 2|false");
    // Keyed list: removing the middle item keeps the other DOM nodes.
    let first_before = r.eval("document.querySelector('#list li').dataset.id").unwrap();
    let li_b = r.document().descendants(id(&r, "list")).find(|n| r.document().attr(*n, "data-id") == Some("2")).unwrap();
    let li_a = r.document().descendants(id(&r, "list")).find(|n| r.document().attr(*n, "data-id") == Some("1")).unwrap();
    click(&mut r, li_b);
    r.run_until_idle(50);
    ...
    assert_eq!(r.document().descendants(id(&r, "list")).find(|n| r.document().attr(*n, "data-id") == Some("1")), Some(li_a));
    { let n = id(&r, "add"); click(&mut r, n) };
    r.run_until_idle(50);
    assert_eq!(r.eval("[...document.querySelectorAll('#list li')].map(l => l.textContent).join()").unwrap(), "new2,a,c");
    // Controlled input through typing.
    r.dispatch(UiEvent::Focus { node: Some(id(&r, "text")) });
