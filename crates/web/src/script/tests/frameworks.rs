    ...
    fn add(dir: &std::path::Path, prefix: &str, mut h: MemoryHost) -> MemoryHost {
        ...
        for p in entries {
            ...
            if p.is_dir() {
                ...
                let ty = if name.ends_with(".css") { "text/css" } else if name.ends_with(".json") { "application/json" } else { "text/javascript" };
...
fn id(r: &Realm, id: &str) -> NodeId {
    let d = r.document();
    d.by_id(id).iter().copied().find(|n| d.ancestors(*n).any(|a| a == crate::dom::Document::ROOT)).unwrap_or_else(|| panic!("no connected #{id}"))
}

fn click(r: &mut Realm, el: &str) -> DefaultAction {
    let node = id(r, el);
    let a = r.dispatch(UiEvent::ClickNode { node, modifiers: Modifiers::default(), detail: 1 });
    r.run_until_idle(20);
    a
}
...
fn by_class(r: &Realm, parent: &str, class: &str) -> NodeId {
    ...
    let d = r.document();
    d.descendants(p)
        .find(|n| d.attr(*n, "class").map(|c| c.split_ascii_whitespace().any(|x| x == class)).unwrap_or(false))
...
fn key(r: &mut Realm, k: &str) {
    r.dispatch(UiEvent::Key { key: k.into(), code: String::new(), modifiers: Modifiers::default(), repeat: false });
    r.run_until_idle(20);
}

fn set_value(r: &mut Realm, el: &str, value: &str) {
    let node = id(r, el);
    r.dispatch(UiEvent::SetValue { node, value: value.into(), commit: true });
    r.run_until_idle(20);
}
...
fn computed(r: &mut Realm, el: &str, prop: &str) -> String {
    js(r, &format!("getComputedStyle(document.getElementById('{el}')).getPropertyValue('{prop}')"))
}
...
fn painted_fill(r: &mut Realm, el: &str) -> String {
    let node = id(r, el);
    let i = r.layout();
    let scene = crate::paint::paint(&i.doc, &i.styles, i.tree.as_ref().unwrap(), i.viewport, &crate::paint::PaintContext::default());
    let fill = scene.nodes.iter().find(|n| (n.id >> 28) as u32 == node.0 && (n.id & 0xFFFF) as u32 == 1).map(|n| match n.primitive {
        cw_scene::Primitive::Box { fill, .. } | cw_scene::Primitive::RoundedBox { fill, .. } => fill,
...
fn all_logs(r: &Realm) -> String {
    r.logs().iter().map(|l| format!("[{:?}] {}", l.level, l.text)).collect::<Vec<_>>().join("\n")
}

fn assert_clean(r: &Realm) {
    assert!(errors(r).is_empty(), "console errors/warnings:\n{}", errors(r));
}
...
#[ignore]
fn scratch_page() {
    let Ok(path) = std::env::var("M4_PAGE") else { return };
    let html = std::fs::read_to_string(&path).unwrap();
    let t = std::time::Instant::now();
    let mut r = Realm::new(&html, &format!("{BASE}scratch.html"), Box::new(host()));
    r.run_document();
    r.run_until_idle(std::env::var("M4_MS").ok().and_then(|v| v.parse().ok()).unwrap_or(50));
    ...
    if let Ok(steps) = std::env::var("M4_STEPS") {
        for line in std::fs::read_to_string(steps).unwrap().lines() {
            ...
            match cmd {
                ...
                "hover" => hover(&mut r, a),
                "idle" => { r.run_until_idle(a.parse().unwrap()); }
...
fn react18_create_root_hooks_events_and_concurrent_features() {
    ...
    assert_eq!(js(&mut r, "trace.filter((t) => t !== 'T-raf0').join()"), "layout:0:Count: 0,T-layout:0,T-micro,effect:0,T-passive:0");
    assert_eq!(js(&mut r, "String(trace.indexOf('T-raf0') > trace.indexOf('T-micro'))"), "true");
    ...
    assert_eq!(js(&mut r, "lbl.htmlFor + '|' + document.querySelector('[data-useid]').id"), ":r0:|:r0:");
    ...
    assert_eq!(js(&mut r, "trace.join()"), "layout-cleanup:0,layout:1:Count: 1,cleanup:0,effect:1");
    ...
    let lis = |r: &Realm| -> Vec<(String, NodeId)> {
        let d = r.document();
        d.descendants(id(r, "list")).filter(|n| d.is(*n, "li")).map(|n| (d.attr(n, "data-k").unwrap().to_owned(), n)).collect()
    };
    let first = lis(&r);
    click(&mut r, "reverse");
    let reversed = lis(&r);
    assert_eq!(reversed.iter().map(|(k, _)| k.as_str()).collect::<Vec<_>>(), ["c", "b", "a"]);
    assert_eq!(reversed.iter().rev().cloned().collect::<Vec<_>>(), first);
    click(&mut r, "prepend");
    click(&mut r, "drop");
    let after = lis(&r);
    assert_eq!(after.iter().map(|(k, _)| k.as_str()).collect::<Vec<_>>(), ["z", "c", "a"]);
    ...
    assert_eq!(click(&mut r, "agree"), DefaultAction::Toggle(id(&r, "agree")));
    ...
    assert_eq!(text(&mut r, "form-out"), "HELL|true|blue|n:xyz|two|helloBackspace");
    ...
    assert_eq!(js(&mut r, "trace.join()"), "focus:name,blur:name,focus:name,submit:HELL,blur:name,submit:HELL");
    ...
    click(&mut r, "open");
    assert_eq!(js(&mut r, "document.getElementById('portal-target').innerHTML + bubbled.textContent"), "<div id=\"dialog\" role=\"dialog\"><button id=\"in-portal\">inside</button></div>1");
    click(&mut r, "in-portal");
    assert_eq!(text(&mut r, "bubbled"), "2");
    click(&mut r, "open");
    assert_eq!(js(&mut r, "document.getElementById('portal-target').innerHTML + bubbled.textContent"), "3");
    ...
    assert_eq!(js(&mut r, "raw.innerHTML"), "<b id=\"bold\">bold</b> &amp; <i>it</i>");
    ...
    click(&mut r, "timing");
    r.run_until_idle(50);
    assert_eq!(js(&mut r, "trace.slice(0, 3).join() + '|' + trace.slice(3).sort().join()"), "T-layout:1,T-passive:1,T-micro|T-raf,T-timeout".replace("T-micro", "T-microtask"));
    assert_clean(&r);
    ...
    js(&mut r, "trace.length = 0");
    click(&mut r, "bomb");
    assert_eq!(js(&mut r, "document.getElementById('fallback').textContent + ' ' + trace.join()"), "Error: boom caught:boom:string");
    let errs = errors(&r);
...
fn react17_legacy_render_class_components_and_forms() {
    ...
    assert_eq!(js(&mut r, "syncChildren + ' ' + trace.join() + ' ' + React.version"), "1 mount,rendered,items:1 17.0.2");
    r.run_until_idle(250);
    assert!(text(&mut r, "ticks").starts_with("ticks: "), "{}", text(&mut r, "ticks"));
    ...
    assert_eq!(js(&mut r, "!!document.getElementById('ticks') + ' ' + trace[trace.length - 1]"), "false unmount");
    assert_clean(&r);
}
...
fn vue3_global_build_compiles_templates_and_drives_the_dom() {
    ...
    assert_eq!(js(&mut r, "Vue.version + '|' + typeof Vue.compile"), "3.4.38|function");
    ...
    type_text(&mut r, "draft", "milk");
    assert_eq!(js(&mut r, "[draft.value, add.disabled, vm.draft].join()"), "milk,false,milk");
    key(&mut r, "Enter");
    assert_eq!(js(&mut r, "[...document.querySelectorAll('#todos li')].map(l => l.id).join()"), "todo-1,todo-2,todo-3");
    ...
    assert_eq!(js(&mut r, "[draft.value, add.disabled, !!document.getElementById('many')].join()"), ",true,true");

    // A keyed `v-for` reordered in place keeps the same DOM nodes.
    let lis = |r: &Realm| -> Vec<(String, NodeId)> {
        let d = r.document();
        d.descendants(id(r, "todos")).filter(|n| d.is(*n, "li")).map(|n| (d.attr(n, "id").unwrap().to_owned(), n)).collect()
    };
    let before = lis(&r);
    click(&mut r, "reverse");
    let after = lis(&r);
    assert_eq!(after.iter().map(|(k, _)| k.as_str()).collect::<Vec<_>>(), ["todo-3", "todo-2", "todo-1"]);
    assert_eq!(after.iter().rev().cloned().collect::<Vec<_>>(), before);
    assert_eq!(js(&mut r, "[...document.querySelectorAll('#todos li')].map(l => l.dataset.index).join()"), "0,1,2");

    // Every `v-model` flavour, driven through the realm's own events.
    assert_eq!(click(&mut r, "agree"), DefaultAction::Toggle(id(&r, "agree")));
    assert_eq!(js(&mut r, "[getComputedStyle(shown).display, shown.className].join('|')"), "block|box active");
    set_value(&mut r, "color", "blue");
    ...
    assert_eq!(js(&mut r, "document.getElementById('modals').innerHTML"), "<div id=\"modal\">Teleported: Todos</div>");
    click(&mut r, "open-modal");
    assert_eq!(js(&mut r, "document.getElementById('modals').innerHTML"), "<!---->");
    ...
    assert_eq!(js(&mut r, "document.getElementById('fading').className"), "fade-leave-from fade-leave-active");
    ...
    assert_eq!(js(&mut r, "[!!document.getElementById('fading'), trace.slice(-1)[0], tevents.join()].join(' ~ ')"), "false ~ after-leave ~ fading:opacity:0.3");
    click(&mut r, "toggle-fade");
    r.run_until_idle(50);
    assert_eq!(js(&mut r, "document.getElementById('fading').className"), "fade-enter-active fade-enter-to");
    r.run_until_idle(500);
    ...
    assert_eq!(js(&mut r, "[document.getElementById('todo-1').className, title.textContent].join('|')"), "done|Todos (1 left)");
    ...
    assert_eq!(js(&mut r, "document.getElementById('watch-log').textContent"), "1>2,2>1");
    assert_clean(&r);
}
...
fn styled_components_6_inserts_rules_at_runtime_and_the_cascade_follows() {
    ...
    assert_eq!(computed(&mut r, "box", "background-color"), "rgb(128, 0, 0)");
...
fn emotion_11_inserts_rules_at_runtime_and_the_cascade_follows() {
    ...
    assert_eq!(js(&mut r, "[...document.styleSheets].map((s) => s.ownerNode.getAttribute('data-emotion')).join()"), "css,css-global");
    ...
    click(&mut r, "toggle-box");
    assert_eq!(js(&mut r, "box.className + '|' + getComputedStyle(box).backgroundColor"), "css-1h4ce1a|rgb(0, 128, 0)");
    assert_eq!(painted_fill(&mut r, "box"), "0,128,0,255");
...
fn ids(r: &mut Realm, sel: &str) -> String {
    js(r, &format!("[...document.querySelectorAll('{sel}')].map((n) => n.id).join('|')"))
}
...
fn svelte4_compiled_components_run_stores_events_and_transitions() {
    ...
    assert_eq!(js(&mut r, "trace.join()"), "remaining:2,todos:before-update,counter:mount,todos:mount,todos:after-update");
    ...
    assert_eq!(ids(&mut r, "#sv-list li"), "sv-item-1|sv-item-2|sv-item-3|sv-item-4");
    ...
    key(&mut r, "Escape");
    assert_eq!(js(&mut r, "[document.getElementById('sv-new-input').value, trace.includes('escape')].join(' ~ ')"), " ~ true");
    ...
    let before: Vec<NodeId> = {
        let d = r.document();
        d.descendants(id(&r, "sv-list")).filter(|n| d.is(*n, "li")).collect()
    };
    ...
    assert_eq!(ids(&mut r, "#sv-list li"), "sv-item-4|sv-item-3|sv-item-2|sv-item-1");
    ...
    assert_eq!(js(&mut r, "trace.filter((t) => t.startsWith('tick:')).join()"), "tick:0->2,tick:2->4,tick:4->6,tick:6->8,tick:8->10");
    assert_eq!(js(&mut r, "trace.filter((t) => t.includes('milestone')).join()"), "todos:milestone:10,counter:milestone:10");
    click(&mut r, "sv-dec");
    ...
    assert_eq!(js(&mut r, "trace.slice(-4).join()"), "remove:4,remaining:2,todos:before-update,todos:after-update");
    ...
    assert!(rule.starts_with("@keyframes K0 { 0% { opacity: 0; }") && rule.ends_with("100% { opacity: 1; } }"), "{rule}");
    ...
    click(&mut r, "sv-note-toggle");
    assert_eq!(js(&mut r, "[!!document.getElementById('sv-note'), document.styleSheets.length].join(' ~ ')"), "true ~ 1");
    r.run_until_idle(400);
    assert_eq!(js(&mut r, "[!!document.getElementById('sv-note'), document.styleSheets.length].join(' ~ ')"), "false ~ 0");
    assert_clean(&r);
}
...
fn probe_page(name: &str) -> (Realm, Vec<String>) {
    ...
    let marker = "<head>";
    assert!(html.contains(marker), "{name}: no <head> to inject the probe into");
    ...
    let probed = if probed.is_empty() { Vec::new() } else { probed.split(',').map(str::to_owned).collect() };
    (r, probed)
}
...
fn every_bundle_loads_with_a_clean_console_and_a_known_set_of_feature_misses() {
    ...
    for (name, _) in expected {
        ...
        assert!(logs.is_empty(), "{name} wrote to the console at load:\n{logs}");
        misses.push(((*name).to_owned(), probed.join(",")));
    }
    eprintln!("{report}");
    let got: Vec<(&str, &str)> = misses.iter().map(|(a, b)| (a.as_str(), b.as_str())).collect();
    assert_eq!(got, expected.to_vec(), "the feature detections that come up empty changed");
}
