...
fn linear_is_filtered_and_an_issue_is_opened_commented_and_moved() {
    ...
    for key in ["ATL", "INF", "ONB", "OPS", "REN", "WEB"] {
        assert_eq!(link_to(&home, &format!("nav-{key}")), format!("http://linear.app/projects/{key}"));
    }
    assert_eq!(link_to(&home, "team-ATL"), "http://linear.app/projects/ATL");
    assert!(body_text(&home).contains("6 unfinished"), "no open count for Atlas");
    ...
    assert!(list.iter().any(|e| e["kind"] == "heading" && e["text"] == "Atlas"));
    assert_eq!(link_to(&list, "view-board"), "http://linear.app/projects/ATL?view=board");
    assert_eq!(link_to(&list, "view-cycle"), "http://linear.app/projects/ATL?view=cycle");
    assert_eq!(text(&list, "card-open-1"), "Sort refs before iterating in BFS");
    assert_eq!(link_to(&list, "card-open-1"), "http://linear.app/projects/ATL/issues/1");
    ...
    assert_eq!(url(&world, &session), "http://linear.app/projects/ATL?assignee=bob");
    ...
    assert!(!has(&mine, "card-open-1"), "alice's ATL-1 survived the filter");
    // The filter is kept when the view changes.
    assert_eq!(link_to(&mine, "view-board"), "http://linear.app/projects/ATL?assignee=bob&view=board");
    assert_eq!(link_to(&mine, "filter-all"), "http://linear.app/projects/ATL");

    // Open one of them: the properties rail, the activity feed, the composer.
    click(&mut world, &session, "card-open-4");
    assert_eq!(url(&world, &session), "http://linear.app/projects/ATL/issues/4");
    assert_eq!(title(&world, &session), "ATL-4 Windows matrix in CI");
    let issue = all(&world, &session);
    assert!(issue.iter().any(|e| e["kind"] == "heading" && e["text"] == "Windows matrix in CI"));
    assert_eq!(link_to(&issue, "back"), "http://linear.app/projects/ATL");
    assert_eq!(link_to(&issue, "issue-project"), "http://linear.app/projects/ATL");
    ...
    act(&mut world, &session, "browser.v1", "fill", json!({"id":"comment-body","value":"Runner is green on the Windows image now."}));
    ...
    assert!(!has(&moved, "status-in_progress"), "In Progress is still offered after the move");
    assert!(has(&moved, "status-open"), "Todo is not offered after the move");
    ...
    assert!(!has(&list, "move-4-in_progress"), "ATL-4 is still listed as Todo");
    assert!(has(&list, "move-4-open") && has(&list, "move-4-blocked"));

    // A new issue is created from the composer at the foot of the list.
    act(&mut world, &session, "browser.v1", "fill", json!({"id":"new-issue-title","value":"Pin the Windows image"}));
    act(&mut world, &session, "browser.v1", "fill", json!({"id":"new-issue-body","value":"Float is how we got here."}));
...
fn the_internal_tracker_stays_plain_and_keeps_its_ids() {
    ...
    let home = all(&world, &session);
    assert!(!has(&home, "rail") && !has(&home, "workspace"), "the plain tracker grew a sidebar");
    assert_eq!(link_to(&home, "project-OPS"), "http://issues.internal/projects/OPS");
    ...
    assert!(project.iter().any(|e| e["kind"] == "heading" && e["text"] == "Operations"));
    assert_eq!(link_to(&project, "issue-1"), "http://issues.internal/projects/OPS/issues/1");
    ...
    assert_eq!(link_to(&issue, "back"), "http://issues.internal/projects/OPS");
    ...
    act(&mut world, &session, "browser.v1", "fill", json!({"id":"comment-body","value":"Warmer shipped."}));
    click(&mut world, &session, "comment-submit");
