...
fn northwind_accounts_are_read_filtered_and_a_transfer_is_made_through_the_agent_api() {
    ...
    browser_do(&mut world, &session, "fill", json!({"id":"filter-q","value":"payroll"}));
    ...
    browser_do(&mut world, &session, "navigate", json!({"url":"http://northwind.example/accounts/cc-3310"}));
    browser_do(&mut world, &session, "click", json!({"id":"t-tx-1"}));
    assert_eq!(url(&world, &session), "http://northwind.example/accounts/cc-3310/transactions/tx-1");
    let (_, all) = page(&world, &session);
    assert!(shows(&all, "-$429.99"));
    assert_eq!(by_id(&all, "origin")["url"], "http://amazon.com/orders/1001");
    browser_do(&mut world, &session, "click", json!({"id":"back"}));
    assert_eq!(url(&world, &session), "http://northwind.example/accounts/cc-3310");
    ...
    let (_, all) = page(&world, &session);
    for (id, label) in [("xfer-from", "From account id"), ("xfer-to", "To account id"), ("xfer-amount", "Amount in cents")] {
    ...
    browser_do(&mut world, &session, "fill", json!({"id":"xfer-from","value":"chk-4417"}));
    browser_do(&mut world, &session, "fill", json!({"id":"xfer-to","value":"sav-9902"}));
    browser_do(&mut world, &session, "fill", json!({"id":"xfer-amount","value":"12455"}));
    browser_do(&mut world, &session, "click", json!({"id":"xfer-go"}));
    let (title, all) = page(&world, &session);
    assert!(title.starts_with("Everyday Checking"), "{title}");
    assert!(shows(&all, "$8,000.00"), "the debited balance is on the page that comes back");
    browser_do(&mut world, &session, "navigate", json!({"url":"http://northwind.example/"}));
    let (_, all) = page(&world, &session);
    assert!(shows(&all, "$14,624.55"), "the savings tile shows the credited leg");

    // A bill is paid from the second form, and a new payee is added from the third.
    browser_do(&mut world, &session, "navigate", json!({"url":"http://northwind.example/transfers"}));
    browser_do(&mut world, &session, "fill", json!({"id":"pay-account","value":"chk-4417"}));
    browser_do(&mut world, &session, "fill", json!({"id":"pay-payee","value":"cascade-power"}));
    browser_do(&mut world, &session, "fill", json!({"id":"pay-amount","value":"10000"}));
    browser_do(&mut world, &session, "click", json!({"id":"pay-go"}));
    let (_, all) = page(&world, &session);
    assert!(shows(&all, "$7,900.00"));
    assert!(shows(&all, "Cascade Power"));
    browser_do(&mut world, &session, "navigate", json!({"url":"http://northwind.example/transfers"}));
    browser_do(&mut world, &session, "fill", json!({"id":"addpayee-name","value":"Rainier Fibre"}));
    browser_do(&mut world, &session, "fill", json!({"id":"addpayee-hint","value":"...7710"}));
...
fn paypal_shows_the_balance_and_activity_and_sends_a_payment_through_the_agent_api() {
    let (mut world, session) = world();
    browser_do(&mut world, &session, "navigate", json!({"url":"http://paypal.com/"}));
    let (title, all) = page(&world, &session);
    ...
    assert_eq!(by_id(&all, "a-pp-alice")["url"], "http://paypal.com/accounts/pp-alice");
    ...
    browser_do(&mut world, &session, "fill", json!({"id":"pay-account","value":"pp-alice"}));
    browser_do(&mut world, &session, "fill", json!({"id":"pay-payee","value":"bob-tanaka"}));
    browser_do(&mut world, &session, "fill", json!({"id":"pay-amount","value":"8312"}));
    browser_do(&mut world, &session, "click", json!({"id":"pay-go"}));
    let (title, all) = page(&world, &session);
    assert!(title.starts_with("PayPal Balance"), "{title}");
    assert!(shows(&all, "$400.00"), "the wallet balance after the payment");
    assert!(shows(&all, "Bob Tanaka"));
    ...
    let row = all
        .iter()
        .find(|e| e["kind"] == "link" && e["text"].as_str().is_some_and(|t| t.contains("Bob Tanaka")))
