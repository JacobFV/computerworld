...
fn every_retail_storefront_is_searched_and_an_order_is_placed_through_the_agent_api() {
    ...
    for (host, title, query, product) in RETAIL {
        ...
        assert_eq!(
            (basket["kind"].as_str(), basket["url"].as_str()),
            (Some("link"), Some(format!("http://{host}/cart").as_str()))
        );
        assert_eq!(
            by_id(&all, "nav-orders")["url"],
            format!("http://{host}/orders")
        );
        ...
        assert!(
            all.iter().any(|e| e["kind"] == "link"
                && e["id"].as_str().is_some_and(|id| id.starts_with("cat-"))),
            "{host}"
        );
        ...
        assert!(
            text_of(&all, "lead").contains(&format!("Results for \"{query}\"")),
            "{host}: {}",
            text_of(&all, "lead")
        );
        assert!(
            has(&all, &format!("p-{product}")),
            "{host}: {query} finds {product}"
        );
        ...
        let all = elements(&page(&world, &session));
        assert_eq!(
            by_id(&all, &format!("qty-{product}-n"))["value"],
            "2",
            "{host}: the cart shows the line"
        );
        ...
        let others: Vec<String> = all
            ...
            .filter(|id| {
                id.starts_with("rm-") && id.ends_with("-go") && *id != format!("rm-{product}-go")
            })
            .map(str::to_owned)
            .collect();
        for id in others {
        ...
        act(
            ...
            "browser.v1",
            "navigate",
            json!({"url": format!("http://{host}/orders")}),
        );
        let all = elements(&page(&world, &session));
        ...
        act(
            ...
            "browser.v1",
            "click",
            json!({"id": format!("o-{order}")}),
        );
        assert_eq!(
            url(&world, &session),
            format!("http://{host}/orders/{order}")
        );
    }
}
...
fn ticketmaster_sells_a_seat_through_the_agent_api() {
    ...
    act(
        ...
        "browser.v1",
        "navigate",
        json!({"url": "http://ticketmaster.com/"}),
    );
    let seen = page(&world, &session);
    assert_eq!(seen["title"], "Ticketmaster");
    let all = elements(&seen);
    assert_eq!(
        by_id(&all, "nav-basket")["url"],
        "http://ticketmaster.com/my-tickets"
    );
    assert_eq!(
        by_id(&all, "cat-conference")["url"],
        "http://ticketmaster.com/s?c=conference"
    );
    act(
        &mut world,
        &session,
    ...
    act(
        ...
        "browser.v1",
        "click",
        json!({"id": "buy-floor-go"}),
    );
    let all = elements(&page(&world, &session));
    assert!(
        text_of(&all, "lead").starts_with("Order TM-2211"),
        "{}",
        text_of(&all, "lead")
    );
    ...
    act(
        ...
        "browser.v1",
        "navigate",
        json!({"url": "http://ticketmaster.com/my-tickets"}),
    );
    let all = elements(&page(&world, &session));
    assert_eq!(
        by_id(&all, "o-TM-2211")["url"],
        "http://ticketmaster.com/orders/TM-2211"
    );
}
