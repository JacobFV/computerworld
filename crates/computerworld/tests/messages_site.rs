...
fn a_conversation_is_opened_texted_tapped_back_and_read_through_the_agent_api() {
    ...
    act(&mut world, &session, "browser.v1", "navigate", json!({"url":"http://messages.internal/"}));
    ...
    assert!(row["text"].as_str().unwrap().contains("Bob Martinez"), "{row:?}");
    assert_eq!(by_id(&all, "new")["kind"], "form");
    assert_eq!(by_id(&all, "new-to")["kind"], "input");

    act(&mut world, &session, "browser.v1", "click", json!({"id": format!("row-{AB}")}));
    ...
    assert!(all.iter().any(|e| e["text"].as_str().is_some_and(|t| t.contains("lunch?"))));
    ...
    act(&mut world, &session, "keyboard.v1", "type", json!({"text":"spicy it is"}));
    act(&mut world, &session, "browser.v1", "key", json!({"key":"Enter"}));
    let all = elements(&page(&world, &session));
    assert!(all.iter().any(|e| e["text"].as_str().is_some_and(|t| t.contains("spicy it is"))), "{all:?}");
    assert!(all.iter().any(|e| e["text"].as_str().is_some_and(|t| t.contains("Delivered"))), "{all:?}");
    ...
    act(&mut world, &session, "browser.v1", "click", json!({"id":"sms-7-tapback-laughed"}));
    let all = elements(&page(&world, &session));
    assert!(has_glyph(&all), "{all:?}");

    // Mark as read, then go back to the list.
    act(&mut world, &session, "browser.v1", "click", json!({"id":"read-submit"}));
    act(&mut world, &session, "browser.v1", "click", json!({"id":"back"}));
    assert_eq!(browser(&world, &session)["url"], "http://messages.internal/");
    let all = elements(&page(&world, &session));
    assert!(!all.iter().any(|e| e["id"] == format!("row-{AB}-unread")), "the thread is read");

    // A new conversation from the sidebar form lands on its thread.
    act(&mut world, &session, "browser.v1", "fill", json!({"id":"new-to","value":"carol"}));
    act(&mut world, &session, "browser.v1", "click", json!({"id":"new-submit"}));
