...
fn google_docs_opens_a_document_comments_edits_and_sets_a_cell_through_the_agent_api() {
    ...
    act(&mut world, &session, "navigate", json!({"url":"http://docs.google.com/"}));
    ...
    assert!(card["text"].as_str().unwrap().contains("Atlas launch checklist"), "{card:?}");
    assert!(by_id(&all, "file-q3-metrics")["text"].as_str().unwrap().contains("Sheet"));
    assert_eq!(by_id(&all, "create")["kind"], "form");
    for id in ["create-title", "create-readers", "create-writers", "create-body"] {
    ...
    assert_eq!(url(&world, &session), "http://docs.google.com/documents/atlas-launch");
    let (page, all) = elements(&world, &session);
    ...
    assert!(all
        .iter()
        .any(|e| e["kind"] == "link" && e["url"] == "http://drive.google.com/drive/folders/f-atlas"));
    ...
    fill(&mut world, &session, "comment-text", "Walkthrough is up; item 2 is done.");
    ...
    assert_eq!(by_id(&all, "star")["text"], "Starred", "alice starred the checklist in the seed");
    ...
    assert_eq!(by_id(&all, "file-atlas-launch")["url"], "http://docs.google.com/documents/atlas-launch");

    // Edit: an optimistic write with the revision the editor was showing.
    act(&mut world, &session, "navigate", json!({"url":"http://docs.google.com/documents/atlas-launch"}));
    let (_, all) = elements(&world, &session);
    let revision = by_id(&all, "edit-revision")["value"].as_str().unwrap().to_owned();
    fill(&mut world, &session, "edit-revision", &revision);
    fill(&mut world, &session, "edit-body", "Atlas launch checklist\n\nSigned off by alice.\n");
    click(&mut world, &session, "edit-submit");
    let (_, all) = elements(&world, &session);
    assert!(says(&all, "Signed off by alice."));
    assert!(says(&all, &format!("revision {}", revision.parse::<u64>().unwrap() + 1)));
    ...
    assert!(!has(&sheets, "file-atlas-launch"), "the Sheets tab lists sheets only");
    click(&mut world, &session, "file-q3-metrics");
    assert_eq!(url(&world, &session), "http://docs.google.com/documents/q3-metrics");
    let (page, all) = elements(&world, &session);
    ...
    let (_, all) = elements(&world, &session);
    assert!(says(&all, "Signed off"), "the new cell is drawn in the grid");
    ...
    assert!(by_id(&all, "slide-title-0")["text"].as_str().unwrap().contains("Where we are"));
    assert!(!has(&all, "edit-submit"), "carol owns the deck and alice may only read it");
}
...
fn notion_opens_a_page_from_the_sidebar_and_comments_through_the_agent_api() {
    let (mut world, session) = world();
    act(&mut world, &session, "navigate", json!({"url":"http://notion.so/"}));
    let (page, all) = elements(&world, &session);
    assert_eq!(page["title"], "Notion");
    assert!(says(&all, "Good morning, Alice"));
    assert_eq!(by_id(&all, "side-home")["url"], "http://notion.so/");
    assert_eq!(by_id(&all, "side-starred")["url"], "http://notion.so/starred");
    ...
    assert_eq!(entry["url"], "http://notion.so/documents/onboarding-checklist");
    ...
    assert_eq!(url(&world, &session), "http://notion.so/documents/onboarding-checklist");
    let (page, all) = elements(&world, &session);
    ...
    assert_eq!(by_id(&all, "document-title")["text"], "Onboarding checklist");
    ...
    assert_eq!(by_id(&all, "comment-text")["kind"], "input");
    fill(&mut world, &session, "comment-text", "Linked the onboarding checklist here too.");
    click(&mut world, &session, "comment-submit");
    let (_, all) = elements(&world, &session);
    assert!(says(&all, "Linked the onboarding checklist here too."));
    assert!(says(&all, "Added the Lisbon page to the list."), "the earlier comment is still there");
}
