...
fn google_drive_is_browsed_starred_filed_and_searched_through_the_agent_api() {
    ...
    act(
        ...
        "browser.v1",
        "navigate",
        json!({"url":"http://drive.google.com/"}),
    );
    ...
    assert_eq!(
        by_id(&all, "nav-shared")["url"],
        "http://drive.google.com/shared-with-me"
    );
    assert_eq!(
        by_id(&all, "nav-starred")["url"],
        "http://drive.google.com/starred"
    );
    assert_eq!(
        by_id(&all, "nav-trash")["url"],
    ...
    assert!(
        atlas["text"].as_str().unwrap().contains("Atlas"),
        "{atlas:?}"
    );
    ...
    assert_eq!(
        by_id(&all, "crumb-root")["url"],
        "http://drive.google.com/drive/folders/root"
    );
    // The shortcut is a link out of the site, to the document it stands for.
    assert_eq!(
        by_id(&all, "item-atlas-launch")["url"],
    ...
    assert!(all.iter().any(|e| e["text"]
        .as_str()
        .is_some_and(|t| t.contains("Press kit contents"))));
    ...
    act(
        ...
        "browser.v1",
        "navigate",
        json!({"url":"http://drive.google.com/drive/folders/f-personal"}),
    );
    let all = elements(&page(&world, &session));
    ...
    fill(
        ...
        &session,
        "upload-content",
        "Monitor receipt 429.99",
    );
    click(&mut world, &session, "upload-submit");
    let all = elements(&page(&world, &session));
    assert!(all.iter().any(|e| e["kind"] == "link"
        && e["text"]
            .as_str()
            .is_some_and(|t| t.contains("monitor.txt"))));
...
fn dropbox_lists_a_table_of_files_and_shares_by_link_through_the_agent_api() {
    ...
    assert!(all.iter().any(|e| e["kind"] == "link"
        && e["text"]
            .as_str()
            .is_some_and(|t| t.contains("notes-from-alice.txt"))));
    ...
    assert!(elements(&public).iter().any(|e| e["text"]
        .as_str()
        .is_some_and(|t| t.contains("Opened with a share link"))));
}
