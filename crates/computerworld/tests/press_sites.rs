...
fn read_first_story(world: &mut World, session: &str, origin: &str, brand: &str) -> String {
    ...
    assert_eq!(
        by_id(&all, "masthead-archive")["url"],
        format!("{origin}/archive")
    );
    assert_eq!(
        by_id(&all, "masthead-saved")["url"],
        format!("{origin}/saved")
    );
    ...
    assert_eq!(
        by_id(&story, "comment-text")["label"],
        "Join the discussion"
    );
...
fn the_verge_is_read_saved_commented_on_and_followed_through_the_agent_api() {
    ...
    act(
        ...
        "keyboard.v1",
        "type",
        json!({"text":"Replay or it did not happen."}),
    );
    act(
        &mut world,
        &session,
    ...
    assert!(
        has_text(&all, "Replay or it did not happen."),
        "the comment is on the page"
    );
    assert!(has_text(&all, "7 comments"));

    // Like the first comment: a one-button form that lands back on the story.
    let before = by_id(&all, "comment-c1-like")["text"]
        .as_str()
        .unwrap()
        .to_owned();
    act(
        ...
        "browser.v1",
        "click",
        json!({"id":"comment-c1-like"}),
    );
    let all = elements(&page(&world, &session));
    assert_ne!(
        by_id(&all, "comment-c1-like")["text"].as_str().unwrap(),
        before
    );
    assert!(by_id(&all, "article-title")["text"]
        .as_str()
        .unwrap()
        .contains("Atlas"));

    // Save is a toggle, and the reading list reflects it.
    let was = by_id(&all, "article-save")["text"]
        .as_str()
        .unwrap()
        .to_owned();
    act(
        ...
        "browser.v1",
        "click",
        json!({"id":"article-save"}),
    );
    let all = elements(&page(&world, &session));
    let now = by_id(&all, "article-save")["text"]
        .as_str()
        .unwrap()
        .to_owned();
    assert_ne!(was, now);
    act(
        ...
        "browser.v1",
        "navigate",
        json!({"url":"http://theverge.com/saved"}),
    );
    ...
    act(
        ...
        "browser.v1",
        "click",
        json!({"id":"article-tag-determinism"}),
    );
    assert_eq!(
        browser(&world, &session)["url"],
        "http://theverge.com/tag/determinism"
    );
    let tagged = elements(&page(&world, &session));
    assert!(tagged
        .iter()
        .any(|e| e["id"] == "card-atlas-determinism" && e["kind"] == "link"));
    let was = by_id(&tagged, "list-follow")["text"]
        .as_str()
        .unwrap()
        .to_owned();
    act(
    ...
    act(
        ...
        "browser.v1",
        "fill",
        json!({"id":"subscribe-email","value":"alice.chen@gmail.com"}),
    );
    act(
        &mut world,
        &session,
        "browser.v1",
        "click",
        json!({"id":"subscribe-submit"}),
    );
    let front = elements(&page(&world, &session));
    assert!(front
        .iter()
        .any(|e| e["id"] == "front-grid" || e["id"] == "masthead-home"));
    // Follow the publication from the masthead.
    let was = by_id(&front, "masthead-follow")["text"]
        .as_str()
        .unwrap()
        .to_owned();
    act(
        ...
        "browser.v1",
        "click",
        json!({"id":"masthead-follow"}),
    );
    let front = elements(&page(&world, &session));
    assert_ne!(
        by_id(&front, "masthead-follow")["text"].as_str().unwrap(),
        was
    );
}
...
fn every_publication_opens_its_first_story_and_its_sections_through_the_agent_api() {
    ...
    for (origin, brand, section) in [
        ...
        act(
            ...
            "browser.v1",
            "click",
            json!({"id":"article-section"}),
        );
        let listing = elements(&page(&world, &session));
        assert!(
            listing
                .iter()
                .any(|e| e["kind"] == "link" && e["url"] == story.as_str()),
            "{origin}: the section lists the story"
        );
        act(
            &mut world,
            &session,
            "browser.v1",
            "click",
            json!({"id": format!("masthead-{section}")}),
        );
        assert_eq!(
            browser(&world, &session)["url"],
