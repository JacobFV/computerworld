...
use super::view::{
    self, act, cover, div, el, field_form, here_aware, href, icon, press, short, span, Html,
};
use super::*;
...
pub fn route(
    ...
    let doc = view::document(
        ...
        vec![
            ...
            div("frame")
                .id("frame")
                .child(sidebar(s, &ctx.actor, section, &back))
        ...
        .child(
            ...
            .child(
                span("label")
                    .id("nav-new-text")
                    .child(icon("plus"))
                    .child(span("").id("nav-new-text-label").text("New playlist")),
            ),
        )
        .child(entry(
            "liked",
...
fn shelf(id: &str, title: &str, class: &str, tiles: Vec<Html>) -> Vec<Html> {
    ...
    vec![
        ...
        div(&format!("shelf {class}")).id(id).children(tiles),
    ]
}
...
fn home(state: &Value, ctx: &ServiceContext, mood: &str, here: &str) -> Vec<Html> {
    ...
    if mood.is_empty() {
        ...
        let artists: Vec<Html> = keys(state, "channels")
            .iter()
...
fn explore(state: &Value, ctx: &ServiceContext, back: &str, part: &str) -> Vec<Html> {
                    ...
                    el("a")
                        ...
                        .child(
                            span("row")
                                ...
                                .child(span("edge").id(format!("genre-{}-edge", slug(t))).style(
                                    &format!("background-color: {}", tint(&format!("mood{t}"))),
                                ))
                                .child(
                                    span("name")
    ...
    if show("charts") {
        ...
        let playing = catalog::player(state, &ctx.actor, ctx.tick)
            .and_then(|p| p.current().map(str::to_owned));
        let row = Row {
            prefix: "chart",
            context: "charts",
        ...
        out.push(
            div("tracks").id("explore-chart-tracks").each(
                catalog::charts(state)
                    .iter()
                    .take(if part.is_empty() { 10 } else { 50 })
                    .enumerate(),
                |(rank, id)| {
                    kit::track_row(
                        state,
                        ...
                        record(state, "items", id)
                            .map(|i| format!("{} plays", short(num(i, "plays")))),
                    )
                },
            ),
        );
...
fn library(state: &Value, ctx: &ServiceContext, tab: &str, back: &str) -> Vec<Html> {
    ...
    let mut out = vec![
        ...
        div("chips")
            .id("library-chips")
            .each(["playlists", "songs", "albums", "artists"], |t| {
                chip(
    ...
    match tab {
        "songs" => {
            ...
            if songs.is_empty() {
                ...
                out.push(div("tracks").id("library-songs").each(
    ...
    div("header")
        ...
        .child(
            div("identity")
                ...
                .each(lines.into_iter().enumerate(), |(i, line)| {
                    span("line").id(format!("{id}-line-{i}")).text(line)
                })
...
fn play_buttons(
    ...
    let (glyph, name) = if playing_this {
        ("pause", "Pause")
    } else {
        ("play", "Play")
    };
...
fn album_page(
    ...
    buttons.push(act(
        ...
        press("outline", "").child(
            span("label")
                ...
                .child(span("").id("album-save-text-label").text(if saved {
                    "Saved to library"
                } else {
                    "Save to library"
                })),
    ...
    vec![
        ...
        div("tracks").id("tracks").children(rows(
            state,
            ctx,
            "track",
            &album.tracks,
...
fn artist_page(state: &Value, ctx: &ServiceContext, id: &str, back: &str) -> Vec<Html> {
    ...
    let mut out = vec![div("banner")
        ...
        .child(
            div("identity")
                ...
                .child(
                    span("line")
                        .id("artist-subscribers")
                        .text(format!("{} subscribers", short(subs))),
                )
                .child(
                    div("buttons")
                        .id("artist-buttons")
                        .child(kit::command(
                            ...
                            press("solid", "").child(
                                span("label")
                                    ...
                                    .child(icon("shuffle"))
                                    .child(
                                        span("").id("artist-shuffle-text-label").text("Shuffle"),
                                    ),
                            ),
                        ))
                        .child(kit::command(
                            ...
                            press("outline", "").child(
                                span("label")
                                    .child(icon("radio"))
                                    .child(span("").id("artist-radio-text").text("Radio")),
                            ),
                        ))
                        .child(act(
                            "artist-subscribe",
                            ...
                            .child(
                                span("").id("artist-subscribe-text").text(if subscribed {
    ...
    out.push(div("tracks").id("artist-top").children(rows(
        state,
        ctx,
        "top",
        &top,
...
fn watch(state: &Value, ctx: &ServiceContext, v: &str, tab: &str, back: &str) -> Vec<Html> {
    ...
    let tab_cell = |id: &str, label: &str, on: bool, to: Option<String>| {
        let inner = [
            span("").id(format!("{id}-text")).text(label),
            span("line").id(format!("{id}-line")),
        ];
        match to {
            Some(to) => here_aware(
                el("a")
                    .id(id)
                    .class(if on { "tab on" } else { "tab" })
                    .attr("href", to.as_str())
                    .children(inner),
                &to,
                back,
    ...
    if tab == "lyrics" && !lines.is_empty() {
        ...
        side = side
            .child(
                div("lyrics")
                    .id("lyrics")
                    .children(kit::lyric_lines("lyric", &lines, at, back)),
            )
            .child(
                el("p")
                    .id("lyrics-source")
                    .class("fine")
                    .text("Lyrics synced to the music"),
            );
    } else if tab == "related" {
                    ...
                    span("row")
                        ...
                        .child(
                            span("dur")
                                ...
                                .text(clock(num(&row_item, "duration_s"))),
                        ),
                ),
            ));
        }
                        ...
                        .child(
                            span("names")
                                ...
                                .child(
                                    span("title")
                                        .id("results-top-name")
                                        .text(catalog::artist_name(state, top)),
                                )
...
fn player_bar(state: &Value, ctx: &ServiceContext, back: &str) -> Html {
    ...
    left.push(span("time").id("bar-time").text(format!(
        "{} / {}",
                    ...
                    .child(
                        div("names")
                            ...
                            .child(
                                ...
                                .child(
                                    span("").id("bar-meta-text").text(match album {
                                        Some(a) => {
